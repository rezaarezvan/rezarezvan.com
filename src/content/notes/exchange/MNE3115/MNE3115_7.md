...
where $Q\,[\mathrm{J}] = [\mathrm{kg\,m^2/s^2}]$ amount of **Energy** produced (in the form of heat).

$I\,[\mathrm{A}]$ is the **Current** passing through the wire.

$R\,[\Omega]$ is the **Resistance** of the wire.

$t\,[\mathrm{s}]$ is the **Time** the current is passed through the wire.
...
where $C\,[\mathrm{J/(kg\,K)}]$ is the **Specific Heat of Capacity** of the material.

$M\,[\mathrm{kg}]$ is the **Mass** of the material.

$\Delta T\,[\mathrm{K}]$ is the **Temperature Change** of the material.
...
where $A_{\text{total}}\,[\mathrm{m^2}]$ is the **Total Area** attaching with nearby bodies.

$\kappa\,[\mathrm{W/(m\,K)}]$ is the **Thermal Conductivity** of the material.

$\Delta T\,[\mathrm{K}]$ is the **Temperature Difference** relative to the nearby material.
...
V^2 & = \xi L \left(\frac{A_{\text{total}}}{A}\right) \kappa \Delta T.
\end{align*}
$$
...
Therefore, we can convert **electrical** voltage to **temperature**, and to **mechanical deformation/deflection**.
...
* The wide arm
    ...
    - Large area &rarr; Heat loss due to convection.
...
#### Metal Strain Gauge
For metals, the resistivity is not changed significantly by the stress.
...
& = \frac{6F\left(l + \frac{L}{2}\right)}{Ewt^2}.
\end{align*}
$$
...
$$
\frac{\Delta R}{R} = G \varepsilon_{\text{max}} = \frac{6GF\left(l + \frac{L}{2}\right)}{Ewt^2} = \left(\frac{6Gm\left(l + \frac{L}{2}\right)}{Ewt^2}\right) a.
$$
