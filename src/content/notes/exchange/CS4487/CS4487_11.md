...
The architecture was designed for hand-written digit recognition (MNIST).
...
It has a similar architecture to LeNet, but deeper (14 layers). It has $11 \times 11$, $5 \times 5$, and $3 \times 3$ convolutions.
...
Vision Transformers (ViTs) emerge as a competitive alternative to CNNs that are currently the state-of-the-art (SOTA) in computer vision and widely used for various image recognition tasks @cite:dosovitskiy2021vit.
...
It uses both a text encoder (transformers) and an image encoder (ViT or ResNet architectures).
...
Not only does this involve recognizing the object categories within an image, but also accurately determining their locations in it.
...
SRCNN maps low-resolution features nonlinearly to high-resolution representations.
...
$$
\hat{y}_i = \operatorname{round}(y_i), \quad p_{\hat{y}_i}(n) = \int_{n - \frac{1}{2}}^{n + \frac{1}{2}} p_{y_i}(t) \, dt, \quad \text{for all } n \in \mathbb{Z},
$$

$$
\tilde{y}_i = y_i + \Delta y_i, \quad \text{for } \Delta y_i \sim \mathcal{U}\left(-\frac{1}{2}, \frac{1}{2}\right).
$$

The density function of $\tilde{y}_i$ is a continuous relaxation of the probability mass function of $\hat{y}_i$, identical at integer values.
...
* Cast a regression task into a multiclass classification task to cope with the inherent ambiguity and multimodal nature of colorization.
...
This is mainly accomplished by a weighted summation framework $\mathbf{y} = \sum_{k=1}^K \mathbf{w}_k \odot \mathbf{x}_k$.
...
#### Mean Squared Error (MSE)
$$
\operatorname{MSE} = \frac{1}{N} \sum_{i=1}^N (\mathbf{x}_i - \hat{\mathbf{x}}_i)^2,
$$

where $\mathbf{x}_i$ is the ground-truth and $\hat{\mathbf{x}}_i$ is the prediction.

#### Why Do We Love MSE?
It is simple, parameter-free, cheap to compute and also memory-less!

It is per definition a valid *distance* metric (i.e., satisfying non-negativity, identity, symmetry, and triangular inequality).
...
$$
D(\mathbf{x}, \hat{\mathbf{x}}) = \frac{1}{N} \sum_{i=1}^N (\mathbf{f}(\mathbf{x})_i - \mathbf{f}(\hat{\mathbf{x}})_i)^2.
$$
...
$$
\operatorname{SSIM}(\mathbf{x}, \hat{\mathbf{x}}) = l(\mathbf{x}, \hat{\mathbf{x}}) \cdot s(\mathbf{x}, \hat{\mathbf{x}}) = \left(\frac{2 \mu_{\mathbf{x}} \mu_{\hat{\mathbf{x}}} + c_1}{\mu_{\mathbf{x}}^2 + \mu_{\hat{\mathbf{x}}}^2 + c_1}\right) \cdot \left( \frac{2 \sigma_{\mathbf{x} \hat{\mathbf{x}}} + c_2}{\sigma_{\mathbf{x}}^2 + \sigma_{\hat{\mathbf{x}}}^2 + c_2} \right).
$$
...
$$
\operatorname{LPIPS}(\mathbf{x}, \hat{\mathbf{x}}) = \sum_{i=1}^S \sum_{j=1}^{N_i} w_{ij} \operatorname{MSE}(\mathbf{f}(\mathbf{x})_j^{(i)}, \mathbf{f}(\hat{\mathbf{x}})_j^{(i)}).
$$
...
$$
\operatorname{DISTS}(\mathbf{x}, \hat{\mathbf{x}}) = 1 - \sum_{i=0}^S \sum_{j=1}^{N_i} \left( \alpha_{ij} l\left( \mathbf{x}_j^{(i)}, \hat{\mathbf{x}}_j^{(i)} \right) + \beta_{ij} s\left( \mathbf{x}_j^{(i)}, \hat{\mathbf{x}}_j^{(i)} \right) \right).
$$

### Summary
Advances of deep learning have been driven by the ImageNet competition.

As depth increases, need to have a smart architecture design to make training more effective.

Deep learning also begins to dominate low-level vision, e.g., image denoising, deblurring, super-resolution, compression etc.
