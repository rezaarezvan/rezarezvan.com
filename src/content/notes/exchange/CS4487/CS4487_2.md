...
It has the *Naive Bayes assumption*, which assumes that all of the feature dimensions are statistically independent given the value of the class variable, for example,
$$
...
& - \left( \log p(y = 2) + \sum_{j = 1}^2 \left(-\frac{1}{2} \log(2\pi\sigma_{j | 2}^2) - \frac{(x_j - \mu_{j | 2})^2}{2\sigma_{j | 2}^2}\right) \right) = 0.
\end{align*}
$$
...
$$
p(x_1, \ldots, x_N | y = c) = \prod_{j = 1}^{N} p(x_j | y = c), \quad \log p(x_1, \ldots, x_N | y = c) = \sum_{j = 1}^{N} \log p(x_j | y = c).
$$
...
$$
p(x_j = x | y = c) = \varphi_{x, j | c}, \quad \sum_{x \in \chi_j} \varphi_{x, j | c} = 1.
$$
...
It can be interpreted as an approximation to the Bayes optimal classifier, but for real-valued data.

Instead of a product of independent Gaussians, LDA assumes that $p(\mathbf{x} | y = c) = \mathcal{N}(\mathbf{x}; \boldsymbol{\mu}_c, \mathbf{\Sigma})$.
A multivariate Gaussian with a shared covariance matrix $\mathbf{\Sigma}$.

Which equates to,
$$
p(\mathbf{x} | y = c) = \frac{1}{|(2\pi)^N \mathbf{\Sigma}|^{\frac{1}{2}}} \exp\left(-\frac{1}{2} (\mathbf{x} - \boldsymbol{\mu}_c)^T \mathbf{\Sigma}^{-1} (\mathbf{x} - \boldsymbol{\mu}_c)\right).
$$
...
Class means are,
$$
\boldsymbol{\mu}_c = \frac{\sum_{i = 1}^M \mathbb{I}[y^{(i)} = c] \mathbf{x}^{(i)}}{\sum_{i = 1}^M \mathbb{I}[y^{(i)} = c]} = \frac{\sum_{i = 1}^{M_c} \mathbf{x}^{(i)}}{M_c},
$$
...
$$
\mathbf{\Sigma} = \frac{1}{M} \sum_{i = 1}^M \left(\mathbf{x}^{(i)} - \boldsymbol{\mu}_{y^{(i)}}\right) \left(\mathbf{x}^{(i)} - \boldsymbol{\mu}_{y^{(i)}}\right)^T.
$$

#### Geometric Interpretation
Let's take a look at the geometry of the decision boundary for LDAs.

The decision boundary is the set of points $\mathbf{x}$ where,
$$
\begin{align*}
& \log p(y = 1) - \frac{1}{2} \log |(2\pi)^2 \mathbf{\Sigma}| - \frac{1}{2}(\mathbf{x} - \boldsymbol{\mu}_1)^T \mathbf{\Sigma}^{-1} (\mathbf{x} - \boldsymbol{\mu}_1) \newline
& - \left( \log p(y = 2) - \frac{1}{2} \log |(2\pi)^2 \mathbf{\Sigma}| - \frac{1}{2}(\mathbf{x} - \boldsymbol{\mu}_2)^T \mathbf{\Sigma}^{-1} (\mathbf{x} - \boldsymbol{\mu}_2) \right) = 0.
\end{align*}
$$

By cancelling out the common terms, we get,
$$
\log \frac{p(y = 1)}{p(y = 2)} - \frac{1}{2} \boldsymbol{\mu}_1^T \mathbf{\Sigma}^{-1} \boldsymbol{\mu}_1 + \frac{1}{2} \boldsymbol{\mu}_2^T \mathbf{\Sigma}^{-1} \boldsymbol{\mu}_2 + (\boldsymbol{\mu}_1 - \boldsymbol{\mu}_2)^T \mathbf{\Sigma}^{-1} \mathbf{x} = 0.
$$
...
From this value generated by the function, if $f(\mathbf{x}) > 0$, we predict class $+1$, otherwise we predict class $-1$, or just $\operatorname{sign}(f(\mathbf{x}))$.

##### Geometric Interpretation
As the name suggests, the linear classifier separates the feature space into 2 half-spaces.
...
$$
\sigma(z) = \frac{1}{1 + e^{-z}}, \quad \text{for } z \in \mathbb{R}.
$$
...
p(y = -1 | \mathbf{x}) & = 1 - \sigma(f(\mathbf{x})) = \sigma(-f(\mathbf{x})), \quad \text{since } \sigma(-z) = 1 - \sigma(z).
\end{align*}
$$
...
So the important thing here is that we are **directly** modeling the class posterior probability, not the class-conditional probability.
...
If we define $z^{(i)} = y^{(i)} f(\mathbf{x}^{(i)})$, we have an interesting observation:
...
:::algorithm[Cross-validation]{#alg:cross-validation}
...
3. Select the value of $C$ that has the highest average accuracy over all $K$ rounds.
...
For more than 2 classes, a good strategy is to simply, split the problem into several binary classifier problems.
...
When predicting, calculate the probability for each binary classifier. Select the class with the highest probability.
...
$$
p(y = c | \mathbf{x}) = \frac{\exp(\mathbf{w}_c^T \mathbf{x})}{\exp(\mathbf{w}_1^T \mathbf{x}) + \ldots + \exp(\mathbf{w}_C^T \mathbf{x})}.
$$

Here we need to estimate the $\{\mathbf{w}_c\}_{c = 1}^C$ parameters using MLE as before.

#### Geometry of Multiclass Logistic Regression
In logistic regression, it is explicitly designed to have a linear decision boundary in the binary case.
