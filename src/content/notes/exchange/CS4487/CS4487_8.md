...
### Eigendecomposition
Let $\mathbf{V} \in \mathbb{R}^{N \times N}$ be a matrix whose columns $\mathbf{v}_i$ are $N$ linearly independent eigenvectors of $\mathbf{A}$ with $\mathbf{\Lambda}$ the corresponding diagonal matrix of eigenvalues such that $\Lambda_{ii} = \lambda_i$.
...
If $\mathbf{A}$ is a real symmetric matrix, we can choose $N$ orthonormal eigenvectors so that $\Vert \mathbf{v}_i \Vert_2^2 = 1$, $\mathbf{v}_i^T \mathbf{v}_j = 0$ and $N$ real eigenvalues $\lambda_i \in \mathbb{R}$.
...
Let $\mathbf{v} \in \mathbb{R}^N$ such that $\Vert \mathbf{v} \Vert_2^2 = \mathbf{v}^T \mathbf{v} = 1$.

The variance in the direction $\mathbf{v}$ is given by the expression,

$$
\frac{1}{M} \sum_{i = 1}^M (\mathbf{v}^T \mathbf{x}^{(i)} - \boldsymbol{\mu})^2,
$$

where $\boldsymbol{\mu} = \frac{1}{M} \sum_{i = 1}^M \mathbf{v}^T \mathbf{x}^{(i)}$ is the mean of the projected data.
...
& = \frac{1}{M} \sum_{i = 1}^M \left(\mathbf{v}^T \mathbf{x}^{(i)}\right) \left((\mathbf{x}^{(i)})^T \mathbf{v}\right) \newline
...
$$
\begin{align*}
2 \mathbf{v}_1^T \mathbf{\Sigma} \mathbf{v} - 2 \nu \mathbf{v}_1^T \mathbf{v} + \lambda \mathbf{v}_1^T \mathbf{v}_1 & = 0 \newline
2(\mathbf{\Sigma} \mathbf{v}_1)^T \mathbf{v} - 0 + \lambda & = 0 \newline
2 \sigma_1 \mathbf{v}_1^T \mathbf{v} - 0 + \lambda & = 0 \newline
\lambda & = 0.
\end{align*}
$$
...
If higher-order dependencies exist between the features in the data, PCA may be insufficient at revealing all structure in the data.
...
#### Feature Mapping + SVD
Given a data set $\mathbf{X} \in \mathbb{R}^{M \times N}$ and a feature mapping function $\phi : \mathbb{R}^N \to \mathbb{R}^L$ for $L > N$, we obtain the following SVD-based algorithm,

:::algorithm[Feature mapping + SVD]{#alg:feature-svd}
1. Compute $\mathbf{U}, \mathbf{S}, \mathbf{V} = \operatorname{SVD}(\phi(\mathbf{X}))$.
2. Return $\mathbf{Z} = \mathbf{U} \mathbf{S}$.
:::

#### Feature Mapping + PCA
Given a data set $\mathbf{X} \in \mathbb{R}^{M \times N}$ and a feature mapping function $\phi : \mathbb{R}^N \to \mathbb{R}^L$ for $L > N$, we obtain the following PCA-based algorithm,

:::algorithm[Feature mapping + PCA]{#alg:feature-pca}
1. Compute $\mathbf{\Sigma} = \frac{1}{M} \sum_i (\phi(\mathbf{x}^{(i)}) - \boldsymbol{\mu}) (\phi(\mathbf{x}^{(i)}) - \boldsymbol{\mu})^T$,
   where $\boldsymbol{\mu} = \frac{1}{M} \sum_i \phi(\mathbf{x}^{(i)})$.
...
Given $\phi : \mathbb{R}^N \to \mathbb{R}^L$, we can compute the covariance matrix in the new feature space,

$$
\mathbf{\Sigma} = \frac{1}{M} \sum_{i = 1}^M \phi(\mathbf{x}^{(i)}) \phi(\mathbf{x}^{(i)})^T.
$$
...
$$
\mathbf{\Sigma} \mathbf{v}_k = \frac{1}{M} \sum_{i = 1}^M \phi(\mathbf{x}^{(i)}) \phi(\mathbf{x}^{(i)})^T \mathbf{v}_k = \lambda_k \mathbf{v}_k, \quad \forall k = 1, \ldots, L.
$$

It is not hard to see that $\mathbf{v}_k$ can be expressed as,

$$
\mathbf{v}_k = \sum_{j = 1}^M w_k^{(j)} \phi(\mathbf{x}^{(j)}),
$$
...
$$
\phi(\mathbf{x}^{\star})^T \mathbf{v}_k = \sum_{i = 1}^M w_k^{(i)} \phi(\mathbf{x}^{\star})^T \phi(\mathbf{x}^{(i)}) = \sum_{i = 1}^M w_k^{(i)} \mathcal{K}(\mathbf{x}^{\star}, \mathbf{x}^{(i)}).
$$

#### Kernel PCA Algorithm
Given a data set $\mathbf{X} \in \mathbb{R}^{M \times N}$ and a kernel function $\mathcal{K}$, kernel PCA can be computed as follows,

:::algorithm[Kernel PCA]{#alg:kernel-pca}
...
### Summary
Kernel PCA uses the kernel trick to perform PCA in high-dimensional space.
