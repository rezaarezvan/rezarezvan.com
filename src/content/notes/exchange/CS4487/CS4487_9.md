...
Other synapses are electrical (i.e., ions flow directly between cells).
...
![Multiple Perceptrons.](./imgs/network.svg "fig:perceptron-layer")
...
We have an input $\mathbf{x} \in \mathbb{R}^N$, which is an $N$-dim vector.
...
$$
y = f(\mathbf{w}^T \mathbf{x}) = f\left(\sum_{j=0}^{N} w_j x_j\right),
$$
...
Since the computational power was not enough back then, they could only look at one data point at a time, with for example stochastic gradient descent (SGD).

:::algorithm[Perceptron]{#alg:perceptron}
**Procedure:**
...
3. If $y^{(i)} a^{(i)} \leq 0$, then $\mathbf{w}^{(t+1)} = \mathbf{w}^{(t)} + \alpha y^{(i)} \mathbf{x}^{(i)}$ and $t \leftarrow t+1$.
...
* If $y^{(i)} = 1$ and $\hat{y}^{(i)} = \operatorname{sign}(a^{(i)}) = -1$, the activation is initially negative and will be increased.
* If $y^{(i)} = -1$ and $\hat{y}^{(i)} = \operatorname{sign}(a^{(i)}) = 1$, the activation is initially positive and will be decreased.
...
![Activation Functions.](./imgs/activation_functions.svg "fig:activation-functions")
...
$$
\operatorname{Maxout}(\mathbf{x}; \mathbf{w}_1, \mathbf{w}_2) = \max(\mathbf{w}_1^T \mathbf{x}, \mathbf{w}_2^T \mathbf{x}).
$$
...
$$
\operatorname{Swish}(x) = x \cdot \sigma(\beta x) = \frac{x}{1 + \exp(-\beta x)}.
$$
...
$$
\begin{aligned}
\frac{\partial \ell}{\partial \mathbf{W}_2} &= \frac{\partial \ell}{\partial g_2} \frac{\partial g_2}{\partial \mathbf{W}_2}, \newline
\frac{\partial \ell}{\partial \mathbf{W}_1} &= \frac{\partial \ell}{\partial g_2} \frac{\partial g_2}{\partial g_1} \frac{\partial g_1}{\partial \mathbf{W}_1}.
\end{aligned}
$$
...
* Circular Padding
    - Copy the pixels at the edges of the images and append them to the opposite side of the image.
...
:::algorithm[Discrete convolution]{#alg:discrete-convolution}
