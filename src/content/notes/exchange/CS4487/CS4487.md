...
:::definition[Matrix]
A matrix $\mathbf{X} \in \mathbb{R}^{M \times N}$ is a rectangular array of elements $x_{ij} \in \mathbb{R}$, $1 \leq i \leq M$, $1 \leq j \leq N$.
$$
...
* **Additivity**: For all $\omega$, $\omega^\prime \subseteq \Omega$ that are **disjoint** sets, $p(\omega \cup \omega^\prime) = p(\omega) + p(\omega^\prime)$.
:::
...
For each $x \in \chi$, the **event** $\{X = x\}$ refers to the subset of the sample space $\{\omega \mid \omega \in \Omega, f_x(\omega) = x\}$.

For each $x \in \chi$, the probability $p(X = x) = p(\{\omega \mid \omega \in \Omega, f_x(\omega) = x\})$.
:::
...
$$
\mathcal{Y} = \{ \text{"versicolor"}, \text{"setosa"}, \text{"virginica"} \}, \quad \text{where } |\mathcal{Y}| = C = 3.
$$
...
learn a function $f: \mathbb{R}^N \to \mathcal{Y}$ that accurately predicts the class label $y$ for any feature vector $\mathbf{x}$.
:::
...
Given a data set of example pairs $\mathcal{D} = \{ (\mathbf{x}^{(i)}, y^{(i)}), i = 1, \ldots, M \}$ and a function $f: \mathbb{R}^N \to \mathcal{Y}$,
the classification error rate of $f$ on $\mathcal{D}$ is defined as:
$$
\operatorname{Err}(f, \mathcal{D}) = \frac{1}{M} \sum_{i=1}^{M} \mathbb{I}[y^{(i)} \neq f(\mathbf{x}^{(i)})].
$$
:::
...
$$
\operatorname{Acc}(f, \mathcal{D}) = \frac{1}{M} \sum_{i=1}^{M} \mathbb{I}[y^{(i)} = f(\mathbf{x}^{(i)})] = 1 - \operatorname{Err}(f, \mathcal{D}).
$$
:::
...
Use of the KNN requires choosing the distance function $d : \mathbb{R}^N \times \mathbb{R}^N \to \mathbb{R}$ and the number of neighbors $K$.
...
Generally, the more structure the distance function has (symmetry, triangle inequality, etc.), the more structure you can exploit when designing algorithms.

The Minkowski distance ($\ell_p$ norm) is defined as:

:::definition[Minkowski distance]
Given two data vectors $\mathbf{x}, \mathbf{x}^\prime \in \mathbb{R}^N$, the Minkowski distance with parameter $p$ (the $\ell_p$ norm) is a proper metric defined as:
$$
\begin{align*}
...
$$
f_{KNN}(\mathbf{x}) = \underset{c \in \{1, \ldots, C\}}{\arg\max} \frac{\sum w_i \mathbb{I}[y^{(i)} = c]}{\sum w_i}, \quad w_i = \exp(-\alpha d_i).
$$
...
In the real world, the frequency that class $y$ occurs is given by the probabilistic distribution $p(y)$.
...
Let $\mathcal{D}_c = \{x^{(i)} \mid y^{(i)} = c\}$ for $i = 1, \ldots, M_c$ be the data for class $c$.
