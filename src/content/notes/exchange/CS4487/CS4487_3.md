...
$$
p(y = +1 | \mathbf{x}) = \frac{1}{1 + \exp(-(\mathbf{w}^T \mathbf{x} + b))}, \quad \log\left(\frac{p(y = +1 | \mathbf{x})}{p(y = -1 | \mathbf{x})}\right) = \mathbf{w}^T \mathbf{x} + b.
$$
...
However, the one drawback with logistic regression compared to the previous models that we have covered is learning the parameters.
...
$$
\underset{\boldsymbol{\theta}}{\min} \ \ell(\mathcal{D}; \boldsymbol{\theta}) = \underset{\boldsymbol{\theta}}{\min} \frac{1}{M} \sum_{i=1}^M \ell(\mathbf{x}^{(i)}, y^{(i)}; \boldsymbol{\theta}) + r(\boldsymbol{\theta}),
$$

where $\boldsymbol{\theta}$ is a parameter vector to be optimized.

$\mathcal{D} = \{(\mathbf{x}^{(i)}, y^{(i)}), i = 1, \ldots, M\}$ is a finite training set.

$r(\cdot)$ is a regularizer to prevent overfitting to $\mathcal{D}$.

In simple terms, we want to find the parameter vector $\boldsymbol{\theta}$ that minimizes the loss function $\ell(\cdot)$.

In logistic regression ($\boldsymbol{\theta} = \{ \mathbf{w}, b \}$),
$$
\ell(\mathbf{x}^{(i)}, y^{(i)}; \boldsymbol{\theta}) = \log(1 + \exp(-y^{(i)}(\mathbf{w}^T \mathbf{x}^{(i)} + b))), \quad r(\boldsymbol{\theta}) = \frac{1}{C} \mathbf{w}^T \mathbf{w}.
$$
...
$$
\{ \mathbf{x} \mid \mathbf{a}^T \mathbf{x} = b \}, \quad \mathbf{a} \neq 0,
$$
...
$$
\{ \mathbf{x} \mid \mathbf{a}^T \mathbf{x} \leq b \}, \quad \mathbf{a} \neq 0.
$$
...
* On $\mathbb{R}^{M \times N}$ ($M \times N$ matrices)
    - Affine function:
    $$
    f(\mathbf{X}) = \sum_{i = 1}^M \sum_{j = 1}^N A_{ij} X_{ij} + b = \operatorname{tr}(\mathbf{A}^T \mathbf{X}) + b.
    $$

    - Spectral norm (maximum singular value):
    $$
    f(\mathbf{X}) = \Vert \mathbf{X} \Vert_2 = \sigma_{\max}(\mathbf{X}) = (\lambda_{\max}(\mathbf{X}^T \mathbf{X}))^{1/2}.
    $$
...
For example $\ell(\boldsymbol{\theta}) = \boldsymbol{\theta}^2$.
...
$\nabla \ell(\boldsymbol{\theta})$ is the gradient of $\ell(\boldsymbol{\theta})$ evaluated at $\boldsymbol{\theta}$.

So, with our example $\ell(\boldsymbol{\theta}) = \boldsymbol{\theta}^2$, $\nabla \ell(\boldsymbol{\theta}) = 2\boldsymbol{\theta}$.

We can repeat this process until convergence, so GD first chooses an initial point $\boldsymbol{\theta}^{(0)}$ and repeats,
$$
\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} - \alpha \nabla \ell(\mathcal{D}; \boldsymbol{\theta}^{(t)}) = \boldsymbol{\theta}^{(t)} - \alpha \frac{1}{M} \sum_{i=1}^M \nabla \ell(\mathbf{x}^{(i)}, y^{(i)}; \boldsymbol{\theta}^{(t)}).
$$

How do we stop?
$$
\Vert \boldsymbol{\theta}^{(t+1)} - \boldsymbol{\theta}^{(t)} \Vert_2 < \epsilon \quad \text{or} \quad \Vert \nabla \ell(\mathcal{D}; \boldsymbol{\theta}^{(t)}) \Vert_2 < \epsilon,
$$

where $\Vert \boldsymbol{\theta} \Vert_2 = \sqrt{\sum_{j} \boldsymbol{\theta}_j^2}$, is the $\ell_2$-norm of $\boldsymbol{\theta}$.
...
$$
\begin{align*}
\ell(\boldsymbol{\theta}^{(1)}) & \approx \ell(\boldsymbol{\theta}^{(0)}) + \nabla \ell(\boldsymbol{\theta}^{(0)})^T (\boldsymbol{\theta}^{(1)} - \boldsymbol{\theta}^{(0)}) \newline
& = \ell(\boldsymbol{\theta}^{(0)}) + \nabla \ell(\boldsymbol{\theta}^{(0)})^T (-\alpha \nabla \ell(\boldsymbol{\theta}^{(0)})) \newline
& = \ell(\boldsymbol{\theta}^{(0)}) - \alpha \Vert \nabla \ell(\boldsymbol{\theta}^{(0)}) \Vert_2^2.
\end{align*}
$$

Hence, if $\nabla \ell(\boldsymbol{\theta}^{(0)}) \neq 0$ and $\alpha$ is sufficiently small, we have,
$$
\ell(\boldsymbol{\theta}^{(1)}) - \ell(\boldsymbol{\theta}^{(0)}) \approx -\alpha \Vert \nabla \ell(\boldsymbol{\theta}^{(0)}) \Vert_2^2 < 0.
$$
...
At $t$-th iteration, randomly sample a single training example and compute the gradient to update $\boldsymbol{\theta}^{(t)}$.
$$
\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} - \alpha \nabla \ell(\mathbf{x}^{(i)}, y^{(i)}; \boldsymbol{\theta}^{(t)}).
$$
...
$$
\mathbb{E}[\nabla \ell(\mathbf{x}, y; \boldsymbol{\theta})] = \frac{1}{M} \sum_{i=1}^M \nabla \ell(\mathbf{x}^{(i)}, y^{(i)}; \boldsymbol{\theta}) = \nabla \ell(\mathcal{D}; \boldsymbol{\theta}).
$$
...
Unlike SGD, mini-batch gradient descent randomly samples a small subset of training data $\mathcal{B} \subset \mathcal{D}$ at $t$-th iteration, and compute the gradient to update $\boldsymbol{\theta}^{(t)}$.
$$
\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} - \alpha \nabla \ell(\mathcal{B}; \boldsymbol{\theta}^{(t)}) = \boldsymbol{\theta}^{(t)} - \alpha \frac{1}{|\mathcal{B}|} \sum_{(\mathbf{x}, y) \in \mathcal{B}} \nabla \ell(\mathbf{x}, y; \boldsymbol{\theta}^{(t)}).
$$
...
$$
y^{\star} = \operatorname{sign}(\mathbf{w}^T \mathbf{x}^{\star} + b).
$$
...
Allow some training samples to violate the margin. I.e., are inside the margin (or even misclassified).
...
* **Classifier**
    ...
    * Given new sample $\mathbf{x}^{\star}$, predict $y^{\star} = \operatorname{sign}(\mathbf{w}^T \mathbf{x}^{\star} + b)$.
* **Training**
...
* **Linear classifiers**
    ...
    * $y = \operatorname{sign}(\mathbf{w}^T \mathbf{x} + b)$.
* **Two formulations**
