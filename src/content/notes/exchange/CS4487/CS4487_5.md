...
Given a data set of example pairs $\mathcal{D} = \{(\mathbf{x}^{(i)}, y^{(i)}), i = 1, \ldots, M \}$ and a function $f : \mathbb{R}^N \to \mathbb{R}$, the MSE of $f$ on $\mathcal{D}$ is defined as,
$$
...
Or equivalently,
$$
y = \mathbf{w}^T \mathbf{x} + w_0 = \sum_{i=1}^{N} w_i x_i + w_0 = \sum_{i=0}^{N} w_i x_i, \quad x_0 = 1.
$$
...
Let's solve this for one feature, i.e., take the derivative of the objective with respect to $w$ and set it to zero.
$$
\begin{align*}
...
\frac{\partial}{\partial \mathbf{w}} \frac{1}{M} (\mathbf{y}^T - (\mathbf{X} \mathbf{w})^T)(\mathbf{y} - \mathbf{X} \mathbf{w}) & = 0 \newline
\frac{\partial}{\partial \mathbf{w}} \frac{1}{M} (\mathbf{y}^T \mathbf{y} - \mathbf{y}^T \mathbf{X} \mathbf{w} - (\mathbf{X} \mathbf{w})^T \mathbf{y} + (\mathbf{X} \mathbf{w})^T \mathbf{X} \mathbf{w}) & = 0 \newline
\frac{\partial}{\partial \mathbf{w}} \frac{1}{M} (\mathbf{y}^T \mathbf{y} - \mathbf{y}^T \mathbf{X} \mathbf{w} - \mathbf{w}^T \mathbf{X}^T \mathbf{y} + \mathbf{w}^T \mathbf{X}^T \mathbf{X} \mathbf{w}) & = 0 \quad \| \ (AB)^T = B^T A^T \newline
\frac{\partial}{\partial \mathbf{w}} \frac{1}{M} (\mathbf{y}^T \mathbf{y} - \mathbf{y}^T \mathbf{X} \mathbf{w} - \mathbf{y}^T \mathbf{X}\mathbf{w} + \mathbf{w}^T \mathbf{X}^T \mathbf{X} \mathbf{w}) & = 0 \quad \| \ (AB)^T = B^T A^T \newline
\frac{\partial}{\partial \mathbf{w}} \frac{1}{M} (\mathbf{y}^T \mathbf{y} - 2 \mathbf{y}^T \mathbf{X} \mathbf{w} + \mathbf{w}^T \mathbf{X}^T \mathbf{X} \mathbf{w}) & = 0.
\end{align*}
$$
...
$$
\mathbf{y} = \mathbf{w}^T \mathbf{x} + \boldsymbol{\epsilon}, \quad \text{where } p(\boldsymbol{\epsilon}; \sigma^2) = \mathcal{N}(\boldsymbol{\epsilon}; 0, \sigma^2 \mathbf{I}).
$$
...
To get a better shrinkage to zero, we can change the regularization term to encourage more weights to be zero.
...
Which is just,
$$
\operatorname{sign}(\mathbf{w}_L) \max(|\mathbf{w}_L| - \alpha, 0).
$$
