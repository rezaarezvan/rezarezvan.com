...
$$
L(\boldsymbol{\theta}) = \sum_{i = 1}^M \log p(\mathbf{x}^{(i)}; \boldsymbol{\theta}) = \sum_{i = 1}^M \log \sum_{\mathbf{z}^{(i)} = 1}^K p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta}).
$$

* $\boldsymbol{\theta}$: Parameters of the probabilistic model we are trying to find.
...
Suppose $f : \mathbb{R} \to \mathbb{R}$ is **concave**, then for all probability distributions $p$, we have,
...
&= \sum_{i = 1}^M \log \sum_{\mathbf{z}^{(i)} = 1}^K p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta}) \newline
&= \sum_{i = 1}^M \log \sum_{\mathbf{z}^{(i)} = 1}^K q(\mathbf{z}^{(i)}) \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta})}{q(\mathbf{z}^{(i)})} \newline
&= \sum_{i = 1}^M \log \mathbb{E}_{\mathbf{z}^{(i)} \sim q} \left[ \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta})}{q(\mathbf{z}^{(i)})} \right].
\end{aligned}
$$
...
$$
\begin{aligned}
L(\boldsymbol{\theta}) &\geq \sum_{i = 1}^M \mathbb{E}_{\mathbf{z}^{(i)} \sim q} \left[ \log \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta})}{q(\mathbf{z}^{(i)})} \right].
\end{aligned}
$$
...
$$
\begin{aligned}
L(\boldsymbol{\theta}) &\geq \sum_{i = 1}^M \sum_{\mathbf{z}^{(i)} = 1}^K q(\mathbf{z}^{(i)}) \log \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta})}{q(\mathbf{z}^{(i)})} \newline
&= \sum_{i = 1}^M \sum_{\mathbf{z}^{(i)} = 1}^K q(\mathbf{z}^{(i)}) \log p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta}) \newline
& \quad - \sum_{i = 1}^M \sum_{\mathbf{z}^{(i)} = 1}^K q(\mathbf{z}^{(i)}) \log q(\mathbf{z}^{(i)}).
\end{aligned}
$$

Let's call this last expression for $\ell(\boldsymbol{\theta})$, this is a lower bound of the original objective $L(\boldsymbol{\theta})$.
The equality holds when $\frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta})}{q(\mathbf{z}^{(i)})}$ is a constant.

This can be achieved for $q(z^{(i)}) = p(z^{(i)} | x^{(i)}; \boldsymbol{\theta})$.

The EM algorithm aims to optimize the lower bound $\ell(\boldsymbol{\theta})$,

$$
\boldsymbol{\theta}^{\star} = \underset{\boldsymbol{\theta}}{\arg \max} \ell(\boldsymbol{\theta}) = \underset{\boldsymbol{\theta}}{\arg \max} \sum_{i = 1}^M \sum_{\mathbf{z}^{(i)} = 1}^K q(\mathbf{z}^{(i)}) \log \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta})}{q(\mathbf{z}^{(i)})}.
$$
...
1. E-step: For each index $i$, we compute,
   $$
   q^{(t)}(z^{(i)}) = p(z^{(i)} | x^{(i)}; \boldsymbol{\theta}^{(t)}).
   $$
2. M-step: Compute,
   $$
   \boldsymbol{\theta}^{(t + 1)} = \underset{\boldsymbol{\theta}}{\arg \max} \sum_{i = 1}^M \sum_{\mathbf{z}^{(i)} = 1}^K q^{(t)}(\mathbf{z}^{(i)}) \log p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta}).
   $$
:::

In the E-step, we do not fill in the unobserved $z^{(i)}$ with hard values, but find a posterior distribution $q(z^{(i)})$, given $x^{(i)}$ and $\boldsymbol{\theta}^{(t)}$, i.e.,

$$
q^{(t)}(z^{(i)}) = p(z^{(i)} | x^{(i)}; \boldsymbol{\theta}^{(t)}).
$$

In the M-step, we maximize the lower bound $\ell(\boldsymbol{\theta})$, while holding $q^{(t)}(z^{(i)})$ fixed, which is computed from the E-step.
...
$$
\begin{aligned}
L(\boldsymbol{\theta}^{(t)}) &\stackrel{(1)}{=} \sum_{i = 1}^M \log p(\mathbf{x}^{(i)}; \boldsymbol{\theta}^{(t)}) \stackrel{(2)}{=} \sum_{i = 1}^M \log \sum_{\mathbf{z}^{(i)} = 1}^K q(\mathbf{z}^{(i)}) \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta}^{(t)})}{q(\mathbf{z}^{(i)})} \newline
& \stackrel{(3)}{=} \sum_{i = 1}^M \sum_{\mathbf{z}^{(i)} = 1}^K q^{(t)}(\mathbf{z}^{(i)}) \log \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta}^{(t)})}{q^{(t)}(\mathbf{z}^{(i)})} \newline
& \stackrel{(4)}{\leq} \sum_{i = 1}^M \sum_{\mathbf{z}^{(i)} = 1}^K q^{(t)}(\mathbf{z}^{(i)}) \log \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta}^{(t + 1)})}{q^{(t)}(\mathbf{z}^{(i)})} \newline
& \stackrel{(5)}{\leq} \sum_{i = 1}^M \log \sum_{\mathbf{z}^{(i)} = 1}^K q^{(t)}(\mathbf{z}^{(i)}) \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta}^{(t + 1)})}{q^{(t)}(\mathbf{z}^{(i)})} \stackrel{(6)}{=} L(\boldsymbol{\theta}^{(t + 1)}).
\end{aligned}
$$

1. By definition, this is the (log) likelihood of the data.
2. By marginalization over $z^{(i)}$ and multiplication of an arbitrary distribution $q(z^{(i)})$ to both numerator and denominator inside log.
3. By Jensen's inequality where equality condition is satisfied by setting $q^{(t)}(z^{(i)}) = p(z^{(i)} | x^{(i)}; \boldsymbol{\theta}^{(t)})$.
4. By M-step of EM, where we maximize (3), holding $q^{(t)}(z^{(i)})$ fixed.
5. By Jensen's inequality (in reverse order). Note that we have already updated $\boldsymbol{\theta}$ from $\boldsymbol{\theta}^{(t)}$ to $\boldsymbol{\theta}^{(t + 1)}$, $q^{(t)}(z^{(i)})$ may now not satisfy the equality condition.
...
$$
J(q, \boldsymbol{\theta}) = \sum_{i = 1}^M \sum_{\mathbf{z}^{(i)} = 1}^K q(\mathbf{z}^{(i)}) \log \frac{p(\mathbf{x}^{(i)}, \mathbf{z}^{(i)}; \boldsymbol{\theta})}{q(\mathbf{z}^{(i)})}.
$$

We can view this as coordinate ascent on $J$, in which the E-step maximizes $J$ with respect to $q$, and the M-step maximizes $J$ with respect to $\boldsymbol{\theta}$.
...
Dimensions in the low-dimensional data represent co-occurring features in the high-dimensional data.
...
If we assume that $\boldsymbol{\epsilon} \in \mathbb{R}^{M \times N}$ is a matrix of noise values from some probability distribution, we have,

$$
\mathbf{X} = \mathbf{Z}\mathbf{B} + \boldsymbol{\epsilon}.
$$
...
:::algorithm[Alternating least squares]{#alg:als}
...
Let $\mathbf{X}$ be an $M \times N$ matrix, with $M \geq N$. It can be factorized as,
...
$$
\mathbf{\Sigma} = \operatorname{diag}(\sigma_1, \sigma_2, \ldots, \sigma_N), \quad \sigma_1 \geq \sigma_2 \geq \ldots \geq \sigma_N \geq 0.
$$

$\sigma_i$'s are called **singular values** of $\mathbf{X}$.

![SVD visualization of the matrices.](./imgs/svd_matrices.svg "fig:svd")

![SVD visualization of the different operations.](./imgs/svd_operations.svg "fig:svd2")
...
Where
* $\mathbf{U} = [\mathbf{u}_1, \mathbf{u}_2, \ldots, \mathbf{u}_K] \in \mathbb{R}^{M \times K}$.
    - $\mathbf{U}^T \mathbf{U} = \mathbf{I}_K$.
* $\mathbf{V} = [\mathbf{v}_1, \mathbf{v}_2, \ldots, \mathbf{v}_K] \in \mathbb{R}^{N \times K}$.
    - $\mathbf{V}^T \mathbf{V} = \mathbf{I}_K$.
* $\mathbf{\Sigma}_K = \operatorname{diag}(\sigma_1, \sigma_2, \ldots, \sigma_K) \in \mathbb{R}^{K \times K}$.
...
Given an $M \times N$ matrix $\mathbf{X}$ of rank $R \leq \min \{M, N\}$ and its singular value decomposition $\mathbf{X} = \mathbf{U} \mathbf{\Sigma}_R \mathbf{V}^T$
...
$$
\Vert \mathbf{X} - \mathbf{Y}^{\star} \Vert_F^2 = \min \{\Vert \mathbf{X} - \mathbf{Y} \Vert_F^2 \mid \mathbf{Y} \in \mathbb{R}^{M \times N}, \operatorname{rank} \mathbf{Y} \leq K\}.
$$
:::
