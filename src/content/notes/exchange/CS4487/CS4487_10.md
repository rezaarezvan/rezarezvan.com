...
In the loss landscape, we will have lots of plateaus, saddle points and local optima that can hinder our training process.
...
:::algorithm[Gradient descent]{#alg:gd}
**Require**: Learning Rate $\alpha^{(t)}$

**Require**: Initial Parameter $\boldsymbol{\theta}^{(0)}$

1. **while** stopping criterion not met **do**
   1. Compute gradient estimate over $M$ examples:
   2. $\mathbf{g}^{(t)} = \nabla_{\boldsymbol{\theta}} \frac{1}{M} \sum_{i=1}^{M} \ell(f(\mathbf{x}^{(i)}; \boldsymbol{\theta}), y^{(i)})$
   3. Apply update: $\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} - \alpha^{(t)} \mathbf{g}^{(t)}$
   4. $t = t + 1$
2. **end while**
:::
...
:::algorithm[Stochastic gradient descent]{#alg:sgd}
**Require**: Learning Rate $\alpha^{(t)}$

**Require**: Initial Parameter $\boldsymbol{\theta}^{(0)}$

1. **while** stopping criterion not met **do**
   1. Sample example $(\mathbf{x}^{(i)}, y^{(i)})$ from the training set.
   2. Compute gradient estimate:
   3. $\mathbf{g}^{(t)} = \nabla_{\boldsymbol{\theta}} \ell(f(\mathbf{x}^{(i)}; \boldsymbol{\theta}), y^{(i)})$
   4. Apply update: $\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} - \alpha^{(t)} \mathbf{g}^{(t)}$
   5. $t = t + 1$
2. **end while**
:::
...
Update rule: $\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} + \mathbf{v}^{(t+1)}$
...
##### SGD with Momentum
:::algorithm[SGD with momentum]{#alg:sgd-momentum}
**Require**: Learning Rate $\alpha^{(t)}$
...
1. **while** stopping criterion not met **do**
   1. Sample example $(\mathbf{x}^{(i)}, y^{(i)})$ from the training set.
   2. Compute gradient estimate: $\mathbf{g}^{(t)} = \nabla_{\boldsymbol{\theta}} \ell(f(\mathbf{x}^{(i)}; \boldsymbol{\theta}), y^{(i)})$
   3. Update velocity: $\mathbf{v}^{(t+1)} = \rho \mathbf{v}^{(t)} - \alpha^{(t)} \mathbf{g}^{(t)}$
   4. Apply update: $\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} + \mathbf{v}^{(t+1)}$
   5. $t = t + 1$
2. **end while**
:::
...
$$
\begin{align*}
\tilde{\boldsymbol{\theta}} & = \boldsymbol{\theta}^{(t)} + \rho \mathbf{v}^{(t)}, \newline
\mathbf{v}^{(t + 1)} & = \rho \mathbf{v}^{(t)} - \alpha^{(t)} \nabla_{\boldsymbol{\theta}} \ell(f(\mathbf{x}^{(i)}; \tilde{\boldsymbol{\theta}}), y^{(i)}).
\end{align*}
$$

Update: $\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} + \mathbf{v}^{(t+1)}$

##### SGD with Nesterov Momentum
:::algorithm[SGD with Nesterov momentum]{#alg:sgd-nesterov}
**Require**: Learning Rate $\alpha^{(t)}$
...
1. **while** stopping criterion not met **do**
   1. Sample example $(\mathbf{x}^{(i)}, y^{(i)})$ from the training set.
   2. Update: $\tilde{\boldsymbol{\theta}} = \boldsymbol{\theta}^{(t)} + \rho \mathbf{v}^{(t)}$
   3. Compute gradient estimate: $\mathbf{g}^{(t)} = \nabla_{\boldsymbol{\theta}} \ell(f(\mathbf{x}^{(i)}; \tilde{\boldsymbol{\theta}}), y^{(i)})$
   4. Update velocity: $\mathbf{v}^{(t+1)} = \rho \mathbf{v}^{(t)} - \alpha^{(t)} \mathbf{g}^{(t)}$
   5. Apply update: $\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} + \mathbf{v}^{(t+1)}$
   6. $t = t + 1$
2. **end while**
:::
...
2. **while** stopping criterion not met **do**
   1. Sample example $(\mathbf{x}^{(i)}, y^{(i)})$ from the training set.
   2. Compute gradient estimate: $\mathbf{g}^{(t)} = \nabla_{\boldsymbol{\theta}} \ell(f(\mathbf{x}^{(i)}; \boldsymbol{\theta}), y^{(i)})$
   3. Accumulate: $\mathbf{r}^{(t+1)} = \mathbf{r}^{(t)} + \mathbf{g}^{(t)} \odot \mathbf{g}^{(t)}$
   4. Compute update: $\Delta \boldsymbol{\theta} = -\frac{\alpha}{\epsilon + \sqrt{\mathbf{r}^{(t+1)}}} \odot \mathbf{g}^{(t)}$
   5. Apply update: $\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} + \Delta \boldsymbol{\theta}$
   6. $t = t + 1$
3. **end while**
:::
...
AdaGrad might shrink the learning rate too aggressively sometimes as well, but we can overcome these problems.
...
2. **while** stopping criterion not met **do**
   1. Sample example $(\mathbf{x}^{(i)}, y^{(i)})$ from the training set.
   2. Compute gradient estimate: $\mathbf{g}^{(t)} = \nabla_{\boldsymbol{\theta}} \ell(f(\mathbf{x}^{(i)}; \boldsymbol{\theta}), y^{(i)})$
   3. Accumulate: $\mathbf{r}^{(t+1)} = \rho \mathbf{r}^{(t)} + (1 - \rho) \mathbf{g}^{(t)} \odot \mathbf{g}^{(t)}$
   4. Compute update: $\Delta \boldsymbol{\theta} = -\frac{\alpha}{\epsilon + \sqrt{\mathbf{r}^{(t+1)}}} \odot \mathbf{g}^{(t)}$
   5. Apply update: $\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} + \Delta \boldsymbol{\theta}$
   6. $t = t + 1$
3. **end while**
...
1. Initialize moment variables $\mathbf{s}^{(0)} = 0$, $\mathbf{r}^{(0)} = 0$
2. **while** stopping criterion not met **do**
   1. Sample example $(\mathbf{x}^{(i)}, y^{(i)})$ from the training set.
   2. Compute gradient estimate: $\mathbf{g}^{(t)} = \nabla_{\boldsymbol{\theta}} \ell(f(\mathbf{x}^{(i)}; \boldsymbol{\theta}), y^{(i)})$
   3. Update: $\mathbf{s}^{(t+1)} = \rho_1 \mathbf{s}^{(t)} + (1 - \rho_1) \mathbf{g}^{(t)}$
   4. Update: $\mathbf{r}^{(t+1)} = \rho_2 \mathbf{r}^{(t)} + (1 - \rho_2) \mathbf{g}^{(t)} \odot \mathbf{g}^{(t)}$
   5. Correct biases: $\hat{\mathbf{s}} = \frac{\mathbf{s}^{(t+1)}}{1 - \rho_1^{t+1}}$, $\hat{\mathbf{r}} = \frac{\mathbf{r}^{(t+1)}}{1 - \rho_2^{t+1}}$
   6. Compute and apply update: $\Delta \boldsymbol{\theta} = -\alpha \frac{\hat{\mathbf{s}}}{\delta + \sqrt{\hat{\mathbf{r}}}}, \boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} + \Delta \boldsymbol{\theta}$
   7. $t = t + 1$
3. **end while**
:::
...
Weight decay (which is just a fancy word for $\ell_2$-regularization and ridge regularization) is a regularization technique to prevent overfitting.
It is just adding a $\Vert \boldsymbol{\theta} \Vert_2^2$ term to the loss function.
...
2. **while** stopping criterion not met **do**
   1. Sample example $(\mathbf{x}^{(i)}, y^{(i)})$ from the training set.
   2. Compute gradient estimate: $\mathbf{g}^{(t)} = \nabla_{\boldsymbol{\theta}} \ell(f(\mathbf{x}^{(i)}; \boldsymbol{\theta}), y^{(i)}) \htmlClass{eq-hl-pink}{+ \lambda \boldsymbol{\theta}^{(t)}}$
   3. Update: $\mathbf{s}^{(t+1)} = \rho_1 \mathbf{s}^{(t)} + (1 - \rho_1) \mathbf{g}^{(t)}$
   4. Update: $\mathbf{r}^{(t+1)} = \rho_2 \mathbf{r}^{(t)} + (1 - \rho_2) \mathbf{g}^{(t)} \odot \mathbf{g}^{(t)}$
   5. Correct biases: $\hat{\mathbf{s}} = \frac{\mathbf{s}^{(t+1)}}{1 - \rho_1^{t+1}}$, $\hat{\mathbf{r}} = \frac{\mathbf{r}^{(t+1)}}{1 - \rho_2^{t+1}}$
   6. Compute and apply update: $\Delta \boldsymbol{\theta} = -\alpha \frac{\hat{\mathbf{s}}}{\delta + \sqrt{\hat{\mathbf{r}}}} \htmlClass{eq-hl-lime}{- \lambda \boldsymbol{\theta}^{(t)}}$, $\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} + \Delta \boldsymbol{\theta}$
   7. $t = t + 1$
3. **end while**
:::
...
* AdaBelief, unlike Adam, AdaBelief divides by the exponentially decaying moving average of variance of the gradients.
    ...
    - In AdaBelief: $\mathbf{r}^{(t+1)} = \rho_2 \mathbf{r}^{(t)} + (1 - \rho_2) (\mathbf{g}^{(t)} - \mathbf{s}^{(t + 1)}) \odot (\mathbf{g}^{(t)} - \mathbf{s}^{(t + 1)})$
...
We randomly sample some (typically, 10-20) masks, for each mask, we apply it to the trained model to get a prediction, then we take the majority vote (i.e., average) over the predictions.
...
$$
\mathbf{y} = f_{\mathbf{W}} (\mathbf{x}) = \mathbb{E}_{\mathbf{z}} [f_{\mathbf{W}} (\mathbf{x}, \mathbf{z})] = \int f_{\mathbf{W}} (\mathbf{x}, \mathbf{z}) p(\mathbf{z}) \, d\mathbf{z}.
$$
...
$$
\begin{align*}
\mathbb{E}[a] &= \frac{1}{4}(w_1 x_1 + w_2 x_2) + \frac{1}{4}(w_1 x_1 + w_2 \cdot 0) \newline
& \quad + \frac{1}{4}(w_1 \cdot 0 + w_2 x_2) + \frac{1}{4}(w_1 \cdot 0 + w_2 \cdot 0) \newline
...
But we have an issue, what if a zero mean and unit variance are too hard of a constraint?
...
where $\boldsymbol{\mu} = \frac{1}{M} \mathbf{X}^T \mathbf{1}$.

We can also normalize the data by dividing by the standard deviation of each feature,

$$
\mathbf{X} = \mathbf{X} \oslash \mathbf{1} \boldsymbol{\sigma}^T,
$$

where $\boldsymbol{\sigma} = \sqrt{\operatorname{diag}\left(\frac{1}{M} \mathbf{X}^T \mathbf{X}\right)}$.
...
As we lightly discussed earlier, we typically train our network several times on the entire dataset, one complete pass through the data is called an *epoch*.
...
Why? If every neuron in the network computes the same output, then all of them will also compute the same gradients during backpropagation and undergo the exact same parameter updates.
...
$$
w = \texttt{np.random.randn(n)} \times \sqrt{\frac{2}{n}},
$$
...
* If we have a low learning rate, our loss will slowly drop, but we may get stuck in a local minimum.
...
2. Overfit a small sample.
    - Try to train to 100% training accuracy on a small sample of training data (5-10 mini-batches).
        - Fiddle with the architecture, learning rate, weight initialization, etc.
    - Loss not going down?
...
3. Find learning rate that makes loss go down.
    ...
    - Good learning rates to usually start off with are, $10^{-1}, 10^{-2}, 10^{-3}, 10^{-4}$.
4. Coarse grid, train for 1-5 epochs.
    ...
    - Good weight decay to usually start off with are, $10^{-4}, 10^{-5}, 0$.
