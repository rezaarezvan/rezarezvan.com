...
#### Convex Function
$f : \mathbb{R}^N \to \mathbb{R}$ is convex if $\operatorname{dom}(f)$ is a convex set and,

$$
f(\alpha \mathbf{x}^{(1)} + (1 - \alpha) \mathbf{x}^{(2)}) \leq \alpha f(\mathbf{x}^{(1)}) + (1 - \alpha) f(\mathbf{x}^{(2)})
$$

for all $\mathbf{x}^{(1)}, \mathbf{x}^{(2)} \in \operatorname{dom}(f)$ and $0 \leq \alpha \leq 1$.
...
exists at each $\mathbf{x} \in \operatorname{dom}(f)$.
...
A point $\mathbf{x}^{\star}$ is **feasible** if $\mathbf{x} \in \operatorname{dom}(f_0)$, and it satisfies the constraints.
...
$$
\begin{aligned}
\underset{\mathbf{z}}{\text{minimize}} & \quad f_0(\mathbf{z}) \newline
...
##### Examples
* $f_0(x) = -\log x, \operatorname{dom}(f_0) = \mathbb{R}_{++}$
    - $p^{\star} = -\infty$
* $f_0(x) = x \log x, \operatorname{dom}(f_0) = \mathbb{R}_{++}$
...
\begin{aligned}
\text{minimize} & \quad f_0(\mathbf{x}) = x_1^2 + x_2^2 \newline
\text{subject to} & \quad f_1(\mathbf{x}) = x_1 \leq 0 \quad \| \cdot (1 + x_2^2) \newline
& \quad h_1(\mathbf{x}) = x_1 + x_2 = 0 \quad \| \sqrt{\cdot}
\end{aligned}
$$
...
**Lagrangian** $\mathbf{L} : \mathbb{R}^N \times \mathbb{R}^r \times \mathbb{R}^s \rightarrow \mathbb{R}$, with $\operatorname{dom}(\mathbf{L}) = \Chi \times \mathbb{R}^r \times \mathbb{R}^s$,
$$
\mathbf{L}(\mathbf{x}, \boldsymbol{\lambda}, \boldsymbol{\nu}) = f_0(\mathbf{x}) + \sum_{i=1}^r \lambda_i f_i(\mathbf{x}) + \sum_{i=1}^s \nu_i h_i(\mathbf{x}).
$$
...
$$
g(\boldsymbol{\lambda}, \boldsymbol{\nu}) = \underset{\mathbf{x} \in \Chi}{\inf} \mathbf{L}(\mathbf{x}, \boldsymbol{\lambda}, \boldsymbol{\nu}) = \underset{\mathbf{x} \in \Chi}{\inf} \left( f_0(\mathbf{x}) + \sum_{i=1}^r \lambda_i f_i(\mathbf{x}) + \sum_{i=1}^s \nu_i h_i(\mathbf{x}) \right).
$$
...
**Proof**. If $\mathbf{\tilde{x}}$ is feasible and $\boldsymbol{\lambda} \geq 0$, then,
$$
f_0(\mathbf{\tilde{x}}) \geq \mathbf{L}(\mathbf{\tilde{x}}, \boldsymbol{\lambda}, \boldsymbol{\nu}) \geq \underset{\mathbf{x} \in \Chi}{\inf} \mathbf{L}(\mathbf{x}, \boldsymbol{\lambda}, \boldsymbol{\nu}) = g(\boldsymbol{\lambda}, \boldsymbol{\nu}).
$$

Minimizing over all feasible $\mathbf{\tilde{x}}$ gives $p^{\star} \geq g(\boldsymbol{\lambda}, \boldsymbol{\nu})$.
...
Therefore, the Lagrangian is $\mathbf{L}(\mathbf{x}, \boldsymbol{\nu}) = \mathbf{x}^T \mathbf{x} + \boldsymbol{\nu}^T (\mathbf{A}\mathbf{x} - \mathbf{b})$.

To minimize $\mathbf{L}$ over $\mathbf{x}$, set gradient equal to zero,
$$
\nabla_{\mathbf{x}} \mathbf{L}(\mathbf{x}, \boldsymbol{\nu}) = 2\mathbf{x} + \mathbf{A}^T \boldsymbol{\nu} = 0 \Rightarrow \mathbf{x} = -\frac{1}{2} \mathbf{A}^T \boldsymbol{\nu}.
$$

Plug in $\mathbf{L}$ to obtain $g$,
$$
g(\boldsymbol{\nu}) = \mathbf{L}\left(-\frac{1}{2} \mathbf{A}^T \boldsymbol{\nu}, \boldsymbol{\nu}\right) = -\frac{1}{4} \boldsymbol{\nu}^T \mathbf{A} \mathbf{A}^T \boldsymbol{\nu} - \mathbf{b}^T \boldsymbol{\nu}.
$$

which is a concave function of $\boldsymbol{\nu}$.

Using the lower bound property, $p^{\star} \geq -\frac{1}{4} \boldsymbol{\nu}^T \mathbf{A} \mathbf{A}^T \boldsymbol{\nu} - \mathbf{b}^T \boldsymbol{\nu}$ for all $\boldsymbol{\nu}$.
...
$$
g(\boldsymbol{\lambda}, \boldsymbol{\nu}) = \underset{\mathbf{x}}{\inf} \mathbf{L}(\mathbf{x}, \boldsymbol{\lambda}, \boldsymbol{\nu}) =
\begin{cases}
...
$g$ is linear on $\{(\boldsymbol{\lambda}, \boldsymbol{\nu}) \mid \mathbf{A}^T \boldsymbol{\nu} - \boldsymbol{\lambda} + \mathbf{c} = 0\}$, hence concave.

Using the lower bound property, $p^{\star} \geq -\mathbf{b}^T \boldsymbol{\nu}$ if $\mathbf{A}^T \boldsymbol{\nu} + \mathbf{c} \succeq 0$.
...
$\boldsymbol{\lambda}, \boldsymbol{\nu}$ are dual feasible if $\boldsymbol{\lambda} \succeq 0$, $(\boldsymbol{\lambda}, \boldsymbol{\nu}) \in \operatorname{dom}(g)$.
...
Assuming strong duality holds, $\mathbf{x}^{\star}$ is **primal optimal**, $(\boldsymbol{\lambda}^{\star}, \boldsymbol{\nu}^{\star})$ is **dual optimal**.
$$
\begin{aligned}
f_0(\mathbf{x}^{\star}) & = g(\boldsymbol{\lambda}^{\star}, \boldsymbol{\nu}^{\star}) \newline
& = \underset{\mathbf{x}}{\inf} \left(f_0(\mathbf{x}) + \sum_{i=1}^r \lambda_i^{\star} f_i(\mathbf{x}) + \sum_{i=1}^s \nu_i^{\star} h_i(\mathbf{x})\right) \newline
...
Hence, the two inequalities hold with equality, $\mathbf{x}^{\star}$ not only minimizes $f_0(\mathbf{x})$ but also minimizes $\mathbf{L}(\mathbf{x}, \boldsymbol{\lambda}^{\star}, \boldsymbol{\nu}^{\star})$.
...
If strong duality holds and $\mathbf{x}, \boldsymbol{\lambda}, \boldsymbol{\nu}$ are optimal, then they must satisfy the KKT conditions.
...
$$
\mathbf{L}(\mathbf{w}, b, \boldsymbol{\xi}, \boldsymbol{\alpha}, \boldsymbol{\beta}) = \frac{1}{2} \Vert \mathbf{w} \Vert_2^2 + C \sum_{i=1}^M \xi_i - \sum_{i=1}^M \alpha_i [y^{(i)} (\mathbf{w}^T \mathbf{x}^{(i)} + b) - 1 + \xi_i] - \sum_{i=1}^M \beta_i \xi_i.
$$
...
$$
\begin{aligned}
\underset{\boldsymbol{\alpha}}{\max} & \quad \sum_{i=1}^M \alpha_i - \frac{1}{2} \sum_{i,j=1}^M y^{(i)} y^{(j)} \alpha_i \alpha_j \mathbf{x}^{(i)T} \mathbf{x}^{(j)} \newline
...
Thus, classifying a new point $\mathbf{x}^{\star}$ is,
$$
y^{\star} = \operatorname{sign}(\mathbf{w}^T \mathbf{x}^{\star} + b) = \operatorname{sign} \left( \sum_{i=1}^M \alpha_i y^{(i)} \mathbf{x}^{(i)T} \mathbf{x}^{\star} + b \right).
$$
...
$$
\begin{aligned}
\underset{\boldsymbol{\alpha}}{\max} & \quad \sum_{i=1}^M \alpha_i - \frac{1}{2} \sum_{i,j=1}^M y^{(i)} y^{(j)} \alpha_i \alpha_j \phi(\mathbf{x}^{(i)})^T \phi(\mathbf{x}^{(j)}) \newline
\text{subject to} & \quad \sum_{i=1}^M \alpha_i y^{(i)} = 0 \newline
& \quad 0 \leq \alpha_i \leq C, \quad i = 1, \ldots, M
\end{aligned}
$$
...
$$
\mathcal{K}(\mathbf{x}, \mathbf{x}^{\prime}) = (\mathbf{x}^T \mathbf{x}^{\prime})^p = \left(\sum_{i=1}^N x_i x_i^{\prime}\right)^p.
$$
...
$$
\begin{aligned}
\underset{\boldsymbol{\alpha}}{\max} & \quad \sum_{i=1}^M \alpha_i - \frac{1}{2} \sum_{i,j=1}^M y^{(i)} y^{(j)} \alpha_i \alpha_j \mathcal{K}(\mathbf{x}^{(i)}, \mathbf{x}^{(j)}) \newline
\text{subject to} & \quad \sum_{i=1}^M \alpha_i y^{(i)} = 0 \newline
& \quad 0 \leq \alpha_i \leq C, \quad i = 1, \ldots, M
\end{aligned}
$$
...
Our prediction is,
$$
y^{\star} = \operatorname{sign} \left( \sum_{i=1}^M \alpha_i y^{(i)} \mathcal{K}(\mathbf{x}^{(i)}, \mathbf{x}^{\star}) + b \right).
$$
...
$$
\underset{\boldsymbol{\theta}}{\min} \ell(\theta_1, \theta_2, \ldots, \theta_N)
$$
