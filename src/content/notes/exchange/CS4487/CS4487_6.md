...
### RANSAC
**RAN**dom **SA**mple **C**onsensus attempts to robustly fit a regression model in the presence of corrupted data (outliers).
...
3. Determine all data as inlier or outlier by calculating the residuals (prediction errors) and comparing to a threshold. The set of inliers is called the consensus set.
...
$$
\mathcal{K}(\mathbf{x}, \mathbf{z}) = \phi(\mathbf{x})^T \phi(\mathbf{z}).
$$
...
with $\boldsymbol{\lambda} = (\mathbf{X} \mathbf{X}^T + \alpha \mathbf{I}_M)^{-1} \mathbf{y}$.
...
$$
\begin{aligned}
\underset{\mathbf{w}, b, \boldsymbol{\xi}, \boldsymbol{\xi}^{\star}}{\min} & \quad \frac{1}{2} \Vert \mathbf{w} \Vert_2^2 + C \sum_{i=1}^M (\xi_i + \xi_i^{\star}) \newline
...
$$
\underset{\mathbf{c}_1, \ldots, \mathbf{c}_K}{\min} \sum_{i=1}^M \Vert \mathbf{x}^{(i)} - \mathbf{c}_{z^{(i)}} \Vert_2^2,
$$

where $z^{(i)}$ is the index of the closest cluster center to $\mathbf{x}^{(i)}$,

$$
z^{(i)} = \underset{j \in \{1, \ldots, K\}}{\arg \min} \Vert \mathbf{x}^{(i)} - \mathbf{c}_j \Vert_2^2.
$$
...
$$
C_j = \{\mathbf{x}^{(i)} \mid z^{(i)} = j\}.
$$

Cluster center is the mean of the points in that cluster,

$$
\mathbf{c}_j = \frac{1}{\vert C_j \vert} \sum_{\mathbf{x}^{(i)} \in C_j} \mathbf{x}^{(i)}.
$$
...
1. **Assignment step**: calculate assignment $z^{(i)}$ for each point $\mathbf{x}^{(i)}$, closest cluster center using Euclidean distance,
   $$
   z^{(i)} = \underset{j \in \{1, \ldots, K\}}{\arg \min} \Vert \mathbf{x}^{(i)} - \mathbf{c}_j \Vert_2.
   $$
...
$$
\min \ell(\mathbf{z}, \{\mathbf{c}_j \}_{j=1}^K),
$$

$$
\underset{\mathbf{z}, \{\mathbf{c}_1, \ldots, \mathbf{c}_K\}}{\min} \sum_{i=1}^M \Vert \mathbf{x}^{(i)} - \mathbf{c}_{z^{(i)}} \Vert_2^2,
$$

where $\mathbf{z} = [z^{(1)}, z^{(2)}, \ldots, z^{(M)}]^T$.

$K$-means is exactly **coordinate descent** on $\ell$,
where assignment step minimizes $\ell$ with respect to $\mathbf{z}$ while holding $\{\mathbf{c}_j\}$ fixed,
and the update step minimized $\ell$ with respect to $\{\mathbf{c}_j\}$ while holding $\mathbf{z}$ fixed.
...
$$
p(\mathbf{x}) = \sum_{j=1}^K \pi_j \mathcal{N}(\mathbf{x}; \boldsymbol{\mu}_j, \mathbf{\Sigma}_j).
$$

Each Gaussian represents one elliptical cluster:
* $\boldsymbol{\mu}_j$ is mean of $j$-th cluster (the location).
...
$$
\underset{\pi, \boldsymbol{\mu}, \mathbf{\Sigma}}{\max} \sum_{i=1}^M \log \sum_{j=1}^K \pi_j \mathcal{N}(\mathbf{x}^{(i)}; \boldsymbol{\mu}_j, \mathbf{\Sigma}_j).
$$
...
:::algorithm[EM for GMM]{#alg:em-gmm}
1. **E-step**: Calculate cluster membership with "soft" assignment, a data point can have a fractional contribution to different clusters.
   ...
   & = \frac{\pi_j \mathcal{N}(\mathbf{x}^{(i)}; \boldsymbol{\mu}_j, \mathbf{\Sigma}_j)}{\sum_{k=1}^K \pi_k \mathcal{N}(\mathbf{x}^{(i)}; \boldsymbol{\mu}_k, \mathbf{\Sigma}_k)}.
   \end{aligned}
   $$
2. **M-step**: Update each Gaussian cluster (mean, covariance, and weight) using "soft" weighting.
   ...
   Covariance,
   $$
   \mathbf{\Sigma}_j = \frac{1}{M_j} \sum_{i=1}^M z_j^{(i)} (\mathbf{x}^{(i)} - \boldsymbol{\mu}_j)(\mathbf{x}^{(i)} - \boldsymbol{\mu}_j)^T.
   $$
:::
...
$$
p(\mathbf{x}^{(i)} | z^{(i)} = j) = \mathcal{N}(\mathbf{x}^{(i)}; \boldsymbol{\mu}_j, \mathbf{I}) = \frac{1}{(2\pi)^{N/2}} \exp \left( -\frac{1}{2} \Vert \mathbf{x}^{(i)} - \boldsymbol{\mu}_j \Vert_2^2 \right).
$$

We obtain a special case of the EM algorithm for GMM,

$$
z_j^{(i)} = \frac{\exp \left( -\frac{1}{2} \Vert \mathbf{x}^{(i)} - \boldsymbol{\mu}_j \Vert_2^2 \right)}{\sum_{k=1}^K \exp \left( -\frac{1}{2} \Vert \mathbf{x}^{(i)} - \boldsymbol{\mu}_k \Vert_2^2 \right)},
$$
