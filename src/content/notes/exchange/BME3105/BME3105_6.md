...
$x = [x_1, x_2, \ldots, x_n]^T \in \mathbb{R}^n$ is called the state of the system (the components of $x$ can stand for position, velocity, voltage and current).
...
Moreover, $F \in \mathbb{R}^{n \times n}$ is called the system matrix, $G \in \mathbb{R}^{n \times m}$ is called the input matrix, $H \in \mathbb{R}^{p \times n}$ is called the output matrix, and $J \in \mathbb{R}^{p \times m}$ is called the direct transmission matrix.
...
![Circuit with input $u(t)$ and output voltage $V_o$.](./imgs/rlc_circuit.svg "fig:rlc-circuit")
...
\dot{V}_c & = L \frac{d^2 i_L}{dt^2} + R \frac{di_L}{dt}.
\end{align*}
$$

Substituting this into @eq:capacitor-current-balance, we get,
...
$$
\begin{align*}
F & = \frac{\partial f}{\partial x} \bigg|_{x = 0, u = 0} \in \mathbb{R}^{n \times n}, & G & = \frac{\partial f}{\partial u} \bigg|_{x = 0, u = 0} \in \mathbb{R}^{n \times m}, \newline
H & = \frac{\partial h}{\partial x} \bigg|_{x = 0, u = 0} \in \mathbb{R}^{p \times n}, & J & = \frac{\partial h}{\partial u} \bigg|_{x = 0, u = 0} \in \mathbb{R}^{p \times m}.
\end{align*}
$$
...
as the (Jacobian) linearization of the above or small signal model.

Remark, for example, if the nonlinear function is,
...
$$
\begin{align*}
\frac{\partial f_1}{\partial x_1} & = 0, & \frac{\partial f_1}{\partial x_2} & = 1, & \frac{\partial f_1}{\partial u} & = 0, \newline
\frac{\partial f_2}{\partial x_1} & = -\omega^2, & \frac{\partial f_2}{\partial x_2} & = 0, & \frac{\partial f_2}{\partial u} & = 1.
\end{align*}
$$
...
$$
F = \begin{bmatrix} 0 & 1 \newline -\omega^2 & 0 \end{bmatrix}, \quad G = \begin{bmatrix} 0 \newline 1 \end{bmatrix}, \quad H = \begin{bmatrix} 0 & 0 \end{bmatrix}, \quad J = \begin{bmatrix} 0 \end{bmatrix}.
$$
...
The solution of the state space equation that satisfies an initial condition can be a bit tricky.

Given $\dot{x}(t) = Fx(t) + Gu(t), \; t \geq t_0, x \in \mathbb{R}^n, u \in \mathbb{R}^m$ and $x_0, u(t), t \geq t_0$ are given.

Find $x(t), t \geq t_0$ subject to $x(t_0) = x_0$ and $x(t)$ satisfies the above.
...
where $\alpha(s)$ is the characteristic polynomial of $F$ with degree $n$, $\beta(s)$ is a polynomial of degree $n$ when $J \neq 0$ or degree $< n$ when $J = 0$.
