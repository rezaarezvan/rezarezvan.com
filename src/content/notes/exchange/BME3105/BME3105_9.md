...
The relation between input and output is described by a transfer function in the $s$-domain or a differential equation in the time-domain.

Analog computers can be made by RLC (Resistor, Inductor, Capacitor) circuits plus OP-AMPs (Operational Amplifiers).

However, the cost of these devices is high, they're heavy and bulky, and sensitive to noise. Too many drawbacks.

### From Continuous to Discrete
Previously, all signals have been continuous variables (analogue) and defined for all instants in time.

Because controllers are now usually implemented on computers, we must deal with signals which are *discrete* in nature.
...
A digital processor is a discrete-time device that takes a discrete-time signal as input and produces a discrete-time signal as output according to a certain algorithm (*difference equation*, more on this later).
...
With discrete-time systems, $z$ is also an *operator*. Multiplying a signal by $z$ *advances* that signal one "tick" in time.
...
$$
where $z$ is a complex variable, is called the $\mathcal{Z}$-transform of $f(k)$.
:::
...
since the power series $1 + x + x^2 + \ldots$ sums to $\frac{1}{1 - x}$.
:::
...
:::definition[BIBO stability of DT systems]
...
:::example
Let $G(s) = \frac{(1 + 3.2s)}{(1 + 0.2s)}$ with an update-time of $T = 0.2\,\mathrm{s}$.
...
$$
G(z) = \frac{(1 + 3.2(10(z - 1)/(z + 1)))}{(1 + 0.2(10(z - 1)/(z + 1)))} = \frac{(1 + 32(z - 1)/(z + 1))}{(1 + 2(z - 1)/(z + 1))} = \frac{33z - 31}{3z - 1}.
$$
...
$$
\frac{C(z)}{E(z)} = \frac{33z - 31}{3z - 1}.
$$

We can rewrite this as,

$$
C(z)(3z - 1) = E(z)(33z - 31) \implies 3zC(z) - C(z) = 33zE(z) - 31E(z).
$$

This is the difference equation of the system. Let's take the inverse $z$-transform of this equation.

$$
3c(k + 1) - c(k) = 33e(k + 1) - 31e(k).
$$
:::
...
One can do this in the two following ways.
...
:::remark
For Tustin's method, the task at each step is to use a trapezoidal integration, that is, to approximate $e(t)$ by a straight line between two samples.
:::
...
$$
D(z) = D(s) \Big|_{s = 30 \frac{z - 1}{z + 1}} = 70 \frac{30 \frac{z - 1}{z + 1} + 2}{30 \frac{z - 1}{z + 1} + 10} = 70 \frac{32z - 28}{40z - 20}.
$$
:::
...
which gives $z_a = e^{-aT}$ and $z_b = e^{-bT}$.
...
Given $G(s) = \frac{1}{s^2}$, design a digital controller so that the closed-loop system has a natural frequency $\omega_n = 0.3\,\mathrm{rad/s}$ and a damping ratio $\zeta = 0.7$.
...
We first need to find $T$. A rule of thumb of $T$ is that the sample rate satisfies,

$$
\omega_s \triangleq \frac{2 \pi}{T} > 20 \omega_n.
$$

Letting $\frac{2 \pi}{T} = 20 \omega_n = 6$, thus $T \approx 1$.
...
$$
k_d = \frac{0.81 \cdot 0.2 \cdot (1 - 0.135)}{2 \cdot (1 - 0.82)} \approx 0.39.
$$

Thus,

$$
D(z) = 0.39 \frac{z - 0.82}{z - 0.135}.
$$
