...
$$
V = L \frac{dI}{dt} \quad \text{(Inductance)}.
$$
:::
...
* $\mathbf{F}$: the vector sum of all forces applied to each body in a system [$\mathrm{N}$]
* $m$: mass of the body [$\mathrm{kg}$]
* $\mathbf{a}$: the vector acceleration of each body with respect to an inertial frame [$\mathrm{m/s^2}$]
...
* $\mathbf{M}$: the sum of all external moments about the center of mass of a body [$\mathrm{N\,m}$]
* $I$: the body's mass moment of inertia about its center of mass [$\mathrm{kg\,m^2}$]
* $\boldsymbol{\alpha}$: the angular acceleration of the body [$\mathrm{rad/s^2}$]
...
![The basic circuit elements and their voltage–current relations.](./imgs/circuit_elements.svg "fig:circuit-elements")

### Linear Systems
Most of the models that we will be considering are linear systems.
In reality, this will often not be the case.
An electrical resistor will heat up as the current that passes through it increases, thereby raising its resistance.
...
$$
x_1(t) \rightarrow y_1(t), \quad x_2(t) \rightarrow y_2(t) \implies a x_1(t) + b x_2(t) \rightarrow a y_1(t) + b y_2(t).
$$
...
For clarification, the superposition principle is just homogeneity and additivity.
