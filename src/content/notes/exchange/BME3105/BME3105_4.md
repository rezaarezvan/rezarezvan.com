...
The purpose of a closed loop control is to make a system's output follow the input as closely as possible.

Ideally the output would at all times correspond exactly to the input, but this is not possible in real systems which (due to the effects of inertia, inductance, heat transfer, etc.) do not respond instantaneously to changes in the input.
...
3. **How Quickly Does the System Reach Steady State?**
    - Steady-state should be reached quickly without excessive overshoot or oscillation.
...
Let's see how we analyze a system's poles and zeroes to determine a system's response.
...
* **Natural** or **homogeneous response**, this is caused by the poles of the transfer function, $G(s)$.
...
The time constant is the time required for the step response to rise to 63% of its final value,
...
May see responses similar to first-order system, damped oscillations, or undamped oscillations.
...
& = \frac{K_1}{s} + \frac{\alpha + j\beta}{s + 1 + j\sqrt{8}} + \frac{\alpha - j\beta}{s + 1 - j\sqrt{8}}.
\end{align*}
$$
...
$$
\begin{align*}
c(t) & = K_1 + e^{-(0)t}[2\alpha \cos(\omega_d t) + 2\beta \sin(\omega_d t)] \newline
& = K_1 + 2\alpha \cos(\omega_d t) + 2\beta \sin(\omega_d t) \newline
& = K_1 + K_4 \cos(\omega_d t - \phi),
\end{align*}
$$
...
3. **Percent overshoot, %OS** is the percentage that the output overshoots the final value at $t = T_p$.
   $$
   \%\text{OS} = \frac{c_{\text{max}} - c_{\text{final}}}{c_{\text{final}}} \cdot 100\%.
   $$
4. **Settling time** $T_s$ is the time required for the output to reach and stay within $\pm 2\%$ of $c_{\text{final}}$.
