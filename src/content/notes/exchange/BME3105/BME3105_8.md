...
$$
\begin{align*}
\dot{x} &= Fx + Gu, \quad x(0) = x_0, \quad t \geq 0, \newline
y &= Hx + Ju,
\end{align*}
$$
...
![Simple circuit with one inductor.](./imgs/inductor_circuit.svg "fig:inductor-circuit")
...
Since the pole of the system is at $s = 0$, the system is not internally stable.
...
![Circuit with resistor and inductor.](./imgs/rl_circuit.svg "fig:rl-circuit")
...
The pole of the system is at $s = -\frac{R}{L}$.
Since both $R$ and $L$ are positive, the pole is negative, and the system is internally stable.
:::

:::example
Now, let's combine these two circuits and also find the state space equations for it.

![Combined circuit.](./imgs/parallel_inductors.svg "fig:parallel-inductors")
...
\lambda\left(\lambda + \frac{R}{L_2}\right) &= 0 \newline
\lambda_0 = 0, \quad \lambda_1 &= -\frac{R}{L_2}.
\end{align*}
$$
...
$$
u = -Kx + Mr, \quad K \in \mathbb{R}^{m \times n}, \quad M \in \mathbb{R}^{m \times m},
$$
...
which is called the **closed-loop system** under the state feedback control.

**Remark**

1. $K$ and $M$ can modify $F$ and $G$, thus the behavior of the entire system.
2. $K$ is called the **feedback** gain. It can be used to *stabilize* the system and modify the *transient* response.
...
We call the problem of finding $K \in \mathbb{R}^{m \times n}$ subject to the above as the **state feedback stabilization problem**.
...
2. A set of $n$ complex numbers $\{\lambda_{d_1}, \lambda_{d_2}, \ldots, \lambda_{d_n}\}$ is said to be *self-conjugate* if and only if,
   $$
...
&= F(x - N_x r) + F N_x r + G N_u r - GK(x - N_x r) \newline
...
where $L$ is such that $(F - LH)$ is stable.
...
When $r = 0$, $u_{ss} = 0$ and $x_{ss} = 0$, and the control law becomes,
...
Then, we can find $N_x$ and $N_u$ which satisfy,

$$
F N_x + G N_u = \mathbf{0}_{2 \times 1}, \quad H N_x + J N_u = 1.
$$
