...
\vdots \newline
0
\end{bmatrix} \triangleq B_c, \quad
H =
\begin{bmatrix}
b_1 & b_2 & \ldots & b_{n}
\end{bmatrix} \triangleq C_c, \quad J = 0 \triangleq D_c,
$$
...
$$
\begin{align*}
\det(sI - A_c) & = a_3 \begin{vmatrix} -1 & s \newline 0 & -1 \end{vmatrix} + s \begin{vmatrix} s + a_1 & a_2 \newline -1 & s \end{vmatrix} \newline
& = a_3 + s((s + a_1)s + a_2) \newline
& = s^3 + a_1s^2 + a_2s + a_3.
\end{align*}
$$
...
$$
B_o = C_c^T = \begin{bmatrix} b_1 \newline \vdots \newline b_n \end{bmatrix}, \quad C_o = B_c^T = \begin{bmatrix} 1 & 0 & \ldots & 0 \end{bmatrix}, \quad D_o = 0.
$$
...
:::remark
1. Nonuniqueness of realization, as we have seen previously, the realizations are not unique (we will discuss this shortly).
...
Since realizations are not unique, there can be good and bad realizations.

From the control point of view, we need to determine what makes a good realization.

**Controllability** and **observability** are two criteria to determine a good realization.
...
![Controllability: steering the state from $x_0$ to $x_f$.](./imgs/controllability.svg "fig:controllability")
...
$$
\begin{align*}
\dot{x}_1 & = x_1, \quad x_1(0) = x_{10}, \newline
\dot{x}_2 & = x_2 + u, \quad x_2(0) = x_{20}.
\end{align*}
$$
...
$$
where $F \in \mathbb{R}^{n \times n}$ and $G \in \mathbb{R}^{n \times 1}$ is **controllable** on $[0, t_f]$ for some $t_f > 0$, if given any initial state $x_0$ and final state $x_f$, there exists a piecewise continuous input $u(t)$ such that the solution of the system satisfies
$$
x(t_f) = x_f.
$$
:::
...
Let $A = [a_{ij}]$ be an $n \times m$ matrix. If $n = m$, then $A$ has full rank if and only if $A$ is nonsingular if and only if $\det(A) \neq 0$.
