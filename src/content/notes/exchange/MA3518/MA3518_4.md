...
Assume that $\mathbb{E}(e) = 0$ and $\operatorname{Var}(e) = \sigma^2$.
...
with mean $\mathbb{E}(Y) = \beta_0 + \beta_1 x$ and variance $\operatorname{Var}(Y) = \sigma^2$ ($Y$ is normal if $e$ is normal).
...
The minimizers are $(\hat{\beta}_0, \hat{\beta}_1)$, with

$$
\hat{\beta}_1= \frac{\sum_i X_i Y_i - \frac{\left(\sum_i X_i\right) \left(\sum_i Y_i\right)}{n}}{\sum_i X_i^2 - \frac{\left(\sum_i X_i\right)^2}{n}}
$$
...
$$
\hat{e}_i = y_i - (\hat{\beta}_0 + \hat{\beta}_1 x_i) = y_i - \hat{y}_i, \quad i = 1, \ldots, n,
$$
...
The sampling distribution of $\hat{\beta}_1$ is normal with mean $\beta_1$ and variance $\sigma^2 / s_{xx}$.

Now we can write,
$$
\hat{\beta}_0 = \sum_i \frac{Y_i}{n} - \hat{\beta}_1 \bar{x},
$$
...
The sampling distribution of $\hat{\beta}_0$ is normal with mean $\beta_0$ and variance $\sigma^2 \left(\frac{1}{n} + \frac{\bar{x}^2}{s_{xx}}\right)$.
...
where $t_0 = \frac{\hat{\beta}_1}{s / \sqrt{s_{xx}}}$ and $T$ is a random variable with distribution $t_{n - 2}$.
...
Replacing the unknown $\sigma$ by its estimator $S$, we get,
...
$$
\left[\hat{\mu}_{Y | X = x} - t_{n - 2, 1 - \alpha / 2} S \sqrt{\frac{1}{n} + \frac{(x - \bar{x})^2}{s_{xx}}}, \hat{\mu}_{Y | X = x} + t_{n - 2, 1 - \alpha / 2} S \sqrt{\frac{1}{n} + \frac{(x - \bar{x})^2}{s_{xx}}}\right].
$$
...
$\mu_{Y | X = x}$ at $X = x$, a predicted value is $\hat{y}(x) = \hat{\beta}_0 + \hat{\beta}_1 x$.
...
2. As $n$ gets larger ($n \to \infty$), the width of CI for $\mu_{Y | X = x}$ decreases to 0, but this is not the case for the prediction interval.
...
$$
\frac{\hat{\beta}_1}{s / \sqrt{s_{xx}}} \sim t_{n - 2}.
$$
...
If $h_{ii} \approx 1$, $\hat{y}_i \approx y_i$.

A popular rule of thumb for detecting leverage points is,

$$
h_{ii} > 2 \times \operatorname{average}(h_{ii}) = 2 \times \frac{2}{n} = \frac{4}{n}.
$$
...
$$
\operatorname{Var}(\hat{e}_i) = \sigma^2 (1 - h_{ii}).
$$

This is reasonable since $h_{ii} \approx 1$ implies that $\hat{y}_i \approx y_i$ and thus $\hat{e}_i$ is small.
...
where $s = \sqrt{\frac{1}{n - 2} \sum_{j = 1}^n \hat{e}_j^2}$.
...
$$
D_i = \frac{\sum_{j = 1}^n (\hat{y}_{j(i)} - \hat{y}_j)^2}{2S^2} = \frac{r_i^2 h_{ii}}{2(1 - h_{ii})^2}.
$$

where $\hat{y}_{j(i)}$ denotes the $j$-th fitted value when the $i$-th observation is deleted.
A recommended cutoff is $4/(n - 2)$.

#### Normality of Errors
Usually the approximation $\hat{e}_i \approx e_i$ is accurate enough to justify use of a QQ-plot.
...
:::algorithm[Estimating the transformation]{#alg:inverse-response}
1. Fit a linear regression model $y_i = \beta_0 + \beta_1 x_i + e_i$.
2. Estimate $g^{-1}$ by plotting $\hat{y}_i = \hat{\beta}_0 + \hat{\beta}_1 x_i$ against $y_i$.
3. Mathematically, we *minimize* $\sum_i (\hat{y}_i - \alpha_0 - \alpha_1 \psi_s(y_i, \lambda))^2$ over $(\alpha_0, \alpha_1, \lambda)$.
:::
