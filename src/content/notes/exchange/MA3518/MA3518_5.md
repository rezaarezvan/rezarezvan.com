...
But, often in the real world, we may want to model our dependent variable from multiple independent variables.

Or, mathematically represented as,

$$
Y_i = \beta_0 + \beta_1 x_{1i} + \beta_2 x_{2i} + \ldots + \beta_p x_{pi} + e_i.
$$
...
$$
\begin{align*}
RSS &= \sum_{i=1}^{n} (y_i - b_0 - b_1 x_{1i} - \ldots - b_p x_{pi})^2, \newline
...
Since we are trying to generalize linear regression with this, it must mean that simple linear regression is a special case of multiple linear regression.
...
\operatorname{Var}(\hat{\beta} | \mathbf{X}) &= \sigma^2 (\mathbf{X}^T \mathbf{X})^{-1} \newline
...
$$
T_i = \frac{\hat{\beta}_i - \beta_i}{\operatorname{SE}(\hat{\beta}_i)} \sim t_{n-p-1}.
$$

Analysis of variance approach to inference is also possible,

$$
H_0 : \beta_1 = \ldots = \beta_p = 0, \quad H_1 : \text{At least one } \beta_j \neq 0.
$$
...
$$
F = \frac{SS_{\text{reg}}/p}{RSS/(n-p-1)} \sim F_{p, n-p-1},
$$
...
$$
H_0 : \beta_1 = \ldots = \beta_k = 0, \quad H_1 : \text{At least one } \beta_j \neq 0.
$$
...
So, let's first develop a regression model that *predicts* the price of a dinner.
...
We can also observe that, if a new restaurant wants to choose a location, it should be east of Fifth Avenue, since our dummy variable is statistically significantly larger than 0.
...
1. $d$ has no effect on $Y$, that is, $Y = \beta_0 + \beta_1 x + e$.
...
> After you present your model to the management team, the debate focuses on the statistical insignificance of the Service variable.
The team feels like the model may be too simple to reflect the reality of an Italian restaurant in Manhattan.
...
```r
m2<-lm(Price~Food+Decor+Service+East+Food:East+Decor:East+Service:East)
> Coefficients:
>             Estimate Std. Error t value Pr(>|t|)
...
4. Assess the effect of each *predictor variable*, having adjusted for the effect of other predictor variables using **added variable plots**.
...
where $\mathbf{H} = \mathbf{X}(\mathbf{X}^T \mathbf{X})^{-1} \mathbf{X}^T$ (also called the hat matrix).
...
$$
h_{ii} > 2 \times \operatorname{average}(h_{ii}) = 2 \times \frac{p+1}{n}.
$$
...
$$
\operatorname{Var}(\mathbf{\hat{e}} | \mathbf{X}) = \sigma^2(\mathbf{I} - \mathbf{H}),
$$
...
$$
s = \sqrt{\frac{\sum_{i=1}^n \hat{e}_i^2}{n - p - 1}}.
$$
...
:::algorithm[Added variable plot]{#alg:added-variable-plot}
...
When highly correlated predictor variables are included, they are effectively carrying very similar information about the response variable.
...
$$
\operatorname{Var}(\hat{\beta}_j) = \frac{1}{1 - R_j^2} \frac{\sigma^2}{(n - 1) S_{x_j}^2}.
$$
