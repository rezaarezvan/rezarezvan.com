...
$$
\sigma = \sqrt{\operatorname{Var}(X)} = \sqrt{\mathbb{E}[(X - \mathbb{E}(X))^2]}.
$$

#### Sample Quantile
For $0 \leq p \leq 1$, the $p$-th quantile for the sample is the data at position $\approx pn$ in the sorted data.

When this is not an integer, a weighted average is used.
...
quantile(x, c(0.25, 0.75)) # 25th and 75th percentiles
```

##### Quantiles VS. Proportions
For a data vector $x$ we can ask two related but inverse questions.

What proportion of the data is less than or equal to a specified value? Or for a specified proportion, what value has this proportion of the data less than or equal?
...
A rule of thumb is:
> An observation is an outlier if it is more than $1.5 \times \operatorname{IQR}$ from the closest quartile.

Also a good rule to remember is:
> An outlier is said to be *extreme* if it is more than $3 \times \operatorname{IQR}$ from the closest quartile.
...
$$
r = \operatorname{cor}(x, y) = \frac{\sum_{i=1}^{n} (x_i - \bar{x})(y_i - \bar{y})}{\sqrt{\sum_{i=1}^{n} (x_i - \bar{x})^2 \sum_{i=1}^{n} (y_i - \bar{y})^2}}.
$$
:::
...
$$
r = \frac{\mathbb{E}[(X - \mathbb{E}(X))(Y - \mathbb{E}(Y))]}{\sqrt{\operatorname{Var}(X)\operatorname{Var}(Y)}}.
$$
:::
...
A random variable $X$ is said to be gamma distributed with parameters $(\alpha, \beta)$, denoted as $X \sim \operatorname{Gamma}(\alpha, \beta)$, if its PDF is given by,
$$
f(x) = \begin{cases}
...
we get the exponential distribution. Thus, $\operatorname{Gamma}(1, \beta) = \operatorname{Exp}(\beta)$.

With this, if $\alpha = n$ is an integer, $X$ can be written as sum of $n$ i.i.d. $\operatorname{Exp}(\beta)$ random variables.
...
If $X \sim \chi^2(n)$, then $\mathbb{E}(X) = n$ and $\operatorname{Var}(X) = 2n$.
...
Mean and variance,
$$
\mathbb{E}(X) = 0, \quad \operatorname{Var}(X) = \frac{n}{n-2}, \quad n > 2.
$$

If $Z \sim N(0, 1)$, $X \sim \chi^2_n$, and $X$ and $Z$ are independent, then $Z / \sqrt{X/n} \sim t_n$.
