...
The Central Limit Theorem (CLT) implies if $n$ is large enough,
$$
...
This yields,
$$
\ell = \mu_0 - z_{1 - \alpha/2} \frac{\sigma}{\sqrt{n}}, \quad u = \mu_0 + z_{1 - \alpha/2} \frac{\sigma}{\sqrt{n}}.
$$
...
$$
\begin{align*}
p & = 1 - P\left(\sqrt{n} \frac{\bar{X} - \mu_0}{\sigma} \in \left[\sqrt{n} \frac{\mu_0 \pm |\bar{x} - \mu_0| - \mu_0}{\sigma}\right]\right) \newline
...
$$
p = 1 - P(T \in [-|t_0|, |t_0|]) = 2P(T > |t_0|), \quad \text{where } T \sim t_{n - 1}.
$$
...
$$
H_1 : \mu_1 \neq \mu_2 \quad \text{(two-sided alternative)},
$$

or,
$$
H_1 : \mu_1 > \mu_2 \quad \text{or} \quad H_1 : \mu_1 < \mu_2 \quad \text{(one-sided alternative)}.
$$

We know that,
$$
\bar{X}_1 = \frac{1}{n_1} \sum_{i = 1}^{n_1} X_{1i} \sim N\left(\mu_1, \frac{\sigma_1^2}{n_1}\right)
$$

and,
$$
\bar{X}_2 = \frac{1}{n_2} \sum_{i = 1}^{n_2} X_{2i} \sim N\left(\mu_2, \frac{\sigma_2^2}{n_2}\right)
$$

we deduce the *sampling distribution* of $\bar{X}_1 - \bar{X}_2$,
$$
\bar{X}_1 - \bar{X}_2 \sim N\left(\mu_1 - \mu_2, \frac{\sigma_1^2}{n_1} + \frac{\sigma_2^2}{n_2}\right)
$$

Now, testing for $H_0 : \mu_1 = \mu_2$ exactly amounts to for $H_0 : \mu_1 - \mu_2 = 0$, with $\bar{X}_1 - \bar{X}_2$ as an estimator for $\mu_1 - \mu_2$.
...
$$
\left(-\infty, (\bar{x}_1 - \bar{x}_2) + z_{1 - \alpha} \sqrt{\frac{\sigma_1^2}{n_1} + \frac{\sigma_2^2}{n_2}}\right]
$$
...
$$
S_{p}^2 = \frac{\sum_{i = 1}^{n_1} \left(X_{1i} - \bar{X}_1\right)^2 + \sum_{i = 1}^{n_2} \left(X_{2i} - \bar{X}_2\right)^2}{n_1 + n_2 - 2}
$$
...
An easy way is just to consider the **differences**.
$$
D_i = X_{1i} - X_{2i}.
$$
...
This yields,
$$
\ell = \frac{\chi^2_{n - 1;1 - \alpha/2} \sigma_0^2}{n - 1}, \quad u = \frac{\chi^2_{n - 1;\alpha/2} \sigma_0^2}{n - 1}.
$$

The decision rule is then,
$$
\text{Reject } H_0 \text{ if } s^2 \notin \left[\frac{\chi^2_{n - 1;1 - \alpha/2} \sigma_0^2}{n - 1}, \frac{\chi^2_{n - 1;\alpha/2} \sigma_0^2}{n - 1}\right].
$$

One-sided CI,
$$
\left[0, \frac{(n - 1)S^2}{\chi^2_{n - 1;1 - \alpha}}\right], \quad \left[\frac{(n - 1)S^2}{\chi^2_{n - 1;\alpha}}, +\infty\right).
$$
...
Let $X_{11}, X_{12}, \ldots, X_{1n_1}$ be a sample from population 1.
Let $X_{21}, X_{22}, \ldots, X_{2n_2}$ be a sample from population 2.

The samples are **independent**, and we would like to know whether $\sigma_1^2 = \sigma_2^2$ or not.
...
$$
\text{Reject } H_0 \text{ if } F \notin \left[F_{n_1 - 1, n_2 - 1;1 - \alpha/2}, F_{n_1 - 1, n_2 - 1;\alpha/2}\right].
$$
...
One-sided CI,
$$
\left[0, \frac{S_1^2}{S_2^2 F_{n_1 - 1, n_2 - 1;1 - \alpha}}\right], \quad \left[\frac{S_1^2}{S_2^2 F_{n_1 - 1, n_2 - 1;\alpha}}, +\infty\right).
$$
...
The $p$-value is,
$$
P(F > f) = 1 - P(F(n_1 - 1, n_2 - 1) \leq f), \quad P(F < f) = P(F(n_1 - 1, n_2 - 1) \leq f).
$$
