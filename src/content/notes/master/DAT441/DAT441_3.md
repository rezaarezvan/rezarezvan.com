...
By Jensen's inequality [^1] [^note-1], and the fact that $\sum_{a \in \mathcal{A}} n_t(a) = t$, we have that,
...
In each round $t$, pick $\arg\max_{a \in \mathcal{A}} \mathrm{UCB}_t(a)$, where $\mathrm{UCB}_t(a) = \bar{\mu}_t(a) + r_t(a)$.
:::
...
$$
\mathcal{E} \coloneqq \{\forall a \forall t |\bar{\mu}_t(a) - \mu(a) \mid \leq r_t(a) \}.
$$
...
These distances are formally described by the gaps $\Delta(a)$ and the distribution distances $\mathrm{KL}(\mathcal{D}_a, \mathcal{D}_{a^{\star}})$ [^note-2].
...
[^note-1]: Here we will use the concave version which states,

    $$
    \frac{1}{n} \sum_{i=1}^{n} f(x_i) \leq f\left(\frac{1}{n} \sum_{i=1}^{n} x_i\right)
    $$
    for a concave function $f$
[^note-2]: Here, $\mathrm{KL}(\cdot, \cdot)$ is the Kullback-Leibler divergence between two distributions,
