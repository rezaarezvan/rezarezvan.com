...
$$
\begin{align*}
v_{\star}(s) & = \max_a \mathbb{E}[R_{t+1} + \gamma v_{\star}(S_{t+1}) \mid S_t = s, A_t = a] \newline
& = \max_a \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma v_{\star}(s^{\prime})],
\newline
\newline
q_{\star}(s, a) & = \mathbb{E}[R_{t+1} + \gamma \max_{a^{\prime}} q_{\star}(S_{t+1}, a^{\prime}) \mid S_t = s, A_t = a] \newline
& = \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma \max_{a^{\prime}} q_{\star}(s^{\prime}, a^{\prime})],
\newline
\end{align*}
$$
:::
...
& = \mathbb{E}_{\pi, \mathcal{p}}[R_{t+1} + \gamma v_{\pi}(S_{t+1}) \mid S_t = s] \newline
& = \sum_a \pi(a \mid s) \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma v_{\pi}(s^{\prime})]
\end{align*}
$$
...
$$
\begin{align*}
v_{k + 1}(s) & \coloneqq \mathbb{E}_{\pi, \mathcal{p}}[R_{t+1} + \gamma v_k(S_{t+1}) \mid S_t = s] \newline
& = \sum_a \pi(a \mid s) \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma v_k(s^{\prime})]
\end{align*}
$$
:::
...
:::algorithm[Iterative Policy Evaluation]{#alg:iterative-policy-evaluation}
**Input:** $\pi$, the policy to be evaluated.

**Algorithm parameter:** a small threshold $\theta > 0$ determining accuracy of estimation.

1. Initialize $V(s)$, for all $s \in \mathcal{S}^+$, arbitrarily, except that $V(\mathrm{terminal}) = 0$.
2. **repeat**
   1. $\Delta \leftarrow 0$
   2. **for** each $s \in \mathcal{S}$ **do**
      1. $v \leftarrow V(s)$
      2. $V(s) \leftarrow \sum_a \pi(a \mid s) \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma V(s^{\prime})]$
...
& = \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma v_{\pi}(s^{\prime})] \newline
\end{align*}
$$
...
$$
\begin{align*}
\pi^{\prime}(s) & \coloneqq \arg\max_{a \in \mathcal{A}(s)} q_{\pi}(s, a) \newline
& = \arg\max_{a \in \mathcal{A}(s)} \mathbb{E}[R_{t+1} + \gamma v_{\pi}(S_{t+1}) \mid S_t = s, A_t = a] \newline
& = \arg\max_{a \in \mathcal{A}(s)} \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma v_{\pi}(s^{\prime})] \newline
\end{align*}
$$
...
$$
\begin{align*}
v_{\pi}(s) & = \max_{a} \mathbb{E}[R_{t+1} + \gamma v_{\pi^{\prime}}(S_{t+1}) \mid S_t = s, A_t = a] \newline
& = \max_{a} \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma v_{\pi^{\prime}}(s^{\prime})] \newline
\end{align*}
$$
...
:::algorithm[Policy Iteration]{#alg:policy-iteration}
...
2. **Policy Evaluation:**
   1. **repeat**
      ...
      2. **for** each $s \in \mathcal{S}$ **do**
         ...
         2. $V(s) \leftarrow \sum_a \pi(a \mid s) \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma V(s^{\prime})]$
         3. $\Delta \leftarrow \max(\Delta, |v - V(s)|)$
   2. **until** $\Delta < \theta$ (a small positive number determining accuracy of estimation)
3. **Policy Improvement:**
   1. `policy-stable` $\leftarrow$ true
   2. **for** each $s \in \mathcal{S}$ **do**
      1. `old-action` $\leftarrow \pi(s)$
      2. $\pi(s) \leftarrow \arg\max_a \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma V(s^{\prime})]$
      3. **if** `old-action` $\neq \pi(s)$ **then** `policy-stable` $\leftarrow$ false
   3. **if** `policy-stable` **then** stop and **return** $V \approx v_{\star}$ and $\pi \approx \pi_{\star}$; **else** go to step 2
:::
...
$$
\begin{align*}
v_{k + 1}(s) & \coloneqq \max_a \mathbb{E}[R_{t+1} + \gamma v_k(S_{t+1}) \mid S_t = s, A_t = a] \newline
& = \max_a \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma v_k(s^{\prime})] \newline
\end{align*}
$$

Or in other words, we turn the Bellman optimality equation for $v_{\star}$ into an update rule.

:::algorithm[Value Iteration]{#alg:value-iteration}
**Algorithm parameter:** a small threshold $\theta > 0$ determining accuracy of estimation.
...
2. **repeat**
   ...
   2. **for** each $s \in \mathcal{S}$ **do**
      1. $v \leftarrow V(s)$
      2. $V(s) \leftarrow \max_a \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma V(s^{\prime})]$
      3. $\Delta \leftarrow \max(\Delta, |v - V(s)|)$
3. **until** $\Delta < \theta$
4. **Output** a deterministic policy $\pi \approx \pi_{\star}$ such that $\pi(s) = \arg\max_a \sum_{s^{\prime}, r} \mathcal{p}(s^{\prime}, r \mid s, a) [r + \gamma V(s^{\prime})]$
:::
