...
:::algorithm[Tabular TD(0) for estimating $v_{\pi}$]{#alg:td0}
**Input:** a policy $\pi$ to be evaluated, step size $\alpha \in (0, 1]$

1. Initialize $V(s)$ arbitrarily for all $s \in \mathcal{S}^+$, except $V(\text{terminal}) = 0$
...
:::note
Since the TD(0) update rule in @alg:td0 bases its update on an existing estimate $V(S_{t + 1})$, it is said to be a bootstrap method, just like DP methods (which bases their updates for $v_{k + 1}$ on $v_k$).
:::
...
:::algorithm[SARSA (On-Policy TD Control) for estimating $Q \approx q_{\star}$]{#alg:sarsa}
...
2. **for** each episode **do**
   ...
   3. **repeat** (for each step of episode)
      ...
      3. $Q(S, A) \leftarrow Q(S, A) + \alpha [R + \gamma Q(S^{\prime}, A^{\prime}) - Q(S, A)]$
      4. $S \leftarrow S^{\prime}; A \leftarrow A^{\prime}$
...
:::algorithm[Q-Learning (Off-Policy TD Control) for estimating $\pi \approx \pi_{\star}$]{#alg:q-learning}
**Algorithm parameters:** step-size $\alpha \in (0, 1]$, small $\epsilon > 0$
...
2. **for** each episode **do**
   ...
   2. **repeat** (for each step of episode)
      ...
      2. Take action $A$, observe $R, S^{\prime}$
      3. $Q(S, A) \leftarrow Q(S, A) + \alpha [R + \gamma \max_{a} Q(S^{\prime}, a) - Q(S, A)]$
      4. $S \leftarrow S^{\prime}$
   3. **until** $S$ is terminal
:::
