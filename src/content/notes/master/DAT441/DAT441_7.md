...
:::algorithm[First-Visit MC Prediction]{#alg:first-visit-mc}
**Input:** a policy $\pi$ to be evaluated.

1. Initialize $V(s)$, arbitrarily for all $s \in \mathcal{S}$
2. Initialize $\mathrm{Returns}(s) \leftarrow []$, an empty list for all $s \in \mathcal{S}$
3. **loop** forever (for each episode)
   1. Generate an episode following $\pi$: $S_0, A_0, R_1, \ldots, S_{T-1}, A_{T-1}, R_T$
   2. $G \leftarrow 0$
   3. **for** each step of episode, $t = T - 1, T - 2, \ldots, 0$ **do**
      1. $G \leftarrow \gamma G + R_{t + 1}$
      2. **unless** $S_t$ appears in $S_0, \ldots, S_{t - 1}$ **do**
         1. Append $G$ to $\mathrm{Returns}(S_t)$
         2. $V(S_t) \leftarrow$ average of $\mathrm{Returns}(S_t)$
:::
...
$$
\pi(s) \coloneqq \arg\max_{a \in \mathcal{A}(s)} q(s, a)
$$
...
$$
\begin{align*}
q_{\pi_k}(s, \pi_{k + 1}(s)) & = q_{\pi_k}\left(s, \arg\max_{a \in \mathcal{A}(s)} q_{\pi_k}(s, a)\right) \newline
...
:::algorithm[Monte Carlo Control with Exploring Starts]{#alg:mc-es}
...
3. Initialize $\mathrm{Returns}(s, a) \leftarrow []$, an empty list for all $s \in \mathcal{S}, a \in \mathcal{A}(s)$
4. **loop** forever (for each episode)
   1. Choose $S_0 \in \mathcal{S}, A_0 \in \mathcal{A}(S_0)$ randomly such that all pairs have probability $> 0$ (exploring starts)
   ...
   4. **for** each step of episode, $t = T - 1, T - 2, \ldots, 0$ **do**
      1. $G \leftarrow \gamma G + R_{t + 1}$
      2. **unless** $(S_t, A_t)$ appears in $(S_0, A_0), \ldots, (S_{t - 1}, A_{t - 1})$ **do**
         1. Append $G$ to $\mathrm{Returns}(S_t, A_t)$
         2. $Q(S_t, A_t) \leftarrow \mathrm{average}(\mathrm{Returns}(S_t, A_t))$
         3. $\pi(S_t) \leftarrow \arg\max_{a \in \mathcal{A}(S_t)} Q(S_t, a)$
:::
...
$$
\begin{align*}
P(\text{greedy action}) & = P(\text{greedy action} \mid \text{greedy}) P(\text{greedy}) + P(\text{greedy action} \mid \text{random}) P(\text{random}) \newline
...
:::algorithm[On-Policy First-Visit MC Control]{#alg:on-policy-mc}
...
4. **loop** forever (for each episode)
   ...
   3. **for** each step of episode, $t = T - 1, T - 2, \ldots, 0$ **do**
      ...
      2. **unless** $(S_t, A_t)$ appears in $(S_0, A_0), \ldots, (S_{t - 1}, A_{t - 1})$ **do**
         1. Append $G$ to $\mathrm{Returns}(S_t, A_t)$
         2. $Q(S_t, A_t) \leftarrow \mathrm{average}(\mathrm{Returns}(S_t, A_t))$
         3. $A^{\star} \leftarrow \arg\max_{a \in \mathcal{A}(S_t)} Q(S_t, a)$
         4. **for** all $a \in \mathcal{A}(S_t)$ **do**
            $$
            \pi(a \mid S_t) \leftarrow \begin{cases} 1 - \epsilon + \frac{\epsilon}{|\mathcal{A}(S_t)|}, & \text{if } a = A^{\star}, \newline \frac{\epsilon}{|\mathcal{A}(S_t)|}, & \text{otherwise.} \end{cases}
            $$
:::
...
\end{align*}
$$
Notice that the sum is a weighted average with nonnegative weights summing to 1 [^note-1], thus,
$$
\begin{align*}
...
[^note-1]: since $\sum_{a \in \mathcal{A}(s)} \pi(a|s) = 1$
