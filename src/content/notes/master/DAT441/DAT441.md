...
The task is to find a principled way to balance the tradeoff between **exploitation** and **exploration**, which we will define later.
...
Thus, we have a tradeoff between exploration and **exploitation**, making (optimal) near-term decisions based on the available information.
...
For a fixed $n$, we consider the following high-probability event [^note-1],
...
This means $\bar{\mu}(a) > \bar{\mu}(a^{\star})$ [^note-2].
...
$$
\begin{align*}
&\text{(clean event for }a\text{)} && -r(a)\leq \bar\mu(a)-\mu(a)\leq r(a) \newline
& && \Rightarrow \bar\mu(a)\leq \mu(a)+r(a) \newline
&\text{(choice in exploitation)} && \bar\mu(a)>\bar\mu(a^\star) \newline
&\text{(clean event for }a^\star\text{)} && -r(a^\star)\leq \bar\mu(a^\star)-\mu(a^\star) \leq r(a^\star) \newline
& && \Rightarrow \bar\mu(a^\star)\geq \mu(a^\star)-r(a^\star) \newline
&\text{combine all three} && \mu(a)+r(a) \geq \bar\mu(a) > \bar\mu(a^\star) \geq \mu(a^\star)-r(a^\star).
\end{align*}
$$
...
And each round in the **exploration phase** contributes at most 1 to the regret [^note-3].

If we combine these two, we can formulate the upper bound of the expected regret as follows [^note-4]
...
$$
\begin{align*}
P(\text{bad}) & = P(\exists a \colon \text{bad}(a)) \newline
...
[^note-1]: This equation might seem arbitrary, but this comes from Hoeffding's Inequality, which we will cover soon.
[^note-2]: This means that, after the exploration phase, the empirical mean of the wrong arm is larger, essentially our agent has not "converged" or found the optimal arm yet.
[^note-3]: This is because the rewards are bounded between 0 and 1 in our case, so always pulling a suboptimal arm can incur at most 1 regret per round.
[^note-4]: This is not the general form, but for only 2 arms.
