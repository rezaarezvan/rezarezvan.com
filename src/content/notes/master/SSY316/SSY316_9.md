...
:::algorithm[$K$-Means Clustering]{#alg:k-means}
1. **Initialization:** Initialize $\{\boldsymbol{\mu}_k^{(0)}\}$ (e.g., randomly select $K$ data points as initial cluster centers).
2. **for** $\ell = 1, \ldots$ **do**
   1. **Assignment step:** For fixed $\{\boldsymbol{\mu}_k^{(\ell - 1)}\}$, solve the optimization problem for each data point $\mathbf{x}_n$,
      $$
      y_{nk}^{(\ell)} = \begin{cases}
      1, & \text{for } k = \arg \min_j \Vert \mathbf{x}_n - \boldsymbol{\mu}_j^{(\ell - 1)} \Vert^2, \newline
      0, & \text{otherwise}.
      \end{cases}
      $$
   2. **Refitting step:** For fixed $\{\mathbf{y}_n^{(\ell)}\}$, solve the optimization problem for each cluster center $\boldsymbol{\mu}_k$,
      $$
      \boldsymbol{\mu}_k^{(\ell)} = \frac{\sum_{n = 1}^N y_{nk}^{(\ell)} \mathbf{x}_n}{\sum_{n = 1}^N y_{nk}^{(\ell)}}.
      $$
3. Repeat until convergence (i.e., cluster assignments do not change or change in loss function is below a threshold).
:::
::::
...
& = - \mathrm{KL}(q_n(\mathbf{z}_n) \,\|\, p(\mathbf{z}_n \mid \mathbf{x}_n, \boldsymbol{\theta}^{(\ell - 1)})) + \log p(\mathbf{x}_n \mid \boldsymbol{\theta}^{(\ell - 1)}).
\end{align*}
$$
...
:::algorithm[Expectation-Maximization (EM) for GMMs]{#alg:em-gmm}
1. **Initialization:** Initialize $\boldsymbol{\theta}^{(0)}$.
2. **for** $\ell = 1, \ldots$ **do**
   1. **E-step:** Evaluate $p(\mathbf{Z} \mid \mathcal{D}, \boldsymbol{\theta}^{(\ell - 1)})$ and compute,
      $$
      Q(\boldsymbol{\theta}, \boldsymbol{\theta}^{(\ell - 1)}) = \sum_{\mathbf{Z}} p(\mathbf{Z} \mid \mathcal{D}, \boldsymbol{\theta}^{(\ell - 1)}) \log p(\mathcal{D}, \mathbf{Z} \mid \boldsymbol{\theta}).
      $$
   2. **M-step:** Solve the optimization problem,
      $$
      \boldsymbol{\theta}^{(\ell)} = \underset{\boldsymbol{\theta}}{\arg\max} \, Q(\boldsymbol{\theta}, \boldsymbol{\theta}^{(\ell - 1)}).
      $$
   3. If convergence criterion not satisfied, return to step 2.1.
:::
::::
