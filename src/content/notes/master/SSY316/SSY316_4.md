...
1. Bayesian Networks (or Bayes nets) which are DAGs [^note-1]. They represent a set of random variables and their conditional dependence structure.
...
Further, the bayesian network encodes a factorization of a join distribution.
...
Consider the joint distribution conditioned on input data and model parameters,
$$
...
:::algorithm[Ancestral Sampling]{#alg:ancestral-sampling}
Assume,
...
Or, in more general, is a given subset of variables $\mathcal{A}$ independent of another set $\mathcal{B}$ conditioned on a third subset $\mathcal{C}$? ($\mathsf{x}_{\mathcal{A}} \perp \mathsf{x}_{\mathcal{B}} \mid \mathsf{x}_{\mathcal{C}}$)
...
[^note-1]: Directed Acyclic Graphs (DAGs) are graphs with directed edges and no cycles.
