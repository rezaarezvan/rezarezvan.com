...
:::note
For explicitness, low energy is associated with high probability states [^note-1].
:::
::::
...
We can encode images using an array representing numerical values of pixels.
...
$$
We will assume that $\mathsf{x}_i, \mathsf{y}_i \in \{+1, -1\}$ ([Ising model](https://en.wikipedia.org/wiki/Ising_model)) [^note-2]
...
$$
Further, an energy term $hx_i$ is often added for each pixel $i$ of $\mathbf{x}$ to biasing the model toward pixel values of a particular color (black or white),
$$
...
:::algorithm[Iterated Conditional Modes]{#alg:icm}
**Initialization:** $x_i = y_i \; \forall i$.

1. **repeat**
   1. **for** $j = 1, \ldots, N$ (for each pixel) **do**
      1. Evaluate total energy for states $x_j = +1$ and $x_j = -1$ keeping all other variables fixed.
      2. Set $x_j$ to the state with lower energy.
2. **until** convergence or stopping criterion met
:::
::::
:::::
...
A Bayesian network captures statistical dependencies while a MRF captures the mutual compatibility among variables.
...
[^note-1]: This is a common concept in physics where systems tend to settle in low-energy configurations.
[^note-2]: The Ising model is a mathematical model used in statistical mechanics to describe ferromagnetism in materials.
