...
::::example
Assume that 90% of people with Creutzfeldt-Jakob (KJ) disease [^1] ate hamburgers.
...
$p(\mathsf{y})$ is called the marginal likelihood (commonness of the data).
...
$$
Seeing quantities as functions of $\boldsymbol{\theta}$, $p(\mathcal{D})$ can be viewed as a normalization constant and we can rewrite the equation as,
$$
...
Now, in the frequentist approach, we can estimate $\mu$ by maximizing $p(\mathcal{D} \mid \mu)$, i.e., the maximum likelihood estimate [^note-1],
$$
\begin{align*}
...
\end{align*}
$$
By differentiating and equating to zero, we obtain the ML estimator,
$$
...
If we do not know anything about the coin, we can choose a uniform prior,
...
However, our posterior can act as a prior if we subsequently observe additional data, i.e., sequential inference.

![The Prior, Likelihood, and Posterior Distributions of our Example; Prior: Beta(5, 2), Likelihood: $\mu^{4}(1 - \mu)$, Posterior: Beta(9, 3)](./imgs/prior_likelihood_posterior2.svg)
...
[^note-1]: or, equivalently, maximizing the log-likelihood, since $\log(\cdot)$ is a monotonic function and does not change the location of the maximum
