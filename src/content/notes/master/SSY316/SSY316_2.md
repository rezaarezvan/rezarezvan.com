...
Again, in more general terms, we want to identify a predictive algorithm that minimizes the error in the prediction of a new label $y$ for an unobserved input $\mathbf{x}$ (generalization loss).
...
In linear regression, we are interested in values of a function $y(\mathbf{x}): \mathbb{R}^d \to \mathbb{R}$, where $\mathbf{x} = (x_1, \ldots, x_d)^T$.
...
This is the generative aspect of these models. We have the capacity to generate a realization of $\mathbf{x}$ by using the marginal $p(\mathbf{x} \mid \boldsymbol{\theta})$.
...
For example, this can define the polynomial degree $M$ (i.e., defines the capacity of the class).
...
\end{align*}
$$
We know that maximizing the likelihood is equivalent to maximizing the log-likelihood [^note-1],
$$
\begin{align*}
...
The bias (approximation error) is caused by our choice of the hypothesis class (model misspecification) and the estimation error (generalization gap) is caused by the fact that $N$ is not large enough.
:::
...
Validation is suitable when we have plenty of data.
However, when data is scarce, we can use cross-validation (CV) techniques.
:::

:::algorithm[$k$-Fold Cross-Validation]{#alg:k-fold-cv}
1. Randomly partition data points into $k$ partitions (folds).
2. **for** each $\kappa \in \{1, \ldots, k\}$ **do**
   1. Train model over all other $k - 1$ partitions.
   2. Compute generalization error on the $\kappa$-th partition.
3. Generalization error estimated as average over all partitions.
4. Choose hypothesis class that minimizes the estimate of the generalization error in step 3.
:::
...
\end{align*}
$$
Thus, the end goal is making predictions of $y$ for new values of $\mathbf{x}$ $\Rightarrow$ We can directly evaluate the posterior distribution $p(y \mid y_{\mathcal{D}}, x_{\mathcal{D}}, \mathbf{x}, \beta)$.
...
[^note-1]: since $\log(\cdot)$ is a monotonic function and does not change the location of the maximum
