...
Thus, the goal (as last time), given a probabilistic model that specifies $p(\mathbf{x}, \mathbf{z})$, we want to find an approximation of the posterior $p(\mathbf{z} \mid \mathbf{x})$.
...
$$
\begin{align*}
\mathrm{KL}(p(\mathbf{x}) \,\|\, q(\mathbf{x})) & = \int p(\mathbf{x}) \log \left(\frac{p(\mathbf{x})}{q(\mathbf{x})}\right) \, d\mathbf{x} \newline
...
1. $\mathrm{KL}(p(\mathbf{x}) \,\|\, q(\mathbf{x})) \geq 0$ (non-negativity).

2. $\mathrm{KL}(p(\mathbf{x}) \,\|\, q(\mathbf{x})) = 0$ if and only if $p(\mathbf{x}) = q(\mathbf{x})$ almost everywhere.

3. It is not symmetric, i.e., $\mathrm{KL}(p(\mathbf{x}) \,\|\, q(\mathbf{x})) \neq \mathrm{KL}(q(\mathbf{x}) \,\|\, p(\mathbf{x}))$.
:::
...
- Variational Inference [^note-1]: Minimize the reverse KL divergence,
$$
q^{\star}(\mathbf{z}) = \underset{q(\mathbf{z} \in \Omega}{\arg\min} \mathrm{KL}(q(\mathbf{z}) \,\|\, p(\mathbf{z} \mid \mathbf{x}))
$$
- Expectation Propagation: Minimize the forward KL divergence,
$$
q^{\star}(\mathbf{z}) = \underset{q(\mathbf{z} \in \Omega}{\arg\min} \mathrm{KL}(p(\mathbf{z} \mid \mathbf{x}) \,\|\, q(\mathbf{z}))
$$
:::
...
But, we can rewrite $\mathrm{KL}(q(\mathbf{z}) \,\|\, p(\mathbf{z} \mid \mathbf{x}))$ as,
$$
\begin{align*}
\mathrm{KL}(q(\mathbf{z}) \,\|\, p(\mathbf{z} \mid \mathbf{x})) & = - \int q(\mathbf{z}) \log \left(\frac{p(\mathbf{z} \mid \mathbf{x})}{q(\mathbf{z})}\right) \, d\mathbf{z} \newline
...
& = \log p(\mathbf{x}) + \mathrm{KL}(q(\mathbf{z}) \,\|\, p(\mathbf{z} \mid \mathbf{x})) \newline
\end{align*}
$$
...
& = - \mathrm{KL}(q_j(\mathbf{z}_j) \,\|\, \tilde{p}(\mathbf{x}, \mathbf{z}_j)) + \text{const} \newline
\end{align*}
$$
...
& = \underset{q_j}{\arg\max} - \mathrm{KL}(q_j(\mathbf{z}_j) \,\|\, \tilde{p}(\mathbf{x}, \mathbf{z}_j)) + \text{const} \newline
& = \underset{q_j}{\arg\min} \mathrm{KL}(q_j(\mathbf{z}_j) \,\|\, \tilde{p}(\mathbf{x}, \mathbf{z}_j)) \newline
...
$$
:::algorithm[Mean-Field Variational Inference with Coordinate Ascent]{#alg:mean-field-cavi}
**Initialization:** Set $\{q_i(\mathbf{z}_i)\}$.

1. **repeat**
   1. **for** $\ell = 1, \ldots, \ell_{\max}$ **do**
      1. Fix $\{q_i(\mathbf{z}_i)\}_{i \neq j}$ to their last estimated values $q_i^{\star}(\mathbf{z}_i)$.
      2. Update $q_j^{\star}(\mathbf{z}_j)$ as,
         $$
         q_j^{\star}(\mathbf{z}_j) = \exp \left(\mathbb{E}_{i \neq j} [\log p(\mathbf{x}, \mathbf{z})] + \text{const}\right).
         $$
      3. Normalize $q_j^{\star}(\mathbf{z}_j)$.
2. **until** ELBO $\mathcal{L}(q)$ converges
:::
::::
...
$$
q^{\star}(\boldsymbol{\theta}) = \underset{q(\boldsymbol{\theta}) \in \Omega}{\arg\min} \mathrm{KL}(p(\boldsymbol{\theta} \mid \mathcal{D}) \,\|\, q(\boldsymbol{\theta}))
$$
...
:::algorithm[Expectation Propagation]{#alg:ep}
1. Initial factors $q_i(\boldsymbol{\theta})$.
2. **repeat** until convergence
   1. Cycle through factors $q_j(\boldsymbol{\theta})$ and optimize as,
      $$
      q_j^{\text{new}}(\boldsymbol{\theta}) = \underset{q_j(\boldsymbol{\theta}) \in \Omega}{\arg\min} \, \mathrm{KL}\left[\frac{1}{p(\mathcal{D})} f_j(\boldsymbol{\theta}) \prod_{i \neq j} q_i^{\text{old}}(\boldsymbol{\theta}) \,\middle\|\, \frac{1}{Z} q_j(\boldsymbol{\theta}) \prod_{i \neq j} q_i^{\text{old}}(\boldsymbol{\theta})\right].
      $$
:::
::::

[^note-1]: One important application and more recent implementation of this is Variational Autoencoders!
