...
We say that $(x,y)$ has a **conditional density** $p_{y \mid x}$ if,

$$
P(y \in A \mid x \in B) = \frac{P_{x, y}(x, y)}{p_x(x)} = \frac{p_{x,y}(x,y)}{\int p_{x, y}(x, y) dy},
$$
...
$$
P(x \in A \mid y \in B) = \frac{p_x(x) p_{y \mid x}(y \mid x)}{p_y(y)}.
$$
...
$$
p(x(t) \mid x(s)) = \frac{P(x(t) \in dy \mid x(s) = x)}{dy} \biggr\rvert_{\substack{y = x(t) \newline x = x(s)}}
$$
...
& = P(x(t_0)) P(x(t_1) \mid x(t_0)) \ldots P(x(t_n) \mid x(t_{n-1})).
\end{align*}
$$
...
$$
P(x(t) \mid x(s)) = \int P(x(t) \mid x(u)) P(x(u) \mid x(s)) \, dx(u), \quad s < u < t,
$$
...
$$
P(\beta(t) \mid \beta(s)) = \frac{1}{\sqrt{2 \pi (t - s)}} \exp\left(-\frac{(\beta(t) - \beta(s))^2}{2(t - s)}\right),
$$
...
$$
p^\star(\beta(t) \mid \beta(s)) = \frac{P(\beta(t) \in dy \mid \beta(s) = x, \beta(T) = v)}{dy} \biggr\rvert_{y = \beta(t)}
$$
Which we can write as,
$$
\begin{equation}
\label{eq:brownian-bridge-density}
= \frac{P(\beta(s)) P(\beta(t) \mid \beta(s)) h(\beta(t), t)}{\int P(\beta(s)) P(\beta(t) \mid \beta(s)) h(\beta(t), t) d \beta(t)},
\end{equation}
$$
where
$$
h(s, x) = \frac{P(\beta(T) \in dy \mid \beta(s) = x)}{dy} \biggr\rvert_{y = v}
$$
:::
...
$$
\frac{P(\beta(t) \mid \beta(s)) h(\beta(t), t)}{h(\beta(s), s)},
$$
...
$$
\frac{\left[ \mathbb{E}\left[ g(x) \mid x(s) = x \right] - g(x) \right] }{t - s} \xrightarrow{t \to s} (Ag)(x),
$$

note that $\mathbb{E}[g(x) \mid x(s) = x] = g(x)$

:::danger[Remark]
Note that $h(\beta(s), s)$ is a Martingale [^3], because,
$$
h(\beta(s), s) = \int P(\beta(t) \mid \beta(s)) h(\beta(t), t) d \beta(t),
$$
...
$$
\mathbb{E}[g(x(t)) \mid x(s) = x, x(T) = v],
$$
...
$$
\begin{align*}
(\frac{\partial}{\partial s} g + A^\star g)(x) & = \lim_{t \to s} \frac{\mathbb{E}[g(x(t)) \mid x(s) = x, x(T) = v] - g(x)}{t - s} \newline
& = \lim_{t \to s} \frac{\mathbb{E}[g(x(t)) - g(x(s)) \mid x(s) = x, x(T) = v]}{t - s} \newline
& = \lim_{t \to s} \int g(x(t)) - g(x(s)) p^\star(x(t) \mid x(s)) \, dx(t) \newline
& = \lim_{t \to s} \int \left[ g(x(t)) - g(x(s)) \right] \frac{h(x(t), t)}{h(x(s), s)} p(x(t) \mid x(s)) \, dx(t) \newline
& = \lim_{t \to s} \frac{\mathbb{E}[[g(x(t)) - g(x(s))] h(x(t), t) \mid x(s) = x]}{h(x(s), s)} \newline
...
$$
\int \frac{P(x(0)) P(x(t) \mid x(0)) h(x(t), t)}{h(x(0), 0)} \, dx(0),
$$

Let's say now that, if we choose a smart $h$, we can determine the density of $x^\star(t)$,

$$
\int \frac{P(x(0)) P(x(t) \mid x(0)) h(x(t), t)}{h(x(0), 0)} \, dx(0) = \pi(x(t)).
$$
