...
$$
\begin{align*}
\frac{d}{dt} \int \phi(x) p(x, t) \, dx & = - \int \phi(x) \frac{\partial}{\partial x} [f(x, t) p(x, t)] \, dx + \frac{1}{2} \int \phi(x) \frac{\partial^2}{\partial x^2} [L(x, t)^2 p(x, t)] \, dx \newline
...
$$
\frac{d \mathbf{P}}{dt} = \mathbb{E}\left[(\mathbf{x} - \mathbf{m}(t)) \mathbf{f}^T\right] + \mathbb{E}\left[\mathbf{f}(\mathbf{x}, t) (\mathbf{x} - \mathbf{m}(t))^T\right] + \mathbb{E}\left[\mathbf{L}(\mathbf{x}, t) \mathbf{L}^T(\mathbf{x}, t)\right].
$$
...
* In **linear-Gaussian** cases, the mean and covariance are sufficient &mdash; they fully determine the solution.
