...
1. Start by constructing a probability path $p_t(\cdot \mid \mathbf{z})$ for individual samples $z$ which interpolates from noise $p_0(\cdot \mid \mathbf{z}) = p_{\text{init}}(\cdot)$ to data sample $z$, $p_1(\cdot \mid \mathbf{z}) = \delta_{\mathbf{z}}(\cdot)$
...
Such a path $p_t(\cdot \mid \mathbf{z})$ is called a **conditional probability path**.
...
:::note[Theorem]
If the ODE with vector field $\mathbf{f}^{\text{target}}(\mathbf{x}(t), t \mid \mathbf{z})$ yields the conditional path $p_t(\mathbf{x}(t), \mathbf{z})$ then the marginal field,
$$
