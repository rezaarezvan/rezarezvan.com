...
$$
\begin{align*}
\lim_{h \to 0} \mathbb{E}[\Vert \underbrace{\beta(t + h) - \beta(t)}_{\sim \mathcal{N}(0, |h|)} \Vert^2] & = \lim_{h \to 0} |h| \newline
& = 0. \quad \square
\end{align*}
$$
...
* Brownian motion is a martingale [^2], $\mathbb{E}[x(t + \tau) \mid x(t)] = x(t)$ for $\tau > 0$.
* Brownian motion has independent and stationary increments and is the most famous Lévy process [^3].
* It is Markovian, i.e., $p(x(t_i) \mid x(t_1), \ldots, x(t_{i-1})) = p(x(t_i) \mid x(t_{i-1}))$ if $t_j > t_{j-1}$ for all $j$.
...
We assume that $t_{i + 1} - t_i = \frac{b - a}{n}$ for all $i \Rightarrow |P| = \underset{i}{\max}(t_{i + 1} - t_i) = \frac{b - a}{n}$.
...
In a more abstract sense, as $n \to \infty$, samples from $z$ **concentrate close to the surface of a sphere**.
