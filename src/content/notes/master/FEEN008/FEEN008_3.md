...
Consider a partition, $a = t_0 < t_1 < \ldots < t_n = b$ such that $t_{i+1} - t_i = \Delta t = \frac{b - a}{n}$ for $i = 0, 1, \ldots, n - 1$.
...
We introduce $\phi(x(t), t) = \log x(t)$. The Itô formula implies,
$$
...
whereas the Itô formula yields an extra $t$ term.
...
& + \frac{1}{2} \sum_{i,j} \frac{\partial^2 \phi(\mathbf{x}(t), t)}{\partial x_i(t) \partial x_j(t)} dx_i(t) \, d x_j(t).
\end{align*}
$$

All higher-order terms vanish except $(d\beta(t))^2 = dt$.
...
\begin{align*}
d \phi(\mathbf{x}(t), t) & = \frac{\partial \phi(\mathbf{x}(t), t)}{\partial t} dt + \sum_i \frac{\partial \phi(\mathbf{x}(t), t)}{\partial x_i(t)} dx_i(t) \newline
& \quad + \frac{1}{2} \sum_{i,j} \frac{\partial^2 \phi(\mathbf{x}(t), t)}{\partial x_i(t) \partial x_j(t)} dx_i(t) \, d x_j(t) \newline
...
& = \frac{\partial \phi}{\partial t} dt + (\nabla_{\mathbf{x}} \phi)^T d \mathbf{x}(t) + \frac{1}{2} \operatorname{tr} \{\mathbf{L}^T \nabla_{\mathbf{x}} \nabla_{\mathbf{x}}^T \phi \mathbf{L} \} dt.
\end{align*}
$$
