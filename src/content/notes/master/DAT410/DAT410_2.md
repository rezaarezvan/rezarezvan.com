...
![The average sales of the best single book is lower than the average sales when each user is shown their own favorite.](./imgs/average_vs_personalized.svg "fig:avg-vs-personalized")
...
![Simplified purchase logs.](./imgs/purchase_logs.svg "fig:logs")
...
$$
\underset{\boldsymbol{\theta}}{\min} \ \frac{1}{2} \sum_{j = 1}^m \sum_{i: r(i, j) = 1} (\theta_i^T x_j - y_{i j})^2
$$
...
$$
\underset{\boldsymbol{\theta}}{\min} \ \frac{1}{2} \sum_{j = 1}^m \sum_{i: r(i, j) = 1} (\theta_i^T x_j - y_{i j})^2 + \lambda \sum_{i = 1}^m \theta_i^2.
$$
