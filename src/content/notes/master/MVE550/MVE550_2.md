...
Thus, our posterior becomes $\mathrm{Beta}(y + 1, n - y + 1)$.
...
Firstly, we need to choose a prior for $\theta$. We will use $\pi(\theta) \propto_{\theta} \frac{1}{\theta}$ [^note-1]
...
[^note-1]: Note that this is an improper prior; it is a "density" that does not integrate to 1. However, using such improper priors is possible in Bayesian statistics.
