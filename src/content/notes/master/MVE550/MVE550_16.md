...
The chain is said to be time-homogeneous if, for all $n \geq 0$ [^note-1],
$$
...
\end{align*}
$$
:::algorithm[Perfect Sampling for the Ising Model]{#alg:perfect-sampling}
1. **for** $i = -n + 1, \ldots, 0$ **do**
   1. Set $\sigma^{(i)} = \sigma^{(i - 1)}$, $\tau^{(i)} = \tau^{(i - 1)}$.
   2. **for** each vertex $v$ **do**
      1. Compute $\pi(\sigma_v^{\star} = 1 \mid \sigma_{-v}^{(i)})$ and $\pi(\tau_v^{\star} = 1 \mid \tau_{-v}^{(i)})$.
      2. Simulate $U \sim \mathrm{Uniform}(0, 1)$.
      3. Update $\sigma_v^{(i)} = g(\sigma^{(i)}, U)_v$ and $\tau_v^{(i)} = g(\tau^{(i)}, U)_v$.
2. **if** $\sigma^{(0)} = \tau^{(0)}$ **then** this is the result; **else** double $n$ and repeat.
:::
::::
...
[^note-1]: We generally assume this unless otherwise stated.
