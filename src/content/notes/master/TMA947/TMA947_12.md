...
$$
\begin{align*}
S & = \{\mathbf{x} \in \mathbb{R}^n \mid g_i(\mathbf{x}) \leq 0, \quad i = 1, \ldots, m \newline
& \quad \quad h_j(\mathbf{x}) = 0, \quad j = 1, \ldots, l \} \newline
\end{align*}
$$
Choose $C^0$ penalty function $\psi : \mathbb{R}^n \to \mathbb{R}_+$ such that,
$$
\psi(s) = 0 \iff s = 0
$$
...
$$
f(\mathbf{x}^{\star}_{\nu}) + \nu \widehat{\chi}_S(\mathbf{x}^{\star}_{\nu}) \leq f(\mathbf{x}^{\star}) + \nu \underbrace{\widehat{\chi}_S(\mathbf{x}^{\star})}_{=0} = f(\mathbf{x}^{\star}), \quad \forall \nu > 0
$$
...
$$
\underset{\mathbf{x} \in S}{\min} f(\mathbf{x}), \quad S = \{\mathbf{x} \in \mathbb{R}^n \mid g_i(\mathbf{x}) \leq 0, \quad i = 1, \ldots, m \}
$$
Assume strictly feasible points exists $\bar{\mathbf{x}} \in \mathbb{R}^n$ such that $g_i(\bar{\mathbf{x}}) < 0, \quad i = 1, \ldots, m$.
...
\chi_S(\mathbf{x}) \leq \nu \widehat{\chi}_S(\mathbf{x}) \coloneqq
\begin{cases}
\nu \sum_{i=1}^m \phi(g_i(\mathbf{x})), & \text{if } g_i(\mathbf{x}) < 0, \quad i = 1, \ldots, m \newline
+\infty, & \text{otherwise} \newline
\end{cases}
$$
where $\phi : \mathbb{R}_- \to \mathbb{R}_+$ is continuous and $\lim_{s_k < 0, s_k \to 0} \phi(s_k) = +\infty$.
...
\begin{align*}
\min & -x_1^2 + x_2^2 \newline
\text{subject to} \quad & x_1^3 - x_2 = 0
\end{align*}
$$
