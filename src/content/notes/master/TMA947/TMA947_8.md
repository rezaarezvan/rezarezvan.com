...
For the optimization problem [^note-1]
$$
\begin{align*}
(P) \quad &
\begin{cases}
...
\begin{align*}
\inf & f(\mathbf{x}) \newline
\text{subject to} \quad & x \in \mathbf{X}
\end{align*}
$$
is a relaxation of the (primal) problem $(P)$ [^note-2].
:::
...
& \text{subject to} \quad \mathbf{z} \in \mathbf{X} \newline
...
1. $$\mathbf{x}^{\star} \in \begin{aligned} & \arg \inf \mathcal{L}(\mathbf{x}, \boldsymbol{\mu}^{\star}) \newline & \text{subject to} \quad \mathbf{x} \in \mathbf{X} \end{aligned} \ , $$
2. $$\mathbf{x}^{\star} \in \mathbf{X}, \quad g_i(\mathbf{x}^{\star}) \leq 0, \quad i = 1, \ldots, m,$$
3. $$\boldsymbol{\mu}^{\star} \geq \mathbf{0},$$
4. $$\mu_i^{\star} g_i(\mathbf{x}^{\star}) = 0, \quad i = 1, \ldots, m.$$
:::
...
& \implies \mu_i^{\star} g_i(\mathbf{x}^{\star}) = 0, \quad i = 1, \ldots, m _\blacksquare.
\end{align*}
$$
:::
...
$$
\begin{align*}
\text{minimize} \quad & x_1^2 + \frac{1}{100} x_2^2 \newline
\text{subject to} \quad & -x_1^2 - x_2^2 + 1 \leq 0
\end{align*}
$$
...
Further, let $f^{\star} > -\infty$ and let $f, g_i, \quad i = 1, \ldots, m$ be convex functions and $\mathbf{X}$ be a convex set.

Finally, assume that, $\exists\, \mathbf{x}_0 \in \mathbf{X}$ such that, $g_i(\mathbf{x}_0) < 0, \quad i = 1, \ldots, m$ (Slater's condition).
...
\text{subject to} \quad & A^T \mathbf{y} \leq 0 \newline
\ & \mathbf{y} \text{ free}
\end{align*}
$$
...
[^note-1]: Note that our "base set" $\mathbf{X}$ is a set where we have "baked in" the rest of the constraints, e.g. $\mathbf{X} = \mathbb{R}^n$ or $\mathbf{X} = \mathbb{R}^n_+$.
[^note-2]: This is why we have called the problems $(P)$ for primal :)
