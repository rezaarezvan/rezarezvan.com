...
$$
where $\mathcal{O} : \mathbb{R} \to \mathbb{R}$ is a function such that,
$$
...
& f : \mathbb{R}^{n} \to \mathbb{R} \cup \pm \infty & \text{: objective function,} \newline
& X \subseteq \mathbb{R}^{n} & \text{: ground set, } \newline
& g_i : \mathbb{R}^{n} \to \mathbb{R} & \text{: constraint function defining restrictions on } \mathbf{x}, \newline
...
$$
\begin{align*}
& a_{ij} = \text{Amount of nutrient } i \text{ in food } j, \quad i \in \text{Nutrients}, j \in \text{Foods}, \newline
& b_i = \text{Recommended daily amount (RDA) for nutrient } i, \quad i \in \text{Nutrients}, \newline
& c_j = \text{Cost of food } j, \quad j \in \text{Foods}.
\end{align*}
$$
...
\text{subject to} & \sum_{j \in \text{Foods}} a_{ij} x_j \geq b_i, \quad i \in \text{Nutrients}, \newline
& x_j \geq 0, \quad j \in \text{Foods}.
\end{align*}
$$
...
- We want to find a mapping $G : \mathbb{R}^{n} \to \{1, \ldots, M\}$ such that $G(\mathbf{x}_i) = y_i$.

One way to do it,
- Parameterize a neural network $G_{\boldsymbol{\theta}} : \mathbb{R}^{n} \to \Delta$, where $\Delta = \{\mathbf{s} \in \mathbb{R}^{M} | \mathbf{s} \geq \mathbf{0}, \sum_{i=1}^M s_i = 1 \}$.
- Let $\mathbf{e}_i \in \mathbb{R}^{M}$ be the $i$-th unit vector, and $E : \{1, \ldots , M \} \to \{\mathbf{e}_i \}_{i = 1}^M, E : i \to \mathbf{e}_i$.
- Let $F : \Delta \times \Delta \to \mathbb{R}, F : (\mathbf{t}, \mathbf{s}) \to - \sum_{i=1}^M t_i \log(s_i)$ (Cross-Entropy loss function).
- Solve,
$$
\hat{\boldsymbol{\theta}} = \underset{\boldsymbol{\theta} \in \Theta}{\arg \min} \sum_{\ell = 1}^N F(E(y_{\ell}, G_{\boldsymbol{\theta}}(\mathbf{x}_{\ell}))).
$$

So, our ML algorithm for classification is $G_{\hat{\boldsymbol{\theta}}}$.
