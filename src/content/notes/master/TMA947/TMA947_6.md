...
where $f : \mathbb{R}^n \to \mathbb{R}$ and $S \subseteq \mathbb{R}^n$.
...
$$
R_S(\mathbf{x}) \coloneqq \{ \mathbf{p} \in \mathbb{R}^n \mid \exists \delta > 0, \quad \mathbf{x} + \alpha \mathbf{p} \in S, \quad \forall \alpha \in [0, \delta] \}
$$
:::
...
& \quad \quad \text{such that} \quad \lim_{t \to \infty} \mathbf{x}_t = \mathbf{x}, \quad \text{and} \newline
...
where $f, g_i \in C^1$ for $i = 1, \ldots, m$. Thus, $S = \{\mathbf{x} \in \mathbb{R}^n \mid g_i(\mathbf{x}) \leq 0, \quad i = 1, \ldots, m \}$.
...
$$
\overset{\circ}{G}(\mathbf{x}) \coloneqq \{ \mathbf{p} \in \mathbb{R}^n \mid \nabla g_i(\mathbf{x})^T \mathbf{p} < 0, \quad \forall i \in \mathcal{I}(\mathbf{x}) \}
$$
:::
...
$$
G(\mathbf{x}) \coloneqq \{ \mathbf{p} \in \mathbb{R}^n \mid \nabla g_i(\mathbf{x})^T \mathbf{p} \leq 0, \quad \forall i \in \mathcal{I}(\mathbf{x}) \}
$$
:::
...
Let $S = \{\mathbf{x} \in \mathbb{R}^2 \mid \underbrace{(x_1 - 1)^2 + x_2^2}_{g_1(\mathbf{x})} \leq 1, \quad \underbrace{(x_1 + 1)^2 + x_2^2}_{g_2(\mathbf{x})} \leq 1 \}$ and consider the point $\mathbf{x} = \begin{bmatrix} 0 & 0 \end{bmatrix}^T$.
...
$$
\begin{align*}
\overset{\circ}{G}(\mathbf{x}) & = \{ \mathbf{p} \in \mathbb{R}^2 \mid \begin{bmatrix}-2 \newline 0 \end{bmatrix}^T \mathbf{p} < 0, \quad \begin{bmatrix}2 \newline 0 \end{bmatrix}^T \mathbf{p} < 0 \} = \{ \mathbf{p} \in \mathbb{R}^2 \mid p_1 > 0, \quad p_1 < 0 \} = \emptyset \newline
G(\mathbf{x}) & = \{ \mathbf{p} \in \mathbb{R}^2 \mid \begin{bmatrix}-2 \newline 0 \end{bmatrix}^T \mathbf{p} \leq 0, \quad \begin{bmatrix}2 \newline 0 \end{bmatrix}^T \mathbf{p} \leq 0 \} = \{ \mathbf{p} \in \mathbb{R}^2 \mid p_1 \geq 0, \quad p_1 \leq 0 \} = \{ \mathbf{p} \in \mathbb{R}^2 \mid p_1 = 0 \}
\end{align*}
$$
...
\mu_i g_i(\mathbf{x}^{\star}) = 0, \quad i = 1, \ldots, m \newline
...
\nabla g_i(\mathbf{x}^{\star})^T \mathbf{p} \leq 0, \quad i \in \mathcal{I}(\mathbf{x}^{\star})
\end{cases} \newline
...
\end{cases}
\newline
& \text{which are precisely the KKT conditions.} \quad _\blacksquare
\end{align*}
$$
:::
