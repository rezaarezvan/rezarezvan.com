...
Recall the standard form polyhedron $P = \{\mathbf{x} \in \mathbb{R}^n \mid A \mathbf{x} = \mathbf{b}, \quad \mathbf{x} \geq \mathbf{0}\}$.
...
:::algorithm[The Simplex Method]{#alg:simplex-idea}
...
:::algorithm[The Simplex Method]{#alg:simplex}
1. Let $\bar{\mathbf{x}}$ be an initial BFS, corresponding to the partitioning $A = \begin{bmatrix} B & N \end{bmatrix}$.
2. Compute the reduced costs $\tilde{\mathbf{c}}_N^T = \mathbf{c}_N^T - \mathbf{c}_B^T B^{-1} N$.
   1. **if** $\tilde{\mathbf{c}}_N^T \geq \mathbf{0}$ **then** stop, $\bar{\mathbf{x}}$ is optimal.
   2. **else** choose $j \in \underset{k : (\tilde{\mathbf{c}}_N)_k < 0}{\arg \min} (\tilde{\mathbf{c}}_N)_k$.
3. Compute the search direction $\mathbf{d} = \begin{bmatrix} -B^{-1} N \mathbf{e}_j \newline \mathbf{e}_j \end{bmatrix}$.
   1. **if** $-B^{-1} N \mathbf{e}_j \geq \mathbf{0}$ **then** stop, the LP is unbounded.
...
\text{subject to} \quad & A \mathbf{x} + I \mathbf{y} = \mathbf{b} \newline
