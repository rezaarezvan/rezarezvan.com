...
\mathbf{p} & = -\underbrace{\left(\nabla^2 f(\mathbf{x}_k)\right)^{-1}}_{\text{has inverse due to full rank assumption, i.e.,} \quad \succ 0} \nabla f(\mathbf{x}_k)
\end{align*}
$$
:::
...
1) $\Vert \mathbf{p}_k \Vert$ is bounded, i.e., $\exists M > 0 : \Vert \mathbf{p}_k \Vert \leq M, \quad \forall k$.
