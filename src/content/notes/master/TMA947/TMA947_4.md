...
A set $S \subseteq \mathbb{R}^n$ is called *open* if $\forall \mathbf{x} \in S, \quad \exists \epsilon > 0 : B_{\epsilon}(\mathbf{x}) \subseteq S$.

A set $S \subseteq \mathbb{R}^n$ is called *closed* if its complement $\mathbb{R}^n \setminus S$ is open (complement of an open set).

A set $S \subseteq \mathbb{R}^n$ is called bounded if $\exists M > 0 : \forall \mathbf{x} \in S : \Vert \mathbf{x} \Vert < M, \quad \forall \mathbf{x} \in S$.
...
Let $S \subseteq \mathbb{R}^n$ and let $f : S \to \mathbb{R}$.
...
[^note-1]: Technically, this still holds for $f$ is lower semi-continuous on $S$, but we will not cover that here.
