...
$$
\ddot{y}(t) = \dot{(\dot{y})} \approx \dfrac{\dot{y}(t + h) - \dot{y}(t)}{h} \approx \dfrac{\dfrac{y(t + 2h) - y(t + h)}{h} - \dfrac{y(t + h) - y(t)}{h}}{h} = \dfrac{y(t + 2h) - 2y(t + h) + y(t)}{h^2}.
$$

:::example
$$
\dot{y}(t) + ay(t) = a \cdot u(t), \quad t = kh, \quad k = 0, 1, \ldots
$$

$$
\begin{align*}
\dfrac{y(kh + h) - y(kh)}{h} + ay(kh) & = a \cdot u(kh) \newline
y(kh + h) & = (1 - ah)y(kh) + ah \cdot u(kh).
\end{align*}
$$
:::

### Z-transform
We won't go that much in depth about it:
...
& = \sum_{k = 0}^{\infty} (e^{-ah} \cdot z^{-1})^k \newline
...
Inverse sensitivity function, $T_d(z) = \dfrac{L_d(z)}{1 + L_d(z)}$
