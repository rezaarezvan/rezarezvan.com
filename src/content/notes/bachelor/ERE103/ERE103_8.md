...
$$
g(t) = \mathcal{L}^{-1}\{G(s)\}, \quad G(s) = \dfrac{Y(s)}{U(s)} = Y(s).
$$
:::
...
Let's call those specific frequencies for $\omega_c$ and $\omega_\pi$. We'll call them the crossover and phase crossover frequency respectively.

Meaning that:
$$
|L(j\omega_c)| = 1, \quad \arg(L(j\omega_\pi)) = -180^\circ.
$$
...
Phase margin:
$$
\phi_m = \arg(L(j\omega_c)) - (-180^\circ) = \arg(L(j\omega_c)) + 180^\circ, \quad \text{given that } |L(j\omega_c)| = 1.
$$

Amplitude margin:

$$
A_m = \dfrac{1}{|L(j\omega_\pi)|}, \quad \text{given that } \arg(L(j\omega_\pi)) = -180^\circ.
$$
:::
...
$$
\arg(L(j\omega)) = 0 - 90^\circ - 2 \arctan(\omega).
$$
...
$$
\begin{align*}
c_0 & = \dfrac{a_1 a_2 - a_0 a_3}{a_1}, & d_0 & = \dfrac{c_0 a_3 - a_1 c_1}{c_0}, \newline
c_1 & = \dfrac{a_1 a_4 - a_0 a_5}{a_1}, & d_1 & = \dfrac{c_0 a_5 - a_1 c_2}{c_0}, \newline
c_2 & = \dfrac{a_1 a_6 - a_0 a_7}{a_1}, & d_2 & = \dfrac{c_0 a_7 - a_1 c_3}{c_0}.
\end{align*}
$$
...
### Nyquist's simplified criterion
:::theorem[Nyquist's simplified criterion]
...
![Nyquist plot of $L(s) = \frac{e^{-s}}{s(1 + s)}$; the dashed curve is the mirror image for negative $\omega$.](./imgs/nyquist.svg "fig:nyquist")
...
& = \dfrac{2K - 3Kj\omega - K\omega^2}{j\omega(1 + \omega^2)(2^2 + \omega^2)} \newline
& = \dfrac{-3Kj\omega + K(2 - \omega^2)}{j\omega(1 + \omega^2)(2^2 + \omega^2)} \newline
& = \dfrac{-3Kj\omega}{j\omega(1 + \omega^2)(2^2 + \omega^2)} + \dfrac{K(2 - \omega^2)}{j\omega(1 + \omega^2)(2^2 + \omega^2)} \newline
& = \dfrac{-3K}{(1 + \omega^2)(2^2 + \omega^2)} - j\dfrac{K(2 - \omega^2)}{\omega(1 + \omega^2)(2^2 + \omega^2)}.
\end{align*}
$$
...
![Nyquist plot of $L(s) = \frac{K}{s(s + 1)(s + 2)}$ for $K = 2$; the curve crosses the real axis at $-K/6$.](./imgs/k2.svg "fig:nyquist-k2")

For $K = 6$:

![Nyquist plot for $K = 6$; the curve passes through $-1$.](./imgs/k6.svg "fig:nyquist-k6")

For $K = 12$:

![Nyquist plot for $K = 12$; the curve crosses the real axis to the left of $-1$.](./imgs/k12.svg "fig:nyquist-k12")
:::

### Nyquist criterion
The proof and intuition behind the Nyquist criterion is quite a lot and requires knowledge in complex analysis. We don't do that here.
