...
![Bode plot of the low-pass filter $G(s) = \frac{a}{s + a}$ for $a = 1$.](./imgs/lowpass.svg "fig:lowpass")
...
![Bode plot of the high-pass filter $G(s) = \frac{s}{s + b}$ for $b = 1$.](./imgs/highpass.svg "fig:highpass")
...
### Butterworth filters
When we need to perform filter realization, we usually will resort to Butterworth:

$$
\begin{align*}
\text{Order 1: } & G(s) = \dfrac{\omega_1}{s + \omega_1}, \newline
\text{Order 2: } & G(s) = \dfrac{\omega_1^2}{s^2 + 2 \zeta \omega_1 s + \omega_1^2}, \quad \zeta = \frac{1}{\sqrt{2}}.
\end{align*}
$$
