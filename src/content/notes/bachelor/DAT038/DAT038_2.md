...
One thing to remember is - that in the real world the only thing that determines the output isn't just the input. For example an input of $n$ doesn't always give $T(n)$ for a given quicksort algorithm.
...
:::definition[Order of growth]
Let $f$ and $g$ be functions.
$f$ has an **order of growth** of $g$ if there are constants $C$ and $n_0$ such that,
...
#### Orders of growth
One with a background in Calculus soon realizes that *order* of these Big O functions will be the following,
...
:::definition[Relatives of O-notation]
Let $f$ and $g$ be functions.
...
This means that $f(n) \in \Theta(g(n))$ means $f(n) \in \mathcal{O}(g(n))$ **and** $f(n) \in \Omega(g(n))$.
...
and to get a somewhat accurate answer about questions one can have about programs, for example, runtime of a program.

Next part will be about dynamic arrays and how we use them - especially how to implement stacks and queues.
