...
Now we can say that $T(n)$ has a $\mathcal{O}(n^2 \\log(n))$ complexity! Since we are talking about $\mathcal{O}$, this means this function has an **upper** bound of this. This means that $T(n)$ also has a complexity of $\mathcal{O}(n^3)$ for example.
...
### Complexity Analysis
Let's analyze the following snippet of code and its complexity.
```
found = false
for x in list:
    for y in list:
...
So the loops will now go through $n^2$ elements. Which in total results in a complexity of $\mathcal{O}(n^4)$.
...
* Hash Tables:
    * Add/Remove/Search (Given that the hash function is 'good'):
        * $\mathcal{O}(1)$
* General Tree:
    * If you go **down** in a tree, you'll visit:
        * $\mathcal{O}(\text{height})$ nodes
...
Let's take a look at a program which utilizes different data structures now:
```
pairs_list = []
for i in 0 .. n - 1:
    for j in i .. n - 1:
...
So, we've seen the first part, we know it's $\mathcal{O}(n^2)$.
...
$$
\mathcal{O}(n^2 \\log(n^2)) = \mathcal{O}(2 n^2 \\log(n)) = \mathcal{O}(n^2 \\log(n)).
$$
...
The same applies for `add()`. Since the rest of the operations are constant, it will be $\mathcal{O}(n^4)$.
...
* Average-case
    * Can be useful sometimes, mostly gives us an 'indicator'.

Let's now talk about **expected** and **amortised** complexity.

#### Expected Complexity
:::definition[Expected complexity]
This is useful for randomized algorithms! It's the average over all possible random choices for a particular input.
:::
...
:::definition[Amortised complexity]
