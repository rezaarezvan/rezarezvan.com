...
A priority queue is a collection of items which we can:
* Add an item to the queue
* Find the *smallest* item in the queue
* Remove the *smallest* item in the queue
:::

In this part we'll handle a priority queue as a min-priority queue,
...
* The scheduler in a computer operating system
    * A lot of different parameters but for example, prioritize tasks which take the minimum time.
* Sorting a list
...
:::algorithm[Sorting with a priority queue]{#alg:pq-sort}
**Input:** a list `arr`

1. `pq` $\leftarrow$ new min-priority queue
2. **for** `x` in `arr` **do**
   1. `pq.add(x)`
3. `result` $\leftarrow$ `[]`
4. **while not** `pq.empty()` **do**
   1. `result.add(pq.remove_min())`
5. **return** `result`
:::

There are loads more applications, we'll be looking into the graph searching and more AI applications.
...
So the answer is, using a **binary heap** - it has the same complexity as a BST, but is simpler to implement.
...
The tree is *complete* if all levels are full *except* the bottom level, which is filled from left to right.
:::
...
So, if we have node $i$, the left child is at index $2i + 1$ and the right child at $2i + 2$. You can test this for yourself, but this is just indexing left to right level by level.

We can also use that the *parent* of node $i$ is at index $\lfloor (i - 1) / 2 \rfloor$.
