...
The answer is quite simple - it differs from problem to problem! Sometimes a stack might just be *exactly* what you want, and sometimes you might feel hindered or blocked by an ADT.

So you need to do research on what kind of ADT you should use before actually implementing one. Also cost/how hard the implementation will be is also an important factor.
...
For example, iterating through a queue. Note that some data structures actually don't allow iterating throughout the collection!
...
* Usually a kind of iterator for looping over all elements/members in the set.
:::
...
Maps can be seen as a further generalization of lists even. The integer position is now replaced by a generic 'key' type.
...
use an underlying map to implement multisets and multimaps.
...
### Graphs
Graphs should also be familiar from the discrete mathematics course - but let's quickly go over it. Graphs consist of *nodes* (also called vertices, points)
...
As said earlier, graphs come in many different shapes and forms - either they are directed or undirected as well as weighted or unweighted.
...
![The ADTs from this part and the data structures that implement them.](./imgs/adt_summary.svg "fig:adt-summary")

In the next part we'll begin with a concept we mentioned today - Hash tables!
