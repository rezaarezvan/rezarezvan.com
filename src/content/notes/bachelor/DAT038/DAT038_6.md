...
Therefore, all nodes - except the root node - **must** have exactly **one** parent node.

#### Tree rules
I've already stated some of the 'rules' of a tree, but generally we have 3 rules that make a collection of nodes a tree.
* Each node must have exactly one **unique** parent node - except the root node.
...
There are two main methods for traversing - **Pre-order** traversal - meaning you process each node **before** visiting its children.
...
Some terminology commonly used for trees are `size`, `height`, and `level`.
...
Binary search trees or BSTs are a kind of binary tree but with some extra conditions:
...
If we ever hit a 'null' node, we can also say that the key we're searching for doesn't exist in this tree.
...
#### Deleting a leaf
A so-called leaf is a node with no children (or visually the leaves are the bottom nodes). These are fine to remove without any corrections.
...
This is where it becomes tricky, but if we take a step back and think about it, we need to find a node that upholds the requirements of,
...
The *greatest* (rightmost) key in the node's *left* subtree ***or*** the *smallest* (leftmost) in the node's *right* subtree.
...
So the main takeaway is that, if a BST is balanced, it's really powerful and efficient. So let's take a look at how we can make it balanced.
...
### AVL Trees
AVL Trees have a less restrictive invariant compared to BSTs - in an AVL tree we keep the tree *almost* balanced.

:::definition[AVL invariant]
...
In the Left-right case, the extra height is in the root's *left* child then in the *right child* of that. We can't fix it with one rotation - but luckily we still have a solution.
...
In conclusion, AVL trees use rotation to keep the tree balanced. In the worst case the height of the tree is $1.44 \log_2(n)$, but normally close to $\log_2(n)$.

### 2-3 Trees
2-3 trees are also **height** balanced trees. 2-3 trees are made from two kinds of nodes:
...
It proceeds just as in a BST. In a 2-node, compare the search key against the key in the node, descend into one of the two children.
...
The resulting tree will have a height of between $\log_3(n)$ and $\log_2(n)$.
...
So to fix this we use something called 'red-black BSTs'.

### Red-black BSTs

A red-black BST implements a 2-3 tree *using a BST*. We can see a 2-3 node as a BST tree since - a 2-node becomes a regular BST node, however, a 3-node becomes **two** BST nodes.
...
So a Red-black BST is really just a 2-3 tree but stored in a BST since we translate it to the corresponding 2-3 tree.

### Red-black Tree Invariant
:::definition[Red-black tree invariant]
...
Update strategy is: Insert in a way that breaks the rules on red nodes, then fix it.
