...
Maps are **sets** of *keys*, each having an associated value - or you can formulate it as - a **set** of *key-value pairs*.
...
Only problem is that, our usual implementation of maps makes `search()` and `update()` quite slow, in fact, the complexity is usually
$\mathcal{O}(n)$ or in case we're using a *multimap* $\mathcal{O}(n \cdot m)$.
...
One thing to know before we dive in is - there are two different strategies when it comes to hash tables, we'll begin with separate chaining.

#### Hash Functions
Before we start talking about what a hash **table** is we first need to look into what a hash **function** is. The problem with ordinary maps is that they are an unordered data structure.
...
At the same time our hash function needs to be 'good'. We can't have a hash function that only returns index 1 and stack every item into the same slot. But we also can't be lazy, in the example of sorting a collection of people.
We can't just go by their first letter in their name, there are a lot more names that begin with an 'A' than a 'Z' for example.

So we want a hash function that also *evenly* distributes the objects, and at the same time is efficient to compute.

With this in mind we can now define a hash table.

### Hash Tables
:::definition[Hash table]
A hash table uses a hash function to compute the array index.
:::

It's not possible to put more than one object into one array slot, so if our hash function returns the same index for different objects (we will later see, by definition, this is a bad hash function) we need a solution.
...
### Load Factor
We should always assume that we have a good hash function, since in most libraries, they are good! But to understand why it's 'good' we need to understand something called *Load factor*.
:::definition[Load factor]
The load factor is the average number of elements per slot/index.

So load factor $= \frac{N}{M}$, where $N$ is the # of elements and $M$ is the array size.
:::
...
resize(buckets : int):
    ...
    for bucket in oldtable:
        for k,v in bucket:
            this.put(k, v)
```
In this example I've chosen to resize the table with a factor of 2. However, this can actually lead to some problems.
...
How we compress it is quite simple (often) - it is:
```
index = hash_code % array_size
i = h % M
```

This is called 'modular' hashing, it's the most common one. One thing to remember is that the *hash code* **never** changes. Only the compressed version can change since we can change the size of it.
...
### Deletion
If we want to delete an object while using Open Addressing - we will encounter a very famous problem called 'clusters'.
...
### Clustering
As mentioned before - clusters are a by-product of open addressing - but these make the search times slower, in the worst case it becomes linear!
...
Instead of increasing linearly, we could do *quadratic*, *double hashing* which depends on a *second* hash function, or *moving* alternatives such as:
...
In the next part we'll begin looking at trees - starting at **B**inary **S**earch **T**rees or BSTs for short. We'll actually compare them to Hash maps, since they are quite similar in some aspects.
