...
A *state* in these diagrams is a *possible program state*.
*Transitions* on the other hand, *connect* these states to an execution order.

One thing to note is that, the structural properties in these diagrams,
capture the semantic properties of the corresponding program.
...
While, the final states of a program are usually marked with double-line edges.

#### Transitions
Transitions, as we defined them, connect these states. So it's natural that,
...
As we said earlier, the structural properties of a transition diagram tell
the semantic properties about the program.
...
* Starvation Freedom:
    * There is no (looping) path such that a thread never enters its critical section while trying to do so.
* No Race Conditions:
...
#### Problem description
Let's quickly go over what our problem is, so we have an easier time solving it.
...
We want to design a (concurrent) program where we achieve mutual exclusion,
...
:::table[A trace of the naive attempt where both threads enter the critical section.]{#naive-trace}
| # | $t_0$ | $t_1$ | shared |
|:-:|:--|:--|:--|
| 1 | $pc_0$: `await (!enter[1])` | $pc_1$: `await (!enter[0])` | `enter: false, false` |
| 2 | $pc_0$: `enter[0] = true` | $pc_1$: `await (!enter[0])` | `enter: false, false` |
| 3 | $pc_0$: `enter[0] = true` | $pc_1$: `enter[1] = true` | `enter: false, false` |
| 4 | $pc_0$: critical section | $pc_1$: `enter[1] = true` | `enter: true, false` |
| 5 | $pc_0$: critical section | $pc_1$: critical section | `enter: true, true` |
...
It's important now that `await(condition)` is short for `while(!condition) { }`.
...
Peterson's algorithm ensures all three points that we want to achieve.

We can prove this but really isn't that interesting :P. However, we can use Peterson's algorithm
for `N` threads as well. It's a bit trickier to implement, but the idea is that we use a kind of "filtering".
...
### Fairness
Although Peterson's algorithm ensures all three properties we listed,
threads may access the critical section again before "older" threads.
...
* Bounded waiting:
    * When a thread, $t$ is waiting to enter its critical section, the maximum
    number of times *other* arriving threads are allowed to enter their critical section
...
In Java however we can't have volatile arrays, but there are so-called `AtomicArrays` we can use.
