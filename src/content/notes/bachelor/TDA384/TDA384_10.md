...
:::definition[Lock-free and wait-free]
...
Now let's look at `CoarseSet`'s successor.
...
:::algorithm[Optimistic locking]{#alg:optimistic-locking}
...
3. Validate the position while the nodes are locked:
   1. **if** the position is **valid** **then** perform the operation while the nodes are locked, then release locks.
   2. **if** the position is **invalid** **then** release locks and repeat the operation from **scratch**.
:::
...
it is possible that only one of them is successful.
