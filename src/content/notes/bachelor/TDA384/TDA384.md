...
We, the programmers, don't have any control in what order these threads will be executed, this is the *scheduler's* job.
...
If we compile and run this, we won't necessarily always get the expected answer, 2. In some (rare) cases, we will get the answer 1.
...
where $p$ is the fraction of the program that is being run in parallel.
:::

By plugging in some arbitrary values we find quickly that, after a certain threshold, more parallelism doesn't help.
...
There are 3 states which a process can be in:
