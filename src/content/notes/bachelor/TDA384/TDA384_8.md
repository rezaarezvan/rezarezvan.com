...
This process keeps track of which *other* processes have arrived at the barrier point.

When a new process arrives at the barrier, it sends an `arrived` message to the barrier process.
When the list of all arrived processes is complete, the barrier process sends a `continue` message to everyone.

After notifying all other processes, the barrier process itself goes back to its initial state.
...
The allocator ensures exclusive access to a single user, and keeps track of the number of available resources.
...
If requests exceed the availability, they fall into our built-in mailbox.
...
this doesn't prevent starvation since this version prioritizes readers.

The solution based on two monitors is an approach here, but it's quite cumbersome for a message-passing program.
...
So, as long as the waiter allows strictly fewer philosophers than the total number of forks to sit around the table, deadlock and starvation are avoided.
...
Now, each `fork` is also a process, which keeps track of whether the fork is free or not.
