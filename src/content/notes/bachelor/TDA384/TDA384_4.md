...
One possible solution is to number each fork and philosopher. So that we can easily know what is right and left of each philosopher.
...
We make each seat a semaphore like:
```
// M < N
...
One for keeping track of how many items there are in the buffer. The other to know how many slots are **left**.
...
Even if we use strong semaphores, we cannot prevent this. This is because the
...
* Each writer has exclusive access
    * Meaning that we have the invariant: $\#\text{writers} = 0 \lor (\#\text{writers} = 1 \land \#\text{readers} = 0)$
...
### Summary
A looong part, but as we can see, all solutions use locks and semaphores.
