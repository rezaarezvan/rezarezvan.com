...
### Types in Erlang
Erlang has 8 primitive types and 5 compound types:
...
which are the "normal" equality we are used to.
...
As you can see, `f/n` declares the function name and number of parameters (or arity n, with patterns).
...
Erlang is very similar to Haskell in its clauses, guards, pattern matchings.
...
* If you know the name of a process, you can send a message to it.
...
Messages are as simple, typically a message is the result of evaluating an expression:
```
Pid ! Message
```
...
Evaluating the `receive` expression, selects the **oldest** message in our mailbox, that matches the pattern.
...
These values get updated by the recursive calls used to make a process permanently running.
