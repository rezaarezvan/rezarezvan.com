...
Used for plotting any kind of plot you can think of, scatter plots, line plots, contour plots, etc.
...
Let's reshape our NumPy array so it is a bit more readable and how it looks in Wikipedia.
```py
...
Let's break it up into the four individual datasets:
```py
...
Before we plot the actual graphs, let's take a look at the mean, standard deviation and variance for all four of the datasets.
```py
...
So, from a purely statistical view, we would think that all these datasets should look *somewhat* similar, right?
...
for key, value in anscombe.items():
    ...
    plt.show()
```
![Anscombe's dataset I.](./imgs/anscombe_i.svg "fig:anscombe-i")

![Anscombe's dataset II.](./imgs/anscombe_ii.svg "fig:anscombe-ii")

![Anscombe's dataset III.](./imgs/anscombe_iii.svg "fig:anscombe-iii")

![Anscombe's dataset IV.](./imgs/anscombe_iv.svg "fig:anscombe-iv")
...
plt.show()
```
![Anscombe's quartet: four datasets with the same mean, variance and correlation.](./imgs/anscombe.svg "fig:anscombe")
...
When dealing with large datasets, we'll usually want to use common convenient functions to read, write and manipulate data fast.

Pandas can make use of NumPy as a backend for some data.

Before jumping into a practical example where we'll use Pandas, let's first understand how we'll represent data.
...
Whereas `.loc` is string based, in this case both `iloc` and `loc` yields the same answer since we group the data with index.
...
plt.show()
```
![Culmen depth against culmen length for the three penguin species.](./imgs/palmer.svg "fig:palmer")
