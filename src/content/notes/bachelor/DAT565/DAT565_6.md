...
$$
f(x_1, x_2, \ldots, x_k) = b + \sum_{i = 1}^k a_i x_i.
$$

:::definition[Linear map]
A **linear map** between vector spaces $V$ and $W$ over a field $K$ is a function, $f \colon V \to W$ that satisfies for all $x, y \in V$ and $c \in K$:
$$
\begin{align*}
f(x + y) & = f(x) + f(y), \newline
f(cx) & = cf(x).
...
$$
\operatorname{MSE}(\hat{y}, y) = \frac{1}{n} \sum_{i = 1}^n \left(\hat{y}_i - y_i\right)^2.
$$

A related statistic is the **R**oot **M**ean **S**quared that has the same units as the values themselves:
$$
\operatorname{RMS}(\hat{y}, y) = \sqrt{\operatorname{MSE}(\hat{y}, y)}.
$$
:::
