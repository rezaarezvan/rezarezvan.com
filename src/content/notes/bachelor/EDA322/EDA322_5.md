...
---
In this part we'll cover arithmetic units. Specifically multipliers and dividers.
...
:::algorithm[Binary multiplication]{#alg:binary-multiplication}
1. **for** $i = 1$ to $n$, where $n$ = # of bits of the operands **do**
   1. **if** the $i$-th bit of the *multiplier* is 1 **then**
      1. Add *multiplicand* $\cdot 2^{i - 1}$ to the final product.
:::
...
Multiply the two magnitudes, if the signs of the operands differ, we negate our result.
...
![Carry forward when adding the partial products of two 4-bit numbers.](./imgs/carry_forward.svg "fig:carry-forward")
...
We represent the fixed point numbers using the `.` separator.
So the `1.3` format and specifically `1.011`. Where the `.` says what resolution we have.
...
$$
v = m \cdot 2^{e - x} = \sum_{i = 0}^{n - 1} m_i 2^{i - n} \cdot 2^{\sum_{i = 0}^{k - 1} e_i 2^i - x}.
$$
