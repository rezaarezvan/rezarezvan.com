...
* The set of test patterns (test suite) written to verify a design should be complete.
...
### Defects and Yield
$$
\text{yield} = \frac{\text{number of working chips produced}}{\text{total number of chips produced}}
$$

$$
\text{cost} = \frac{\text{cost of fabricating and testing a wafer}}{\text{yield} \cdot \text{number of chip sites on the wafer}}
$$
...
Therefore, a subset of all possible inputs is used when testing.
...
:::algorithm[Path sensitization]{#alg:path-sensitization}
...
* A conflict has occurred when two different justifications require different values assigned to a node.
