...
---
### Sequential Circuits
When we usually look at circuits we have an input and output that are independent of each other.
In a sequential circuit, as the name suggests, we have a sequence. Meaning we feed back our output as input.

We can easily define this as: Our output no longer depends on the present input - but on the past behavior of the circuit.

![The general form of a sequential circuit.](./imgs/seq_circuit.svg "fig:seq-circuit")
...
* Latch
    * Latches are **level-sensitive**
        * When the clock is *high*, the latch is **transparent** (`output <= input`)
...
Latches are also cheaper to implement than flip-flops, but flip-flops are easier to use in designs than latches.

We'll primarily look over the so-called D flip-flop.

### D Latch
![D latch.](./imgs/d_latch.svg "fig:d-latch")
...
#### Setup and hold time
The D input signal must be stable when clock is changing from $0 \to 1$ (setup).
...
* The output, $z$, should be equal to 1 if in two immediately preceding clock cycles the input $w$ was 1.
Otherwise, $z$ should be 0.

If we plan our state diagram we can say that:
...
:::algorithm[Sequential circuit design procedure]{#alg:seq-design}
1. Specifications
2. Derive the states, and create a state diagram
3. Create a state table
4. (State minimization)
5. State assignment
6. Choose flip-flops
7. Derive logic expressions for state and outputs
8. Implementation
:::
