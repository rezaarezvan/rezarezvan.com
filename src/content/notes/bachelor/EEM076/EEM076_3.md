...
---
When analyzing and computing with circuits, it's often a good idea to try to simplify as much as possible.

But when met with a circuit like:

![A circuit with three nodes.](./imgs/node_voltage_example.svg "fig:node-voltage-example")
...
![Node-voltage analysis, step by step.](./imgs/node_voltage_steps.svg "fig:node-voltage-steps")
...
$$
v_1 = 5\,\mathrm{V}, \quad v_2 = 2\,\mathrm{V}.
$$
...
![Mesh-current analysis example.](./imgs/mesh_current_example.svg "fig:mesh-current-example")
...
![Superposition example.](./imgs/superposition_example.svg "fig:superposition-example")
...
![The current source turned off.](./imgs/superposition_voltage_only.svg "fig:superposition-voltage-only")
...
\begin{align*}
V & = R \cdot I, \newline
3\,\mathrm{V} & = R_{eq} \cdot i^{\prime}_{x}, \newline
i^\prime_x & = \frac{3\,\mathrm{V}}{6\,\Omega + 9\,\Omega} = 0.2\,\mathrm{A}.
\end{align*}
$$

The circuit for $i^{\prime \prime}_x$ becomes:

![The voltage source turned off.](./imgs/superposition_current_only.svg "fig:superposition-current-only")
...
$$
\begin{align*}
i_x^{\prime \prime} & = i_{\text{total}} \cdot \frac{R_6}{R_6 + R_{i^{\prime \prime}_x}} \newline
& = 2\,\mathrm{A} \cdot \frac{6\,\Omega}{6\,\Omega + 9\,\Omega} = 0.8\,\mathrm{A}.
\end{align*}
$$
...
$$
i_x = i^\prime_x + i^{\prime \prime}_x = 0.2\,\mathrm{A} + 0.8\,\mathrm{A} = \boxed{1.0\,\mathrm{A}}.
$$
...
Luckily, there's a more powerful method to find equivalent circuits.

It's called the *Thévenin equivalent circuits* and *Norton equivalent circuits*.

The idea is the following:

![A complex network and its Thévenin equivalent.](./imgs/thevenin_idea.svg "fig:thevenin-idea")
...
:::algorithm[Thévenin equivalent]{#alg:thevenin}
...
![Thévenin example.](./imgs/thevenin_example.svg "fig:thevenin-example")

Let's now disconnect $R_L$ and find the open circuit voltage:

![The open circuit voltage.](./imgs/thevenin_open_circuit.svg "fig:thevenin-open-circuit")

$$
\begin{align*}
V_{oc} & = V_6 \quad \text{(since they are in parallel)}, \newline
V_6 & = V_{\text{total}} \cdot \frac{R_6}{R_3 + R_6} = 12\,\mathrm{V} \cdot \frac{6\,\Omega}{3\,\Omega + 6\,\Omega} = 8\,\mathrm{V}, \newline
V_{TH} & = V_{oc} = 8\,\mathrm{V}.
\end{align*}
$$

Notice how we didn't include the $R_7$, this is due to the fact that, no current is going through that resistor!

Next step, let's find the $R_{eq}$ for this circuit. **Don't forget to turn OFF all independent sources!**:

![The equivalent resistance with the source turned off.](./imgs/thevenin_resistance.svg "fig:thevenin-resistance")
...
![The Thévenin equivalent circuit.](./imgs/thevenin_final.svg "fig:thevenin-final")
...
![A complex network and its Norton equivalent.](./imgs/norton_idea.svg "fig:norton-idea")

The method for finding a Norton equivalent circuit is:

:::algorithm[Norton equivalent]{#alg:norton}
...
We'll be using this tool a lot in the future for analyzing circuits!
