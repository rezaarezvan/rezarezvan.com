...
If $C$ is a simple and closed curve, e.g. a circle. Then $C$ determines an inner region, $D$. In this case, we write $\partial D = C$. Meaning the border of $D$.
...
$$
\begin{align*}
\int_0^1 \int_0^{1 - x} y \, dy \, dx & = \int_0^1 \dfrac{y^2}{2} \bigg\rvert_{y = 0}^{y = 1 - x} \, dx \newline
& = \int_0^1 \dfrac{(1 - x)^2}{2} \, dx \newline
& = \dfrac{1}{2} \int_0^1 (1 - x)^2 \, dx \newline
& = \dfrac{1}{2} \int_0^1 1 - 2x + x^2 \, dx \newline
& = \dfrac{1}{2} \left[x - x^2 + \dfrac{x^3}{3}\right]_{x = 0}^{x = 1} \newline
& = \boxed{\dfrac{1}{6}}
\end{align*}
$$
:::
...
For, $C = \{(x, y) \mid x^2 + y^2 = 9\}$, with positive orientation.
...
$$
\begin{align*}
4 \int_0^{2\pi} \int_0^3 1 \cdot r \, dr \, d\theta & = 4 \int_0^{2\pi} \dfrac{9}{2} \, d\theta \newline
& = 4 \cdot 9 \pi = 36\pi.
\end{align*}
$$
:::
...
$$
\vec{r}(t) = \langle a \cos(t), b \sin(t) \rangle, \quad 0 \leq t \leq 2\pi.
$$
...
:::example
$$
\vec{r}(u, v) = \langle u \cos(v), u \sin(v), 1 \rangle, \quad 0 \leq u \leq 1, 0 \leq v \leq 2\pi.
$$

Plotting this parameterization:

![The unit disc at height $z = 1$, parameterized by $\vec{r}(u, v) = \langle u \cos(v), u \sin(v), 1 \rangle$.](./imgs/unit_disc.svg "fig:unit-disc")
:::
...
$$
x^2 + y^2 = 2, \quad 0 \leq z \leq 1.
$$

$$
\vec{r}(u, v) = \langle \sqrt{2} \cos(u), \sqrt{2} \sin(u), v \rangle, \quad 0 \leq u \leq 2\pi, 0 \leq v \leq 1.
$$

Plot:

![The cylinder $x^2 + y^2 = 2$, $0 \leq z \leq 1$.](./imgs/cylinder.svg "fig:cylinder")
:::
