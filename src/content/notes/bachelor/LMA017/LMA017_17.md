...
Let $C \subset D$ be parameterized as $\vec{r}(t)$, for $a \leq t \leq b$.
...
$$
:::proof
If $F$ is conservative, then there is a function, $f: D \subset \mathbb{R}^2 \rarr \mathbb{R}$ such that $\nabla f = F$. In other words, $f_x = P$ and $f_y = Q$.
...
$$
f(x, y) = \int f_x(x, y) \, dx = \int 3 + 2xy \, dx = 3x + x^2y + g(y).
$$
...
$$
f(x, y) = \int f_y(x, y) \, dy = \int x^2 - 3y^2 \, dy = x^2y - y^3 + g(x).
$$
...
$$
f(x, y) = x^2y - y^3 + 3x + C \quad \text{same!}
$$
:::
...
Suppose $F$ is a force field, suppose that it moves an object with mass, $m$, along a curve $C$, parameterized as $\vec{r}(t)$ for $a \leq t \leq b$. According to Newton's second law of motion:
$$
F = ma
$$
...
$$
W = \int_C F \cdot dr = \int_a^b F(\vec{r}(t)) \cdot \vec{r^\prime}(t) \, dt.
$$
...
& = \dfrac{m}{2} \left( \left| \vec{r^\prime}(b) \right|^2 - \left| \vec{r^\prime}(a) \right|^2 \right) \newline
& = \dfrac{m}{2} \left( \left| \vec{v}(b) \right|^2 - \left| \vec{v}(a) \right|^2 \right).
\end{align*}
$$
