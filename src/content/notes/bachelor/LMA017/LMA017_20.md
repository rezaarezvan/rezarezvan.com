...
$$
\vec{r_{\phi}} \times \vec{r_{\theta}} = \langle \sin^2(\phi) \cos(\theta), \sin^2(\phi) \sin(\theta), \sin(\phi) \cos(\phi) \rangle
$$

Now, one can solve this integral by taking the dot product and applying our definition:
$$
\int_0^{2\pi} \int_0^{\pi} \langle \cos(\phi), \sin(\phi) \sin(\theta), \sin(\phi) \cos(\theta) \rangle \cdot \langle \sin^2(\phi) \cos(\theta), \sin^2(\phi) \sin(\theta), \sin(\phi) \cos(\phi) \rangle \, d\phi \, d\theta
$$
...
However, imagine half the unit sphere with a filled base. The base with parameterization: $D = \{(r, \theta) \mid 0 \leq r \leq 1, 0 \leq \theta \leq 2\pi \}$. Is going to be a negative integral.
...
:::example
Find the flux of $\vec{F} = \langle z, y, x \rangle$ on the unit sphere.
$$
\operatorname{div} \vec{F} = \dfrac{\partial P}{\partial x} + \dfrac{\partial Q}{\partial y} + \dfrac{\partial R}{\partial z} = 0 + 1 + 0 = 1
$$
...
:::example
Compute $\int_C F \cdot dr$ for $\vec{F} = \langle -y^2, x, z^2 \rangle$ and $C$ being the intersection of $y + z = 2$ with $x^2 + y^2 = 1$.

Let's find the curl:
$$
\operatorname{curl} \vec{F} = \langle 0, 0, 1 + 2y \rangle
$$

We need to find the surface, the intersection of $y + z = 2$ and $x^2 + y^2 = 1$ will just be the unit disk but at a slight tilt.

$$
\iint_S \operatorname{curl} \vec{F} \cdot dS
$$
...
Meaning:
$$
\iint_S \operatorname{curl} \vec{F} \cdot dS = \iint_D \operatorname{curl} \vec{F} \cdot (\vec{r_u} \times \vec{r_v}) \, dA
$$
...
$$
\begin{align*}
\iint_D \operatorname{curl} \vec{F} \cdot (\vec{r_u} \times \vec{r_v}) \, dA & = \iint_D \langle 0, 0, 1 + 2y \rangle \cdot \langle 0, 1, 1 \rangle \, dA \newline
& = \iint_D 1 + 2y \, dA \newline
...
:::example
Compute $\iint_S \operatorname{curl} \vec{F} \cdot dS$ where $\vec{F} = \langle xz, yz, xy \rangle$ and $S$ is the part of the sphere of radius 2 around $(0, 0, 0)$ that lies inside the cylinder $x^2 + y^2 \leq 1$ and above the $xy$-plane.
...
$$
\begin{align*}
\vec{r}(t) & = \langle \cos(t), \sin(t), \sqrt{3} \rangle, \quad 0 \leq t \leq 2\pi, \newline
...
$$
\begin{align*}
\iint_S \operatorname{curl} \vec{F} \cdot dS & = \int_C \vec{F} \cdot dr \newline
& = \int_0^{2\pi} \langle \sqrt{3} \cos(t), \sqrt{3} \sin(t), \cos(t) \sin(t) \rangle \cdot \langle -\sin(t), \cos(t), 0 \rangle \, dt \newline
& = \int_0^{2\pi} -\sqrt{3} \cos(t) \sin(t) + \sqrt{3} \sin(t) \cos(t) \, dt \newline
& = \int_0^{2\pi} 0 \, dt = \boxed{0}
\end{align*}
$$
:::
