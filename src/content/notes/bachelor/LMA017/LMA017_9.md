...
### Double integrals
Say we have a function, $f(x, y)$ with rectangular domain, $R = [a, b] \times [c, d]$.

In mathematical terms: $\{(x, y) \mid a \leq x \leq b, c \leq y \leq d \}$.
...
Therefore:
$$
\lim_{\substack{\Delta x \to 0 \newline \Delta y \to 0}} \sum_{j = 1}^{m} \sum_{i = 1}^{n} f(x_{i}^{*}, y_{j}^{*}) \Delta A = \text{volume under the graph}
$$
...
Assume that the domain is a rectangular box, $I = [a, b] \times [c, d] \times [e, f]$

We use the same approach, divide into $n, m, l$ subintervals, the volume of one of these sub-boxes will be:
$$
...
### Iterated integrals
Assume we have a function, $f$, on $R = [a, b] \times [c, d]$.
...
is a function that will depend on $y$, the value we get out from this, is only dependent on $y$. Therefore:
$$
...
This is what we call iterated integrals. We can also do the other way around!
$$
...
But do these functions yield the same integral?

:::theorem[Fubini's Theorem]
Let, $f$, be continuous function on rectangle, $R = [a, b] \times [c, d]$. Then:
$$
...
:::example
Find $\iint_R (x - 3y^2) \, dA$, where $R = [0, 2] \times [1, 2]$.
...
Let's do the integral the other way around as well!
$$
...
$$
\left(\dfrac{x^2}{2} - 7x\right) \bigg\rvert_{x = 0}^{x = 2}
$$
...
:::example
Find $\iiint_R xz \cdot e^{xy} \, dV$ on the rectangle, $R = [0, 1] \times [0, 1] \times [0, 1] = [0, 1]^3$.
...
$$
\int_0^1 \left(\dfrac{e^x}{2} - \dfrac{1}{2}\right) \, dx
$$

$$
\left(\dfrac{e^x}{2} - \dfrac{x}{2}\right) \bigg\rvert_{x = 0}^{x = 1}
$$

$$
\left(\dfrac{e}{2} - \dfrac{1}{2}\right) - \left(\dfrac{1}{2} - 0\right) = \boxed{\dfrac{e}{2} - 1}
$$
:::
...
:::example
Find $\iint_R y \cdot \sin(xy) \, dA$ on the rectangle, $R = [1, 2] \times [0, \pi]$.
