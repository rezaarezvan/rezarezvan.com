...
Before we head into doing a few examples, it's a good practice to always draw where our $D$ lies. So we easily get a geometrical sense of how it lies.
...
$$
\dfrac{1}{2} \int_{-1}^1 \left((1 + x^2)^2 - (2x^2)^2\right) \, dx
$$
...
$$
\int_{-1}^{2} \left(\dfrac{3}{2} y^3 + y^2 - \dfrac{y^5}{2}\right) \, dy
$$

$$
\left(\dfrac{3}{8} y^4 + \dfrac{y^3}{3} - \dfrac{y^6}{12}\right) \bigg\rvert_{y = -1}^{y = 2} = \ldots = \boxed{\dfrac{27}{8}}
$$
:::
...
Graphing this we can see that this is a region of both types.

For, type I:
$$
\iint_D f(x, y) \, dA = \int_0^1 \int_x^1 \sin(y^2) \, dy \, dx
$$
