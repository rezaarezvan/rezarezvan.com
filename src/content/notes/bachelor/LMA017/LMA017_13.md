...
$$
\iint_D f(x, y) \, dA = \iint_S f(x(u, v), y(u, v)) \left\vert \dfrac{\partial(x, y)}{\partial(u, v)} \right\vert \, du \, dv
$$
:::
...
If we plot our trapezoid, we find that it is a type II region. The left curve being $u = -v$ and the right curve $u = v$.

Which means our integral becomes:
$$
\dfrac{1}{2} \int_1^2 \int_{-v}^{v} e^{\frac{u}{v}} \, du \, dv
$$

$$
\dfrac{1}{2} \int_1^2 ve^{\frac{u}{v}} \bigg\rvert_{u = -v}^{u = v} \, dv
$$
...
$$
\iiint_E f(x, y, z) \, dV = \iiint_S f(x(u, v, w), y(u, v, w), z(u, v, w)) \left\vert \dfrac{\partial(x, y, z)}{\partial(u, v, w)} \right\vert \, du \, dv \, dw
$$
...
$$
\dfrac{\partial(x, y, z)}{\partial(r, \theta, z)} = 0 \cdot \det
\begin{bmatrix}
-r \sin(\theta) & 0 \newline
r \cos(\theta) & 0
...
$$
\dfrac{\partial(x, y, z)}{\partial(r, \theta, z)} = r \cos^2(\theta) + r \sin^2(\theta) = r(\cos^2(\theta) + \sin^2(\theta)) = \boxed{r}
$$
...
$$
E = \{(x, y, z) \mid 0 \leq x^2 + y^2 \leq 1, 1 - x^2 - y^2 \leq z \leq 4\}
$$

In spherical coordinates:
$$
S = \{(r, \theta, z) \mid 0 \leq r \leq 1, 1 - r^2 \leq z \leq 4\}
$$

Let's make $z$ our inner variable.
$$
\iiint_E \sqrt{x^2 + y^2} \, dV = \iiint_S \sqrt{r^2} \cdot r \, dV
$$
...
$$
\int_0^{2\pi} \left(r^3 + \dfrac{r^5}{5}\right) \bigg\rvert_{r = 0}^{r = 1} \, d\theta
$$
...
\theta & - \text{angle in } xy\text{-plane}
\end{align*}
$$
...
The Jacobian:
$$
\dfrac{\partial(x, y, z)}{\partial(\rho, \varphi, \theta)} =
\det
\begin{bmatrix}
...
$$
\dfrac{\partial(x, y, z)}{\partial(\rho, \varphi, \theta)} = \ldots = \rho^2 \sin(\varphi)
$$
...
Find
$$
\iiint_B e^{(x^2 + y^2 + z^2)^{\frac{3}{2}}} \, dV
$$
...
In spherical coordinates:
$$
S = \{(\rho, \varphi, \theta) \mid 0 \leq \rho \leq 1, 0 \leq \varphi \leq \pi, 0 \leq \theta \leq 2\pi \}
$$

Which is easy to integrate over.
$$
\iiint_B e^{(x^2 + y^2 + z^2)^{\frac{3}{2}}} \, dV = \iiint_S e^{(\rho^2)^{\frac{3}{2}}} \cdot \rho^2 \sin(\varphi) \, dV
$$
...
Now the solid is placed on a table, such that 2/3 of its length, counting from the left, lies on the table.
Does the rectangular solid fall off the table?
