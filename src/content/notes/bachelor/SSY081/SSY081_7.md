...
### Filters
The idea with filters is quite intuitive, we want to remove unwanted components from a signal.
...
For a physically realizable system (which means causal):
$$
h(t) = 0, \quad t < 0.
$$
...
& = \dfrac{1}{T} \sum_{n = -\infty}^{\infty} F(\omega - n\omega_s).
\end{align*}
$$
...
Using that $\omega_s = 2\pi \mathcal{F}_s$ and $\omega_B = 2\pi B$:
$$
\begin{align*}
2\pi \mathcal{F}_s & > 2(2\pi B) \newline
2\pi \mathcal{F}_s & > 4\pi B \newline
\mathcal{F}_s & > 2B.
\end{align*}
$$
:::
