...
& = (b_m D^m + b_{m - 1}D^{m - 1} + \ldots + b_1 D + b_0)f(t).
\end{align*}
$$
...
For $t \geq 0$, a system's **output** is the *result* of two independent causes.
...
$$
\begin{align*}
y_0(t) & = \dfrac{c}{2} e^{j \theta} e^{(\alpha + j \beta)t} + \dfrac{c}{2} e^{-j \theta} e^{(\alpha - j \beta)t} \newline
& = \dfrac{c}{2} e^{\alpha t} \left[ e^{j(\beta t + \theta)} + e^{-j(\beta t + \theta)}\right].
\end{align*}
$$
