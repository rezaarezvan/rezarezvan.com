...
![Pipeline with two hardware threads and thread selection.](./imgs/multithreading.svg "fig:interleaved-multithreading")
...
![Pipeline with two hardware threads and thread selection.](./imgs/multithreading.svg "fig:blocked-multithreading")
...
![Simultaneous multithreading issuing to several functional units.](./imgs/sm.svg "fig:smt")
...
$$
\text{Speed up} = \dfrac{1}{x + \dfrac{1 - x}{N}}.
$$
...
But in today's computers we mostly see multicore processors. These are chips that consist of multiple processors (cores).
...
![Single instruction, single data (SISD).](./imgs/sisd.svg "fig:sisd")

SIMD, or, Single Instruction Multiple Data. This model is when we fetch a single instruction, but perform multiple instances of that instruction. This is a very powerful for performing vectorized operations.

Note, each processor has its own **local** memory and data.

![Single instruction, multiple data (SIMD).](./imgs/simd.svg "fig:simd")

The next class is, MIMD, or Multiple Instruction Multiple Data. As the name suggests, it's basically parallelizing SIMD.

![Multiple instruction, multiple data (MIMD).](./imgs/mimd.svg "fig:mimd")
...
For our **S**hared **M**emory multi**P**rocessor, we need to use synchronization primitives, to avoid cache invalidation.
