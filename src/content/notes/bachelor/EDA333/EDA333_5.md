...
The locality of reference is the tendency that programs, when accessing memory, usually access memory addresses that are the same or very close to each other.
...
:::definition[Average memory access time]
...
Let's say we have 128 bytes of memory, we divide our memory into blocks of 16 bytes, meaning 8 total blocks.
...
* **Where** shall the block be placed?
...
* Have some *index* bits that indicate what *index* we have to look at, along with the tag and offset bits.
* If we miss:
...
$$
\text{Index in cache} = (\text{Block address}) \bmod (\text{Cache size}).
$$
...
* Miss cycles per instruction ($\Delta \mathrm{CPI}_{\text{cache}}$)
    * I-cache misses: $\Delta \mathrm{CPI}_{\text{instruction}} = 0.02 \cdot 100 = 2$
    * D-cache misses: $\Delta \mathrm{CPI}_{\text{data}} = 0.36 \cdot 0.04 \cdot 100 = 1.44$
