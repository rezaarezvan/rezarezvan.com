...
![Datapath with a forwarding unit.](./imgs/FU.png "fig:forwarding")
...
![Pipelined datapath with forwarding and control.](./imgs/FU2.png "fig:forwarding-control")
...
### Jump conflicts
Remember that conditional jump instructions evaluate in the `EX` stage.
...
#### Delay jumping
This strategy makes use of these stall cycles, so we **always** execute the next instruction after the jump.
...
* MIPS handles exceptions by a so-called "System Control Coprocessor" (CP0)
...
* Do not update the `$s1` register.
* Every instruction that was **before** this one needs to complete.
* Eliminate the effect the instruction had and "flush" the instructions that came after.
* Set the cause and EPC registers.
* Jump to the exception handler.

So our final CPU looks like:

![Pipelined datapath with hazard detection and exception handling.](./imgs/PE.png "fig:exceptions")
...
![Two-bit branch predictor as a finite-state machine.](./imgs/jfsm.svg "fig:branch-predictor")
...
### Loop unrolling
Loop unrolling is a quite simple concept, we just replicate the loop code to achieve more parallelism.
...
* Dynamical scheduling and speculative execution:
    * Hazards limit the achievable parallelism.
