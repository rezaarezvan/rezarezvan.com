...
* Programming language, compiler, and architecture of choice.
...
$$
\text{CPU Time} = \mathrm{IC} \cdot \mathrm{CPI} \cdot T_c.
$$
:::

One way to actually then measure computers or programs is using SPEC.

SPEC, or **S**tandard **P**erformance **E**valuation **C**orp, is a company which specializes in measuring.
...
### MIPS
In this series we will use a MIPS instruction set, MIPS is still used widely in the real world, but it's slowly fading away due to RISC-V's popularity.
...
All arithmetic operations have this form, two source registers and one destination register.
...
MIPS has a $32 \times 32$-bit **register file** (RF).
...
#### Conditional jumps
In MIPS there are only two conditional jumps, `beq` and `bne`:
```
