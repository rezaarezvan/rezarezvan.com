...
![Translating a virtual address to a physical address through the page table.](./imgs/page_table.svg "fig:page-table")

![The page table maps virtual pages to physical memory or disk.](./imgs/page_table_mapping.svg "fig:page-table-mapping")
...
Misses at the TLB can both be handled by hardware and software.

![The TLB caches recent page-table entries.](./imgs/tlb.svg "fig:tlb")
...
:::algorithm[Page fault routine]{#alg:page-fault}
1. Localize the page in secondary memory
2. Choose which page should be replaced:
    1. If dirty, need to write back to secondary memory.
3. Read in the page to memory and update the table.
4. Make the program runnable again (OS level).
    1. Restart the instruction which caused the page fault.
:::
...
* Virtual memory
    * Write-back is the only sane option due to **long** access time to the disk.
