...
---
In this part we'll cover the idea of pipelining - and its performance effects it has on computer systems.
...
![Single-cycle datapath with control.](./imgs/SL.png "fig:single-cycle")
...
![Instruction fetch: the PC, instruction memory and the PC + 4 adder.](./imgs/fetch.svg "fig:fetch")
...
![Register file and ALU for R-format instructions.](./imgs/register_file.svg "fig:register-file")
...
![Data memory and sign-extension unit for load/store instructions.](./imgs/load_store.svg "fig:load-store")
...
![Single-cycle datapath in detail.](./imgs/TP.png "fig:datapath-detail")
...
![Five-stage pipelined datapath.](./imgs/PL.png "fig:pipeline")

Note: The dotted lines represent registers that separate each stage
...
### Hazards
:::definition[Hazards]
There are three different types of hazards that we'll need to take care of:
...
#### Structural hazards
As we defined earlier, it is when two or more instructions need to access the same stage simultaneously.
