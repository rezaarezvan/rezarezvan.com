...
I have read much of the discourse, both from pure mathematicians to engineers, over the last few years as AI systems have become progressively more capable [^2].
...
Naturally, I want to provide my slice to the discourse, as I feel like that many focus on what the AI systems ought to do, missing the bigger picture.
...
Naturally, if our output is something that is testable, (formally) verifiable, and is paried with a cheat sheet, search will be king.
Consquently, with the rapid expansion, investigation, and optimization of modern computer hardware, this has enabled these AI systems to triumph.

I remember being interested in search (and conversely entropy) very early on, as it always appeared as a natural solution to many problems to me.
There exists a finite amount of programs with a maximum of 1000 characters of source code written in Python that are able to correctly solve [Fizz buzz](https://en.wikipedia.org/wiki/Fizz_buzz).
It is clear that, by utilizing heuristics and constraints, we are able to reduce the search space substanially and being able to find sufficient solutions.
However, solving a problem and understanding a _tasteful_ solution are two completely disjoint events.

## The Turn
Since ~December of last year, the software community has been aflame in terms of what (i) the goals of software
