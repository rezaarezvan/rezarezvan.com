...
These last six months have been quite a turbulent time for many scientific disciplines.
...
I have read much of the discourse, from both pure mathematicians and software engineers, during this period as AI systems have become progressively more capable [^2].
...
As the discourse continues, I want to provide my slice to it as a [newly appointed PhD student in AI](/essays/science), as my peers and I, in both academia and industry, are at the imminent frontier of the future.
...
I remember playing around in the GPT-3 playground with the `davinci-002` model early on.
I remember being fascinated by the OpenAI Five project and how such a simple algorithm, when paired with extensive heuristics and compute, could solve such a complex objective.
For me, these were the first times I felt the _magic_ of AI.

I hear expressions like this quite often: "feel the magic", "feel the AGI", "can you feel it anon?"
...
Since I am a computer engineer, I will mainly focus on programming, partly motivated because AI code generation is one of the cornerstones of the evolving frontier, but mainly since this is where my expertise lies.
...
Here, I am assuming that the people responsible for solving these problems are knowledgeable and not students.
I have worked professionally (although I dislike this choice of wording) in the software industry, as well as been fortunate enough to have been surrounded by both smart and competent software engineers for a large portion of my life.
In both my anecdotal experience and from what I have gathered from industry veterans, you always know _a solution_; however, it is often not the perfect fit in the specific context and potentially violates the expected input-output invariants that are constraining.

Practical software has always been about creating verifiable systems that solve exact problems.
Naturally, if our output is something that is testable, (formally) verifiable, and often paired with a cheat sheet, it is not surprising that search will be king.
Even before modern AI code generation, people understood this and extensively researched how one can effectively search for good computer programs @cite:Gulwani2017.
The ambition to automate programming is almost as old as AI itself.
Consequently, the rapid expansion, investigation, and optimization of modern computer hardware have enabled "search is king" to finally triumph.
...
I have been interested in search (and conversely, in entropy) since my undergrad days, as it always appeared as a natural solution to many problems.
...
I have also spent the last ~10 years understanding how computers and software are built from the ground up [^5].
...
However, I could not disagree more with people who, usually "tech bros" as they are colloquially called, argue that churning out (sloppy) AI-generated code is the way forward and the natural step upon Jacob's software ladder.
...
I have seen this quote from both anti-AI and pro-AI folks.
...
However, fully understanding and internalizing the codebase takes time for humans, even when you are handwriting all the code it takes a lot of time, although at a much closer relationship.
This is why I dislike when people (miss)use the quote above, we are not turning our thinking over to the machines, we are turning the scale of search over to the machines.

On the more academic side, [Asking Authors About Their Own Papers](https://medium.com/@TmlrOrg/asking-authors-about-their-own-papers-3d2e04e5dee0) from [Transactions on Machine Learning Research](https://jmlr.org/tmlr/) recently highlighted this issue.
...
Many of these topics are political, or at least influenced and steered by politics, but it is clear that the current systems were not built for this scale of search, and I have seen many proposals that, in summary, say "either make everything open and let us fight for reputation again" or "it is time that we return to our ivory towers, secretive work is what lies ahead".
...
We have opened Pandora's Search Box and there is no going back.
The magic felt by these AI systems is simply the extensive ability to search, at scale, but what is the catch? What is the surprising last act of the magic trick?
...
Like many others, I grew up with [The Social Network](https://en.wikipedia.org/wiki/The_Social_Network) and the idea that anyone, even with the simplest of tools, but with the highest of ambitions, can build anything [^6].
...
Now, we return to ["tokenmaxxing"](https://tokenmaxxing.com/guides/what-is-tokenmaxxing).
By treating the number of tokens and use of AI code generation as a metric for output and performance, I no longer believe in the sentiment that anyone can build anything with the simplest tools.
...
Let me clarify what I mean by cost here, I do not mean costly in the sense that AI inherently costs money, software also requires computational resources which costs money, the difference is that access to sufficient computation is becoming part of the creation of software itself.
...
I find "Tokenmaxxing" to be interesting precisely because it is an cultural manifestation of this, more tokens, larger contexts, more agents, more attempts.
If one failed attempt costs us nothing, we can simply search again, again, and again! if the performance frontier of a swarm agents perform better than a single agent, but at a higher cost and not necessarily Pareto dominated, we still want to run the swarm! if a larger context window allows you to fit the entire codebase in-context, we are of course going to buy the model with the largest context window!?
But ultimatelty we have to ask ourselves, does generating ten times as much code make us capable of understanding code ten times faster?
A compiler can verify the syntax and compile a source program to machine code for free, a test suite can verify the expected behavior of how many specifications we want, and a formal system may even verify stated invariants.
...
What was once a bottleneck in construction of software has increasingly become a bottleneck in the ability in specification, verification, and understanding, we are producing technically "correct" systems whose rate of quantity exceeds our rate of comprehension and understanding.
...
I want to highlight that I am not a trained mathematician nor do I have extensive knowledge about the problems that have so far been solved by AI systems, but from what I have been told by people who understand these problems is that "things check out" (usually).
...
[^5]: The motto of the [CSE program](https://www.chalmers.se/en/departments/cse/) here is basically "build a computer and understand how it works".
[^6]: "You can just do things", an infamous saying on tech Twitter.
