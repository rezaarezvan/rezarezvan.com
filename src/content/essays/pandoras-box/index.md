...
From solving [^1] longstanding open problems and conjectures in various domains, to [solving one of the Millennium Prize Problems](https://openai.com/index/navier-stokes-solution/), AI systems and the modern labs have formally chosen to step into the realm of science.
...
As the discourse continues, I want to provide my slice to it as a [newly appointed PhD student in AI](/essays/science), as my peers and I, in both academia and industry, are at the imminent frontier of the future where AI might be the cornerstone.
...
Naturally, if our output is something that is testable, (formally) verifiable, and often paired with a cheat sheet of sorts, it is not surprising that search will be king.
...
Consequently, the rapid expansion, investigation, and optimization of modern computer hardware have enabled "search is king" to materialize over the decades and this decade it was time for triumph.
Just as Sutton argued in the Bitter Lesson, one of the prerequisites is that we need scale, without the absurd advancements in modern computer hardware, we would not have been here; appreciate the shoulders of the giants we are standing on.
...
Alas, I argue that the capability of these AI systems is relative to the actor utilizing them and largely in what context, as the training corpus and what knowledge can be retrieved have a physical limitation in what can be retrieved from the weights.
...
If you have spent at least one second on tech Twitter, I believe that you have seen someone quote this famous Dune quote:

> Once men turned their thinking over to machines in the hope that this would set them free.
But that only permitted other men with machines to enslave them. [^7]

I want to comment on and dissect this quote; I have seen countless horror stories of people building sophisticated projects and tools, and I have even sometimes fallen into this trap myself when "vibecoding" various curiosities: once the token limit has been hit, they are not able to do work on their own, becoming trapped under the "search overlords".
...
This is why I dislike when people (mis)use the quote above, we are not turning our thinking over to the machines, we are turning over the scale of search, time, and effort to the machines, all of which are a proxy of understanding.
In many settings, execution, results, and progress are appreciated myopic measures that drive direction, if this is a correct, just, and valuable system and what possible alternatives there exist is hard to answer and political in many cases, but I suspect that these are not the best measures for longevity.
...
While it is hard to predict which of these extremes, or what mixture of them, will play out, I personally believe in and hope for the former more than the latter.
But I digress from this side point, although it is something that will be on my mind, and on the minds of other (to-be) tenured seniors, for a while.
...
Now, we return to the idea of ["tokenmaxxing"](https://tokenmaxxing.com/guides/what-is-tokenmaxxing).
...
Perhaps, but this might only be the fate of practical software and software that requires output in the form of revenue or something of interest.
All of this leads me to believe that practical software is going to become costly.
Let me clarify what I mean by costly: it is not only the cost and access to AI systems, traditional software also requires computational resources which have a cost, the difference is that access to sufficient tools is becoming part of the creation of software itself.
...
But what is happening today is that the amount of software you produce is becoming inherently proportional to the amount of search you can afford.
"Tokenmaxxing" is interesting because it is the precise cultural manifestation of this phenomenon: abuse search, more agents, larger contexts, tokenmaxxing.
The ethos of tokenmaxxing is greedy and myopic: if one failed search attempt costs nothing, why not just search again, again, and again until success? If a swarm of agents performs better than a single agent, but at a higher cost, we still want to run the swarm of course! If a larger context window allows you to fit the entire codebase in-context, we are of course going to buy the model with the largest context window!?
...
If we have access to god-like black-boxes in the form of search overlords, in combination with guardrails that ensure the exact behaviour we want, does it matter?
None of these gives the tokenmaxxing engineer behind it a compressed understanding of why the underlying system and source code looks the way it does or what would need to change when the world around it changes.
The tokenmaxxing engineer simply needs to adhere to the search overlords and accept this fate, or spend an immense amount of time digesting the codebase, its tests, and specifications, and verifying everything the old-fashioned way.
This is what I argue is the reason for the update of the quote above and why I believe practical software is going to become costly.

In the end, we are bestowed with the idea that, the bottleneck in the creation of software is no longer low-level understanding, laborious effort, and time, but rather lies in precise problem specification, formal verification, and high-level understanding, presented as the veil of producing technically "correct" systems whose rate of quantity exceeds the rate of comprehension and understanding, we are sold this illusion of the magic trick of AI systems and their ability to search.
Whether this will be objectively better, in the sense that mundane everyday tasks, or even solving open frontier problems, will be done in a more resource efficient manner while yielding the same if not better performance, is a task of an oracle to tell us.
However, it is possible to compare the different debts these two entail, the resulting debt of tokenmaxxing is something closer to epistemic debt rather than traditional technical debt that has plagued the software industry over the decades: the ever-growing distance between our understanding of the underlying and intricate details we possess about the artifacts we are creating will grow proportional to the amount of access we have the models searching, creating, and giving us artifacts that we "possess".
...
I want to highlight that I am not a trained mathematician nor do I have extensive knowledge about the problems that have so far been solved by AI systems, but what I have been told by people who understand these problems is that "things check out" usually, although at the price of being sloppy.
...
[^8]: Update, as of writing this we are seeing reports of over [60 000 submissions](https://x.com/denny_zhou/status/2101474965405253708/photo/1) to [ICLR 2027](https://iclr.cc/).
